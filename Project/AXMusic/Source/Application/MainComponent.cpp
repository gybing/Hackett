...
void MainContentComponent::OnResponse(DouBanRequestID id, int err /*= 0*/, String message /*= String::empty*/)
{
    ...
    case IDRequest_Douban_Channels:
    {
        ...
        {
            if (const TDChannel* channel = HDouBanMusic::getInstance()->getChanelByIdx(comboBox->getSelectedId() - 1))
    ...
    {
        if (const TDSong* s = HDouBanMusic::getInstance()->getCurrentSong())
        {
            URL url(s->songUrl);
            musicPlayer.playSound(*url.createInputStream(false));
            updateSongInfo(s);
        }
...
{
    const TDChannel* channel = HDouBanMusic::getInstance()->getChanelByIdx(comboBox->getSelectedId() - 1);
