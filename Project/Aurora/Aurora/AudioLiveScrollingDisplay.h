...
public:
    ...
    {
        m_waveFormatEx = *pFormat;
    }

    virtual VOID OnCatpureData(LPBYTE pData, INT nDataLen)
    {
#define FFT_LEN 2048
        double finleft[FFT_LEN/2];
        double finright[FFT_LEN/2];
        double fout[FFT_LEN];
        ...
        {
            {
                ...
                finleft[nCount] = (double)((short*)pData)[dw++];
                //copy audio signal to fft real component for right channel
                finright[nCount++] = (double)((short*)pData)[dw];
            }
        }
        ...
        for(int i=1;i < FFT_LEN/4;i++)//Use FFT_LEN/4 since the data is mirrored within the array.
        {
            ...
            float ff = ((int)fdraw[i]) % 100;
            pushSample (ff); // boost the level to make it more easily visible.
        }
...
private:
    ...
    void paint (Graphics& g) override
    {
        ...
        for (int x = jmin (getWidth(), (int) numElementsInArray (samples)); --x >= 0;)
        {
            ...
            waveform.addWithoutMerging (juce::Rectangle<float> ((float) x, midY - sampleSize, 1.0f, sampleSize * 2.0f));
        }
