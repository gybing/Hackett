#ifndef JUCE_GRIDVIEW_H_INCLUDED
#define JUCE_GRIDVIEW_H_INCLUDED

//==============================================================================
/**
    A subclass of this is used to drive a LXGridView.

    @see LXGridView
*/
class LXGridViewModel
{
public:
    //==============================================================================
    /** Destructor. */
    virtual ~LXGridViewModel()  {}

    //==============================================================================
    /** This has to return the number of items in the list.
        @see LXGridView::getNumIndexs()
    */
    virtual int getNumItems() = 0;

    /** This method must be implemented to draw a row of the list.
        Note that the index value may be greater than the number of rows in your
        list, so be careful that you don't assume it's less than getNumIndexs().
    */
    virtual void paintGridViewItem (int index,
                                   Graphics& g,
    ...
    /** This is used to create or update a custom component to go in a row of the list.
        ...
        Any row may contain a custom component, or can just be drawn with the paintLXGridViewItem() method
        and handle mouse clicks with gridViewItemClicked().

        This method will be called whenever a custom component might need to be updated - e.g.
        when the table is changed, or TableLXGridView::updateContent() is called.
        ...
        The component that your method returns will be deleted by the LXGridView when it is no longer needed.
    ...
    */
    virtual Component* refreshComponentForIndex (int index, bool isIndexSelected,
                                               Component* existingComponentToUpdate);

    /** This can be overridden to react to the user clicking on a row.
        @see gridViewItemDoubleClicked
    */
    virtual void gridViewItemClicked (int row, const MouseEvent&);

    /** This can be overridden to react to the user double-clicking on a row.
        @see gridViewItemClicked
    */
    virtual void gridViewItemDoubleClicked (int row, const MouseEvent&);
    ...
    */
    ...
    /** Override this to be informed when rows are selected or deselected.
        ...
        @param lastIndexSelected      the last row that the user selected. If no
                                    rows are currently selected, this may be -1.
    */
    virtual void selectedItemsChanged (int lastIndexSelected);

    /** Override this to be informed when the delete key is pressed.

        If no rows are selected when they press the key, this won't be called.

        @param lastIndexSelected   the last row that had been selected when they pressed the
                                 key - if there are multiple selections, this might not be
                                 very useful
    */
    virtual void deleteKeyPressed (int lastIndexSelected);
    ...
    /** Override this to be informed when the return key is pressed.
        ...
        @param lastIndexSelected   the last row that had been selected when they pressed the
                                 key - if there are multiple selections, this might not be
                                  very useful
    */
    virtual void returnKeyPressed (int lastIndexSelected);
    ...
    */
    virtual void gridViewWasScrolled();
    ...
    */
    ...
    /** You can override this to provide tool tips for specific rows.
        @see TooltipClient
    */
    virtual String getTooltipForIndex (int row);

    /** You can override this to return a custom mouse cursor for each row. */
    virtual MouseCursor getMouseCursorForIndex (int row);
...
/**
    ...
    To create a list, you'll need to create a subclass of LXGridViewModel. This can
    ...
    @see ComboBox, TableLXGridView
*/
class JUCE_API  LXGridView  : public Component,
                           public SettableTooltipClient
{
public:
    //==============================================================================
    /** Creates a LXGridView.

        The model pointer passed-in can be null, in which case you can set it later
        with setModel().
    */
    LXGridView (const String& componentName = String::empty,
             LXGridViewModel* model = nullptr);
    ...
    void setModel (LXGridViewModel* newModel);

    /** Returns the current list model. */
    LXGridViewModel* getModel() const noexcept                     { return model; }
    ...
    /** Causes the list to refresh its content.
        ...
        to call refreshComponentForIndex() on all the row components.
    ...
    */
    ...
    /** Turns on multiple-selection of rows.
        ...
        selectIndexsBasedOnModifierKeys() method to tell the list that it's been
    ...
    */
    void setClickingTogglesIndexSelection (bool flipIndexSelection) noexcept;
    ...
    */
    void setMouseMoveSelectsIndexs (bool shouldSelect);
    ...
    /** Selects a row.
        ...
        @param dontScrollToShowThisIndex  if true, the list's position won't change; if false and
        ...
        @see isIndexSelected, selectIndexsBasedOnModifierKeys, flipIndexSelection, deselectIndex,
             deselectAllIndexs, selectRangeOfIndexs
    */
    void selectItem (int index,
                    bool dontScrollToShowThisIndex = false,
                    bool deselectOthersFirst = true);

    /** Selects a set of rows.

        This will add these rows to the current selection, so you might need to
        clear the current selection first with deselectAllIndexs()

        @param firstIndex     the first row to select (inclusive)
        @param lastIndex      the last row to select (inclusive)
    */
    void selectRangeOfIndexs (int firstIndex,
                            int lastIndex);

    /** Deselects a row.
        If it's not currently selected, this will do nothing.
        @see selectIndex, deselectAllIndexs
    */
    ...
    */
    ...
    /** Returns the number of rows that are currently selected.
        @see getSelectedIndex, isIndexSelected, getLastIndexSelected
    */
    int getNumSelectedIndexs() const;
    ...
    /** Returns the row number of a selected row.
        ...
        @param index    the index of the selected row to return, (from 0 to getNumSelectedIndexs() - 1)
        @returns        the row number, or -1 if the index was out of range or if there aren't any rows
                        selected
        @see getNumSelectedIndexs, isIndexSelected, getLastIndexSelected
    */
    int getSelectedIndex (int index = 0) const;
    ...
    */
    int getLastIndexSelected() const;
    ...
    /** Multiply-selects rows based on the modifier keys.
        ...
        @see selectIndex
    */
    void selectIndexsBasedOnModifierKeys (int rowThatWasClickedOn,
    ...
    */
    ...
    void scrollToEnsureIndexIsOnscreen (int row);
    ...
    /** Finds the row index that contains a given x,y position.
        The position is relative to the LXGridView's top-left.
        If no row exists at this position, the method will return -1.
        @see getComponentForIndexNumber
    */
    int getIndexContainingPosition (int x, int y) const noexcept;
    ...
    /** Finds a row index that would be the most suitable place to insert a new
        ...
        beyond the end of the list, it will return getNumIndexs() to indicate the end
        of the list.
    ...
    */
    Rectangle<int> getItemPosition (int index,
                                   bool relativeToComponentTopLeft) const noexcept;

    /** Finds the row component for a given row in the list.

        The component returned will have been created using createIndexComponent().

        If the component for this row is off-screen or if the row is out-of-range,
        this will return nullptr.

        @see getIndexContainingPosition
    */
    Component* getComponentForIndexNumber (int index) const noexcept;
    ...
    */
    int getIndexNumberOfComponent (Component* rowComponent) const noexcept;

    /** Returns the width of a row (which may be less than the width of this component
        if there's a scrollbar).
    */
    int getVisibleIndexWidth() const noexcept;
    ...
    */
    int getNumIndexsOnScreen() const noexcept;
    ...
    */
    ...
    /** Sets the thickness of a border that will be drawn around the box.
        ...
        To set the colour of the outline, use @code setColour (LXGridView::outlineColourId, colourXYZ); @endcode
        @see outlineColourId
    */
    ...
    */
    void setItemWidth (int newWidth);
    ...
    */
    void repaintItem (int index) noexcept;
    ...
    */
    virtual Image createSnapshotOfIndexs (const SparseSet<int>& rows, int& x, int& y);

    /** Returns the viewport that this LXGridView uses.
...
private:
    ...
    JUCE_PUBLIC_IN_DLL_BUILD (class ItemComponent)
    friend class ListViewport;
    friend class TableLXGridView;
    LXGridViewModel* model;
    ScopedPointer<ListViewport> viewport;
    ScopedPointer<Component> headerComponent;
    ScopedPointer<MouseListener> mouseMoveSelector;
    int totalItems, totalLines, horizonCount, itemHeight, itemWidth;
    ...
    void selectItemInternal (int index, bool dontScrollToShowThisIndex,
                            bool deselectOthersFirst, bool isMouseClick);

   #if JUCE_CATCH_DEPRECATED_CODE_MISUSE
    // This method's bool parameter has changed: see the new method signature.
    JUCE_DEPRECATED (void setSelectedIndexs (const SparseSet<int>&, bool));
    // This method has been replaced by the more flexible method createSnapshotOfIndexs.
    // Please call createSnapshotOfIndexs (getSelectedIndexs(), x, y) to get the same behaviour.
    JUCE_DEPRECATED (virtual void createSnapshotOfSelectedIndexs (int&, int&)) {}
   #endif

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LXGridView)
};


#endif   // JUCE_GRIDVIEW_H_INCLUDED
