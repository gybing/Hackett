...
CXDocumentWindow::CXDocumentWindow (const String& title,
    ...
    : ResizableWindow (title, backgroundColour, addToDesktop_),
      ...
      borderSize(size),
      titleJustification(juce::Justification::centred)
{
...
void CXDocumentWindow::setTitle (const juce::String& newName, juce::Justification justification /*= juce::Justification::centred*/)
{
    if (newName != getName() || titleJustification != justification)
    {
...
void CXDocumentWindow::paint (Graphics& g)
{
    ...
    BorderSize<int> size=getBorderThickness();
    ((LookAndFeelBase*)&getLookAndFeel())->drawCXDocumentWindowTitleBar (*this, g,
                                                 titleBarArea.getWidth(),
                                                 titleBarHeight,
                                                 titleSpaceX1,
                                                 jmax (1, titleSpaceX2 - titleSpaceX1),
                                                 titleBarIcon.isValid() ? &titleBarIcon : 0,
                                                 ! drawTitleTextCentred, titleJustification);
}
...
{
    if (borderSize != BorderSize<int>(-1))
        return borderSize;
...
void CXDocumentWindow::lookAndFeelChanged()
{
    ...
    {
        LookAndFeelBase& lf = *(LookAndFeelBase*)&getLookAndFeel();

        if ((requiredButtons & minimiseButton) != 0)  titleBarButtons[0] = lf.createCXDocumentWindowButton (minimiseButton);
        if ((requiredButtons & maximiseButton) != 0)  titleBarButtons[1] = lf.createCXDocumentWindowButton (maximiseButton);
        if ((requiredButtons & closeButton)    != 0)  titleBarButtons[2] = lf.createCXDocumentWindowButton (closeButton);
...
void CXDocumentWindow::setBorderThickness(juce::BorderSize<int> border)
{
    borderSize = border;
    resized();
}

const juce::String& CXDocumentWindow::getTitle() const
{
    return title;
}
