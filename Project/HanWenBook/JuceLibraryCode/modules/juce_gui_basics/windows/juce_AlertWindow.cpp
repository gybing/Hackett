...
//==============================================================================
void AlertWindow::addButton (bool line, Colour color, const String& name,
                             const int returnValue,
                             const KeyPress& shortcutKey1,
                             const KeyPress& shortcutKey2)
{
    ColorButton* const b = new ColorButton();
    buttons.add (b);

    b->setButtonText(name);
    b->setColour(ColorButton::ColourIds::buttonColourNormalId, color);
    b->setColour(ColorButton::ColourIds::buttonColourHoverId, color.contrasting(0.2f));
    b->setColour(ColorButton::ColourIds::buttonColourDownId, color.contrasting(0.4f));

    if (line)
    {
        b->setButtonBorder(1);
        b->setColour(ColorButton::ColourIds::textColourNormalId, color);
        b->setColour(ColorButton::ColourIds::textColourHoverId, color.contrasting(0.2f));
        b->setColour(ColorButton::ColourIds::textColourDownId, color.contrasting(0.4f));
    }
    else
    {
        b->setButtonBorder(0);
        b->setColour(ColorButton::ColourIds::textColourNormalId, juce::Colours::white);
        b->setColour(ColorButton::ColourIds::textColourHoverId, juce::Colours::white);
        b->setColour(ColorButton::ColourIds::textColourDownId, juce::Colours::white);
    }
...
void AlertWindow::triggerButtonClick (const String& buttonName)
{
    ...
    {
        ColorButton* const b = buttons.getUnchecked(i);
...
void AlertWindow::updateLayout (const bool onlyIncreaseSize)
{
    ...
    const int wid = messageFont.getStringWidth (text);
    ...
    w = jmax (245, (int) textLayout.getWidth() + edgeGap * 4);
    ...
    for (int i = 0; i < buttons.size(); ++i)
    {
        ColorButton* const c = buttons.getUnchecked(i);
