...
dex byte-code in various places in the H framework. These are the steps
required to re-generate the dex byte-code from any Java source code inside the
native/java subfolders:

1. Create a new H android project with the minimal sdk version which is
...
resemble their package, i.e. a Java class com.roli.H.HelloWorld.java
should be in the module's native/javacore/app/com/roli/H folder.
If you wish to modify existing .java files in the H modules then just rename
...
included into H. You can use the Projector's BinaryData generator
