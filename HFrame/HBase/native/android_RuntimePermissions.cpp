...
//==============================================================================
static String HPermissionToAndroidPermission (RuntimePermissions::PermissionID permission)
{
    switch (permission)
    {
...

        {
            ...
            if (requests.size() > 0)
            {
                ...
                StringArray permissionsArray{
                        HPermissionToAndroidPermission (request.permission)};
                auto jPermissionsArray = HStringArrayToJava (permissionsArray);
                ...
                // be available
                HAssert(requestPermissionsMethodID != 0);
...
{
    auto requestedPermission = HPermissionToAndroidPermission (permission);
    ...
    if (! isPermissionDeclaredInManifest (requestedPermission))
    {
        ...
        // the Projector. Otherwise this can't work.
        HAssertfalse;
