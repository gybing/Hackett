...
private:
    ...
    HDECLARE_NON_COPYABLE (JNIClassBase)
};
...
#define HLOG_JNI_REFERENCES_TABLE getEnv()->CallStaticVoidMethod (AndroidDebug, AndroidDebug.dumpReferenceTables);
...
namespace
{
    inline String HString (JNIEnv* env, jstring s)
    {
        if (s == 0)
            return {};
    ...
    inline String HString (jstring s)
    {
        return HString (getEnv(), s);
    }
    ...
    inline LocalRef<jobjectArray> HStringArrayToJava (const StringArray& HArray)
    {
        auto* env = getEnv();

        LocalRef<jobjectArray> result (env->NewObjectArray ((jsize) HArray.size(),
                                                            JavaString,
                                                            javaString ("").get()));

        for (int i = 0; i < HArray.size(); ++i)
            env->SetObjectArrayElement (result, i, javaString (HArray [i]).get());
    ...
    inline StringArray javaStringArrayToJuce (const LocalRef<jobjectArray>& javaArray)
    {
        ...
        for (int i = 0; i < env->GetArrayLength (javaArray.get()); ++i)
        {
            ...
            result.add (HString (javaString.get()));
        }
...
//==============================================================================
jobject invokeImplementer (JNIEnv*, jobject, jlong, jobject, jobject, jobjectArray);
void    dispatchDelete    (JNIEnv*, jobject, jlong);
...
protected:
    ...
    friend jobject invokeImplementer (JNIEnv*, jobject, jlong, jobject, jobject, jobjectArray);
    friend void    dispatchDelete    (JNIEnv*, jobject, jlong);
private:
