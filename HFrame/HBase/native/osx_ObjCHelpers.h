...
{
    return [NSURL fileURLWithPath: HStringToNS (f)];
}
...
static inline NSArray* createNSArrayFromStringArray (const StringArray& strings)
{
    ...
    for (auto string: strings)
        [array addObject:HStringToNS (string)];
...
static NSDictionary* varObjectToNSDictionary (const var& varToParse)
{
    ...
    if (varToParse.isObject())
    {
        ...
        {
            auto* keyString = HStringToNS (properties.getName (i).toString());
            ...
            {
                auto* valueString = HStringToNS (valueVar.toString());
...
static NSArray* varArrayToNSArray (const var& varToParse)
{
    ...
    for (const auto& aVar : *varArray)
    {
        ...
        {
            auto* valueString = HStringToNS (aVar.toString());
...
// this will return an NSObject which takes ownership of the H instance passed-in
// This is useful to tie the life-time of a H instance to the life-time of an NSObject
template <typename Class>
...
// Get the H class instance that was tied to the life-time of an NSObject with the
// function above
template <typename Class>
