...
JNIEnv* getEnv() noexcept
{
    ...
    // You did not call Thread::initialiseH which must be called at least once in your apk
    // before using any H APIs. The Projector will automatically generate java code
    // which will invoke Thread::initialiseH for you.
    HAssertfalse;
    return nullptr;
}

void JNICALL JavainitialiseH (JNIEnv* env, jobject /*jclass*/, jobject context)
{
    Thread::initialiseH (env, context);
}

extern "C" jint JNIEXPORT JNI_OnLoad (JavaVM* vm, void*)
{
    // Huh? JNI_OnLoad was called two times!
    HAssert (androidJNIJavaVM == nullptr);
    ...
    auto HJavaClass = env->FindClass("com/roli/H/Java");

    if (HJavaClass != nullptr)
    {
        JNINativeMethod method {"initialiseH", "(Landroid/content/Context;)V",
                                reinterpret_cast<void*> (JavainitialiseH)};

        auto status = env->RegisterNatives (HJavaClass, &method, 1);
        HAssert (status == 0);
    }
    else
    {
        // com.roli.H.Java class not found. Apparently this project is a library
        // or was not generated by the Projector. That's ok, the user will have to
        // call Thread::initialiseH manually
        env->ExceptionClear();
    }
        ...
        if (mainActivityClassPath.isEmpty())
        {
            ...
            if (appContext != nullptr)
            {
                ...
                if (resolveInfo != nullptr)
                {
                    ...
                    mainActivityClassPath = HString (jName);
                }
            }
        }
...
void Thread::initialiseH (void* jniEnv, void* context)
{
    static CriticalSection cs;
    ScopedLock lock (cs);

    // jniEnv and context should not be null!
    HAssert (jniEnv != nullptr && context != nullptr);
    ...
    if (androidJNIJavaVM == nullptr)
    {
        ...
        HAssert (status == 0 && javaVM != nullptr);
...
LocalRef<jobject> getAppContext() noexcept
{
    ...
    // before using any H APIs. The Projector will automatically generate java code
    // which will invoke Thread::initialiseH for you.
    HAssert (env != nullptr && context != nullptr);
...
API void CALLTYPE Process::setPriority (ProcessPriority prior)
{
    // TODO
