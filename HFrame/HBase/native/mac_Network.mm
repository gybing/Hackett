                                                      ...
                                                      const StringArray& filesToAttach)
{
    ...
    HAUTORELEASEPOOL
    {
        ...
        NSAppleScript* s = [[NSAppleScript alloc] initWithSource: HStringToNS (script)];
...
#if HIOS || (defined (MAC_OS_X_VERSION_10_10) && MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_10)
...
private:
    ...
    {
        DelegateClass()  : ObjCClass<NSObject> ("HURLDelegate_")
        {
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (URLConnectionState)
};
...
struct BackgroundDownloadTask  : public URL::DownloadTask
{
           ...
           uniqueIdentifier (String (urlToUse.toString (true).hashCode64()) + String (Random().nextInt64()))
    {
        ...
        NSMutableURLRequest* request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:HStringToNS (urlToUse.toString (true))]];
        ...
        session =
            [NSURLSession sessionWithConfiguration: [NSURLSessionConfiguration backgroundSessionConfigurationWithIdentifier: HStringToNS (uniqueIdentifier)]
                                          delegate: delegate
    ...
    {
        DelegateClass()  : ObjCClass<NSObject<NSURLSessionDelegate>> ("HURLDelegate_")
        {
...
void URL::DownloadTask::iosURLSessionNotify (const String& identifier)
{
...
private:
    ...
    struct DelegateClass  : public ObjCClass<NSObject>
    {
        DelegateClass()  : ObjCClass<NSObject> ("HNetworkDelegate_")
        {
            addIvar<URLConnectionState*> ("state");
...
private:
    ...
    {
        HAssert (connection == nullptr);

        if (NSURL* nsURL = [NSURL URLWithString: HStringToNS (url.toString (! isPost))])
        {
