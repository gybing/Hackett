...
struct CURLSymbols
{
    ...
    static std::unique_ptr<CURLSymbols> create()
    {
       ...
       #if HLOAD_CURL_SYMBOLS_LAZILY
        const ScopedLock sl (getLibcurlLock());
        #define HINIT_CURL_SYMBOL(name)  if (! symbols->loadSymbol (symbols->name, #name)) return nullptr;
       #else
        #define HINIT_CURL_SYMBOL(name)  symbols->name = ::name;
       #endif

        HINIT_CURL_SYMBOL (curl_easy_init)
        HINIT_CURL_SYMBOL (curl_easy_setopt)
        HINIT_CURL_SYMBOL (curl_easy_cleanup)
        HINIT_CURL_SYMBOL (curl_easy_getinfo)
        HINIT_CURL_SYMBOL (curl_multi_add_handle)
        HINIT_CURL_SYMBOL (curl_multi_cleanup)
        HINIT_CURL_SYMBOL (curl_multi_fdset)
        HINIT_CURL_SYMBOL (curl_multi_info_read)
        HINIT_CURL_SYMBOL (curl_multi_init)
        HINIT_CURL_SYMBOL (curl_multi_perform)
        HINIT_CURL_SYMBOL (curl_multi_remove_handle)
        HINIT_CURL_SYMBOL (curl_multi_timeout)
        HINIT_CURL_SYMBOL (curl_slist_append)
        HINIT_CURL_SYMBOL (curl_slist_free_all)
        HINIT_CURL_SYMBOL (curl_version_info)
...
public:
    ...
    {
        HAssert (symbols); // Unable to load libcurl!
