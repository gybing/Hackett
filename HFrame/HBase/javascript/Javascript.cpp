...
{
    #define HDECLARE_JS_TOKEN(name, str)  static const char* const name = str;
    HJS_KEYWORDS  (HDECLARE_JS_TOKEN)
    HJS_OPERATORS (HDECLARE_JS_TOKEN)
    HDECLARE_JS_TOKEN (eof,        "$eof")
    HDECLARE_JS_TOKEN (literal,    "$literal")
    HDECLARE_JS_TOKEN (identifier, "$identifier")
}
...
struct JavascriptEngine::RootObject   : public DynamicObject
{
    ...
    struct Scope
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Scope)
    };
    ...
    struct Statement
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Statement)
    };
    ...
    private:
        ...
        TokenType matchNextToken()
        {
            if (isIdentifierStart (*p))
            {
                ...
                #define HJS_COMPARE_KEYWORD(name, str) if (len == sizeof (str) - 1 && matchToken (TokenTypes::name, len)) return TokenTypes::name;
                HJS_KEYWORDS (HJS_COMPARE_KEYWORD)
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ExpressionTreeBuilder)
    };
