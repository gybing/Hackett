...
/**
    ...
    interpreter, which is integrated with the H var and DynamicObject classes. If you need
    ...
    @tags{Core}
*/
class API JavascriptEngine  final
{
public:
...
private:
    HPUBLIC_IN_DLL_BUILD (struct RootObject)
    const ReferenceCountedObjectPtr<RootObject> root;
    void prepareTimeout() const noexcept;

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JavascriptEngine)
};
