...
bool OutputStream::writeText (const String& text, bool asUTF16, bool writeUTF16ByteOrderMark, const char* lf)
{
    ...
    HAssert (lf == nullptr || replaceLineFeedWithWindows || replaceLineFeedWithUnix);
...
API OutputStream& CALLTYPE operator<< (OutputStream& stream, const int number)
{
    writeIntToStream (stream, number);
    return stream;
}

API OutputStream& CALLTYPE operator<< (OutputStream& stream, const int64 number)
{
    writeIntToStream (stream, number);
    return stream;
}

API OutputStream& CALLTYPE operator<< (OutputStream& stream, const double number)
{
    return stream << String (number);
}

API OutputStream& CALLTYPE operator<< (OutputStream& stream, const char character)
{
    stream.writeByte (character);
    return stream;
}

API OutputStream& CALLTYPE operator<< (OutputStream& stream, const char* const text)
{
    stream.write (text, strlen (text));
    return stream;
}

API OutputStream& CALLTYPE operator<< (OutputStream& stream, const MemoryBlock& data)
{
    if (data.getSize() > 0)
...
API OutputStream& CALLTYPE operator<< (OutputStream& stream, const File& fileToRead)
{
...
API OutputStream& CALLTYPE operator<< (OutputStream& stream, InputStream& streamToRead)
{
    stream.writeFromInputStream (streamToRead, -1);
    return stream;
}

API OutputStream& CALLTYPE operator<< (OutputStream& stream, const NewLine&)
{
