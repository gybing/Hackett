...
public:
    ...
    var clone (const var& original) const override
    {
        ...
        HAssertfalse; // can only clone DynamicObjects!
        return {};
    }

    void writeToStream (const ValueUnion&, OutputStream& output) const override
    {
        HAssertfalse; // Can't write an object to a stream!
...
public:
    ...
    void writeToStream (const ValueUnion&, OutputStream& output) const override
    {
        HAssertfalse; // Can't write a method to a stream!
        output.writeCompressedInt (0);
    }
};
...
HDECLARE_DEPRECATED_STATIC (const var var::null;)
...
const var& var::operator[] (int arrayIndex) const
{
    ...
    // must be in-range!
    HAssert (array != nullptr && isPositiveAndBelow (arrayIndex, array->size()));
