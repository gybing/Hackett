...
public:
    ...
    */
    ...
    {
        HAssert (elements != nullptr);
        HAssert (isPositiveAndBelow (index, numUsed));
    ...
    {
        HAssert ((const void*) this != (const void*) &arrayToAddFrom); // can't add from our own elements!
    ...
    {
        HAssert (indexToRemoveAt >= 0);
        HAssert (numElementsToRemove >= 0);
        HAssert (indexToRemoveAt + numElementsToRemove <= numUsed);
...
private:
    ...
    void checkSourceIsNotAMember (const ElementType* element)
    {
        ...
        HAssert (element < begin() || element >= end());
    ...
    HDECLARE_NON_COPYABLE (ArrayBase)
};
