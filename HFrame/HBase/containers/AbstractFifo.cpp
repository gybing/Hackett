...
{
    HAssert (numWritten >= 0 && numWritten < bufferSize);
...
{
    HAssert (numRead >= 0 && numRead <= bufferSize);
    ...
    struct WriteThread  : public Thread
    {
        ...
        void run()
        {
            ...
            while (! threadShouldExit())
            {
                ...
                HAssert (writer.blockSize1 >= 0 && writer.blockSize2 >= 0);
                HAssert (writer.blockSize1 == 0
                         || (writer.startIndex1 >= 0 && writer.startIndex1 < fifo.getTotalSize()));
                HAssert (writer.blockSize2 == 0
