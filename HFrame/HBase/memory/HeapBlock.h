...
#if ! (defined (DOXYGEN) || HEXCEPTIONS_DISABLED)
...
private:
    ...
    void throwOnAllocationFailure() const
    {
        ...
        HAssert (data != nullptr); // without exceptions, you'll need to find a better way to handle this failure case.
       #else
   ...
   #if ! (defined (HDLL) || defined (HDLL_BUILD))
    HDECLARE_NON_COPYABLE (HeapBlock)
    HPREVENT_HEAP_ALLOCATION // Creating a 'new HeapBlock' would be missing the point!
   #endif
};
