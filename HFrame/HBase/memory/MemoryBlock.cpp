...
MemoryBlock::MemoryBlock (const void* const dataToInitialiseFrom, const size_t sizeInBytes)
    : size (sizeInBytes)
{
    HAssert (((ssize_t) sizeInBytes) >= 0);

    if (size > 0)
    {
        HAssert (dataToInitialiseFrom != nullptr); // non-zero size, but a zero pointer passed-in?
...
void MemoryBlock::append (const void* srcData, size_t numBytes)
{
    if (numBytes > 0)
    {
        HAssert (srcData != nullptr); // this must not be null!
        auto oldSize = size;
