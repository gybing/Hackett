...
public:
    ...
    HDEPRECATED_ATTRIBUTE inline ScopedPointer() = default;

    // ScopedPointer is deprecated! You should use std::unique_ptr instead.
    HDEPRECATED_ATTRIBUTE inline ScopedPointer (decltype (nullptr)) noexcept {}

    // ScopedPointer is deprecated! You should use std::unique_ptr instead.
    HDEPRECATED_ATTRIBUTE inline ScopedPointer (ObjectType* objectToTakePossessionOf) noexcept
    ...
    // ScopedPointer is deprecated! You should use std::unique_ptr instead.
    HDEPRECATED_ATTRIBUTE inline ~ScopedPointer()         { reset(); }
    ...
    ScopedPointer& operator= (ScopedPointer& objectToTransferFrom)
    {
        if (this != objectToTransferFrom.getAddress())
        {
            ...
            HAssert (object == nullptr || object != objectToTransferFrom.object);
    ...
    void swapWith (ScopedPointer<ObjectType>& other) noexcept
    {
        // Two ScopedPointers should never be able to refer to the same object - if
        // this happens, you must have done something dodgy!
        HAssert (object != other.object || this == other.getAddress() || object == nullptr);
...
private:
   ...
   #if ! HMSVC  // (MSVC can't deal with multiple copy constructors)
