...
HCONSTEXPR Type jmax (Type a, Type b)                                   { return a < b ? b : a; }

/** Returns the larger of three values. */
template <typename Type>
HCONSTEXPR Type jmax (Type a, Type b, Type c)                           { return a < b ? (b < c ? c : b) : (a < c ? c : a); }

/** Returns the larger of four values. */
template <typename Type>
HCONSTEXPR Type jmax (Type a, Type b, Type c, Type d)                   { return jmax (a, jmax (b, c, d)); }

/** Returns the smaller of two values. */
template <typename Type>
HCONSTEXPR Type jmin (Type a, Type b)                                   { return b < a ? b : a; }

/** Returns the smaller of three values. */
template <typename Type>
HCONSTEXPR Type jmin (Type a, Type b, Type c)                           { return b < a ? (c < b ? c : b) : (c < a ? c : a); }

/** Returns the smaller of four values. */
template <typename Type>
HCONSTEXPR Type jmin (Type a, Type b, Type c, Type d)                   { return jmin (a, jmin (b, c, d)); }

/** Remaps a normalised value (between 0 and 1) to a target range.
    This effectively returns (targetRangeMin + value0To1 * (targetRangeMax - targetRangeMin)).
*/
template <typename Type>
HCONSTEXPR Type jmap (Type value0To1, Type targetRangeMin, Type targetRangeMax)
{
...
{
    HAssert (sourceRangeMax != sourceRangeMin); // mapping from a range of zero will produce NaN!
...
{
    HAssert (lowerLimit <= upperLimit); // if these are in the wrong order, results are unpredictable..
...
{
    HAssert (Type1() <= static_cast<Type1> (upperLimit)); // makes no sense to call this if the upper limit is itself below zero..
...
{
    HAssert (upperLimit >= 0); // makes no sense to call this if the upper limit is itself below zero..
...
HCONSTEXPR int numElementsInArray (Type (&)[N]) noexcept     { return N; }
...
template <typename Type>
Type hypot (Type a, Type b) noexcept
{
   #if HMSVC
...
*/
const HCONSTEXPR double  double_Pi  = MathConstants<double>::pi;
...
/** A single-precision constant for pi.
    ...
    wrongly thinking it meant 2 * pi !
*/
const HCONSTEXPR float   float_Pi   = MathConstants<float>::pi;
#endif

/** Converts an angle in degrees to radians. */
template <typename FloatType>
HCONSTEXPR FloatType degreesToRadians (FloatType degrees) noexcept     { return degrees * (MathConstants<FloatType>::pi / FloatType (180)); }

/** Converts an angle in radians to degrees. */
template <typename FloatType>
HCONSTEXPR FloatType radiansToDegrees (FloatType radians) noexcept     { return radians * (FloatType (180) / MathConstants<FloatType>::pi); }
...
bool isfinite (NumericType) noexcept
{
...
{
    HAssert (value >= static_cast<FloatType> (0));
    HAssert (static_cast<FloatType> (value) <= std::numeric_limits<unsigned int>::max());
...
HCONSTEXPR bool isPowerOfTwo (IntegerType value)
{
...
template <typename NumericType>
inline HCONSTEXPR NumericType square (NumericType n) noexcept
{
    return n * n;
}
...
//==============================================================================
#if HINTEL || defined (DOXYGEN)
 /** This macro can be applied to a float variable to check whether it contains a denormalised
     value, and to normalise it if necessary.
     On CPUs that aren't vulnerable to denormalisation problems, this will have no effect.
 */
 #define HUNDENORMALISE(x)   { (x) += 0.1f; (x) -= 0.1f; }
#else
 #define HUNDENORMALISE(x)
#endif
...
//==============================================================================
 ...
 HDEPRECATED_ATTRIBUTE inline int roundDoubleToInt (double value) noexcept  { return roundToInt (value); }
 HDEPRECATED_ATTRIBUTE inline int roundFloatToInt  (float  value) noexcept  { return roundToInt (value); }

 // This old function isn't needed - just use std::abs() instead
 HDEPRECATED_ATTRIBUTE inline int64 abs64 (int64 n) noexcept                { return std::abs (n); }
#endif
