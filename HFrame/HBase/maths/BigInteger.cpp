...
{
    HAssert (n != 0); // (the built-in functions may not work for n = 0)
...
{
    HAssert (heapAllocation != nullptr || allocatedSize <= numPreallocatedInts);
...
uint32 BigInteger::getBitRangeAsInt (const int startBit, int numBits) const noexcept
{
    if (numBits > 32)
    {
        HAssertfalse;  // use getBitRange() if you need more than 32 bits..
        numBits = 32;
    }
...
void BigInteger::divideBy (const BigInteger& divisor, BigInteger& remainder)
{
    ...
    HAssert (this != &remainder); // (can't handle passing itself in to get the remainder)
...
BigInteger& BigInteger::operator|= (const BigInteger& other)
{
    ...
    HAssert (isNegative() == other.isNegative());
...
//==============================================================================
OutputStream& CALLTYPE operator<< (OutputStream& stream, const BigInteger& value)
{
...
String BigInteger::toString (const int base, const int minimumNumCharacters) const
{
    ...
    {
        HAssertfalse; // can't do the specified base!
        return {};
    }
...
void writeLittleEndianBitsInBuffer (void* buffer, uint32 startBit, uint32 numBits, uint32 value) noexcept
{
    ...
    HAssert (numBits > 0 && numBits <= 32);
    HAssert (numBits == 32 || (value >> numBits) == 0);
