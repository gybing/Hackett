...
struct ThreadPool::ThreadPoolThread  : public Thread
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ThreadPoolThread)
};
...
ThreadPoolJob::~ThreadPoolJob()
{
    ...
    // to remove it first!
    HAssert (pool == nullptr || ! pool->contains (this));
}
...
{
    HAssert (numThreads > 0); // not much point having a pool without any threads!
...
bool ThreadPool::runNextJob (ThreadPoolThread& thread)
{
    if (auto* job = pickNextJobToRun())
    {
        ...
        {
            HAssertfalse; // Your runJob() method mustn't throw any exceptions!
        }
