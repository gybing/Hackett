...
struct CurrentThreadHolder   : public ReferenceCountedObject
{
    ...
    HDECLARE_NON_COPYABLE (CurrentThreadHolder)
};
...
void Thread::threadEntryPoint()
{
    ...
    {
        HAssert (getCurrentThreadId() == threadId.get());
        ...
        {
            HAssertfalse; // Your run() method mustn't throw any exceptions!
        }
    }
...
void API threadEntryPoint (void* userData)
{
...
Thread* CALLTYPE Thread::getCurrentThread()
{
...
bool Thread::waitForThreadToExit (const int timeOutMilliseconds) const
{
    ...
    HAssert (getThreadId() != getCurrentThreadId() || getCurrentThreadId() == ThreadID());
...
bool Thread::stopThread (const int timeOutMilliseconds)
{
    ...
    // would that work??
    HAssert (getCurrentThreadId() != getThreadId());
...
bool Thread::setPriority (int newPriority)
{
    ...
    HAssert (isThreadRunning() && (isRealtime == isAndroidRealtimeThread));
...
struct LambdaThread  : public Thread
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LambdaThread)
};
...
//==============================================================================
bool CALLTYPE Process::IsRunningUnderDebugger() noexcept
{
    return isRunningUnderDebugger();
}
...
public:
    ...
    void runTest() override
    {
      ...
      #if ! H64BIT_ATOMICS_UNAVAILABLE  // 64-bit intrinsics aren't available on some old platforms
