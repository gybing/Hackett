...
#if defined (HSTRINGS_ARE_UNICODE) && ! HSTRINGS_ARE_UNICODE
 #error "HSTRINGS_ARE_UNICODE is deprecated! All strings are now unicode by default."
#endif

#if HNATIVE_WCHAR_IS_UTF8
 using CharPointer_wchar_t = CharPointer_UTF8;
#elif HNATIVE_WCHAR_IS_UTF16
...
private:
    ...
    void compileTimeChecks()
    {
       ...
       #if HNATIVE_WCHAR_IS_UTF8
        static_assert (sizeof (wchar_t) == 1, "HNATIVE_WCHAR_IS_* macro has incorrect value");
       #elif HNATIVE_WCHAR_IS_UTF16
        static_assert (sizeof (wchar_t) == 2, "HNATIVE_WCHAR_IS_* macro has incorrect value");
       #elif HNATIVE_WCHAR_IS_UTF32
        static_assert (sizeof (wchar_t) == 4, "HNATIVE_WCHAR_IS_* macro has incorrect value");
       #else
...
HDECLARE_DEPRECATED_STATIC (const String String::empty;)
    ...
    : text (StringHolder::createFromCharPointer (CharPointer_UTF8 (t)))
{
    /*  If you get an assertion here, then you're trying to create a string from 8-bit data
        ...
        Note that the Projector has a handy string literal generator utility that will convert
        any unicode string to a valid C++ string literal, creating ascii escape sequences that will
        work in any compiler.
    */
    // HAssert (t == nullptr || CharPointer_UTF8::isValidString (t, std::numeric_limits<int>::max()));
}
    ...
    : text (StringHolder::createFromCharPointer (CharPointer_ASCII (t), maxChars))
{
    /*  If you get an assertion here, then you're trying to create a string from 8-bit data
        ...
        work in any compiler.
    */
    HAssert (t == nullptr || CharPointer_ASCII::isValidString (t, (int) maxChars));
}
...
String String::charToString (wchar character)
{
...
namespace NumberToStringConverters
{
    ...
    static char* doubleToString (char* buffer, double n, int numDecPlaces, bool useScientificNotation, size_t& len) noexcept
    {
        ...
        HAssert (len <= charsNeededForDouble);
        return buffer;
    }
...
wchar String::operator[] (int index) const noexcept
{
    HAssert (index == 0 || (index > 0 && index <= (int) text.lengthUpTo ((size_t) index + 1)));
    return text [index];
}
...
//==============================================================================
API bool CALLTYPE operator== (const String& s1, const String& s2) noexcept            { return s1.compare (s2) == 0; }
API bool CALLTYPE operator!= (const String& s1, const String& s2) noexcept            { return s1.compare (s2) != 0; }
...
{
    HAssert (startOfTextToAppend.getAddress() != nullptr && endOfTextToAppend.getAddress() != nullptr);

    auto extraBytesNeeded = getAddressDifference (endOfTextToAppend.getAddress(),
                                                  startOfTextToAppend.getAddress());
    HAssert (extraBytesNeeded >= 0);
...
#if ! HNATIVE_WCHAR_IS_UTF32
String& String::operator+= (wchar ch)
{
    const wchar asString[] = { ch, 0 };
...
//==============================================================================
API String CALLTYPE operator+ (const char* s1, const String& s2)    { String s (s1); return s += s2; }
API String CALLTYPE operator+ (const wchar_t* s1, const String& s2) { String s (s1); return s += s2; }

API String CALLTYPE operator+ (char s1, const String& s2)           { return String::charToString ((wchar) (uint8) s1) + s2; }
API String CALLTYPE operator+ (wchar_t s1, const String& s2)        { return String::charToString (s1) + s2; }

API String CALLTYPE operator+ (String s1, const String& s2)         { return s1 += s2; }
API String CALLTYPE operator+ (String s1, const char* s2)           { return s1 += s2; }
API String CALLTYPE operator+ (String s1, const wchar_t* s2)        { return s1 += s2; }
API String CALLTYPE operator+ (String s1, const std::string& s2)    { return s1 += s2.c_str(); }

API String CALLTYPE operator+ (String s1, char s2)                  { return s1 += s2; }
API String CALLTYPE operator+ (String s1, wchar_t s2)               { return s1 += s2; }

#if ! HNATIVE_WCHAR_IS_UTF32
API String CALLTYPE operator+ (wchar s1, const String& s2)     { return String::charToString (s1) + s2; }
API String CALLTYPE operator+ (String s1, wchar s2)            { return s1 += s2; }
API String& CALLTYPE operator<< (String& s1, wchar s2)         { return s1 += s2; }
#endif
...
API String& CALLTYPE operator<< (String& s1, const wchar_t* s2)     { return s1 += s2; }
API String& CALLTYPE operator<< (String& s1, const String& s2)      { return s1 += s2; }
API String& CALLTYPE operator<< (String& s1, StringRef s2)          { return s1 += s2; }
API String& CALLTYPE operator<< (String& s1, const std::string& s2) { return s1 += s2.c_str(); }

API String& CALLTYPE operator<< (String& s1, uint8  number)         { return s1 += (int) number; }
API String& CALLTYPE operator<< (String& s1, short  number)         { return s1 += (int) number; }
API String& CALLTYPE operator<< (String& s1, int    number)         { return s1 += number; }
API String& CALLTYPE operator<< (String& s1, long   number)         { return s1 += String (number); }
API String& CALLTYPE operator<< (String& s1, unsigned long number)  { return s1 += String (number); }
API String& CALLTYPE operator<< (String& s1, int64  number)         { return s1 += String (number); }
API String& CALLTYPE operator<< (String& s1, uint64 number)         { return s1 += String (number); }
API String& CALLTYPE operator<< (String& s1, float  number)         { return s1 += String (number); }
API String& CALLTYPE operator<< (String& s1, double number)         { return s1 += String (number); }

API OutputStream& CALLTYPE operator<< (OutputStream& stream, const String& text)
{
    return operator<< (stream, StringRef (text));
}

API OutputStream& CALLTYPE operator<< (OutputStream& stream, StringRef text)
{
...
//==============================================================================
int String::indexOfChar (wchar character) const noexcept
{
    return text.indexOf (character);
}

int String::indexOfChar (int startIndex, wchar character) const noexcept
{
    auto t = text;
...
int String::lastIndexOfChar (wchar character) const noexcept
{
    auto t = text;
    int last = -1;
...
bool String::containsChar (const wchar character) const noexcept
{
...
String String::paddedLeft (const wchar padCharacter, int minimumLength) const
{
    HAssert (padCharacter != 0);
...
String String::paddedRight (const wchar padCharacter, int minimumLength) const
{
    HAssert (padCharacter != 0);
...
String String::replaceCharacter (const wchar charToReplace, const wchar charToInsert) const
{
...
String String::replaceCharacters (StringRef charactersToReplace, StringRef charactersToInsertInstead) const
{
    ...
    HAssert (charactersToReplace.length() == charactersToInsertInstead.length());
...
bool String::startsWithChar (const wchar character) const noexcept
{
...
bool String::endsWithChar (const wchar character) const noexcept
{
    HAssert (character != 0); // strings can't contain a null character!
...
String String::toHexString (const void* const d, const int size, const int groupSize)
{
    ...
    for (int i = 0; i < size; ++i)
    {
        ...
        dest.write ((wchar) hexDigits [nextByte >> 4]);
        dest.write ((wchar) hexDigits [nextByte & 0xf]);
...
String String::createStringFromData (const void* const unknownData, int size)
{
    ...
    if (size == 1)
        return charToString ((wchar) data[0]);
         ...
         || CharPointer_UTF16::isByteOrderMarkLittleEndian (data))
    {
        ...
        if (CharPointer_UTF16::isByteOrderMarkBigEndian (data))
        {
            for (int i = 0; i < numChars; ++i)
                builder.write ((wchar) ByteOrder::swapIfLittleEndian (src[i]));
        }
        else
        {
            for (int i = 0; i < numChars; ++i)
                builder.write ((wchar) ByteOrder::swapIfBigEndian (src[i]));
        }
...
//==============================================================================
static const wchar emptyChar = 0;
...
struct StringEncodingConverter
{
    static CharPointerType_Dest convert (const String& s)
    {
       ...
       #if HDEBUG // (This just avoids spurious warnings from valgrind about the uninitialised bytes at the end of the buffer..)
...
struct StringCopier
{
    ...
    {
        HAssert (((ssize_t) maxBufferSizeBytes) >= 0); // keep this value positive!
...
String String::fromUTF8 (const char* const buffer, int bufferSizeBytes)
{
    if (buffer != nullptr)
    {
        ...
        if (bufferSizeBytes > 0)
        {
            HAssert (CharPointer_UTF8::isValidString (buffer, bufferSizeBytes));
   ...
   #endif
{
    ...
    /*  If you get an assertion here, then you're trying to create a string from 8-bit data
        ...
        the compiler, source code editor and platform.
    */
    HAssert (CharPointer_ASCII::isValidString (stringLiteral, std::numeric_limits<int>::max()));
   #endif
}
