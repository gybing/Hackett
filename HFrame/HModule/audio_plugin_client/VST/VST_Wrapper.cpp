...
#include <core/core.h>
  ...
  HCOMPILER_WARNING ("When version has 'major' = 0, VST2 has trouble displaying 'minor' exceeding 9")
 #endif

 #if (JucePlugin_VersionCode & 0xFF) > 9   // check if Bugfix number exceeeds 9
  HCOMPILER_WARNING ("When version has 'major' = 0, VST2 has trouble displaying 'bugfix' exceeding 9")
 #endif
  ...
  HCOMPILER_WARNING ("When version has 'major' > 100, VST2 has trouble displaying 'minor' exceeding 99")
 #endif

 #if (JucePlugin_VersionCode & 0xFF) > 99  // check if Bugfix number exceeeds 99
  HCOMPILER_WARNING ("When version has 'major' > 100, VST2 has trouble displaying 'bugfix' exceeding 99")
 #endif
...
// field in the Projector. The VST2 SDK can be obtained from the
// vstsdk3610_11_06_2018_build_37 (or older) VST3 SDK or H version 5.3.2. You
...
#define HVSTINTERFACE_H_INCLUDED 1
...
#include "../../audio_processors/format_types/VSTCommon.h"

#if HBIG_ENDIAN
 #define HMULTICHAR_CONSTANT(a, b, c, d) (a | (((uint32) b) << 8) | (((uint32) c) << 16) | (((uint32) d) << 24))
#else
 #define HMULTICHAR_CONSTANT(a, b, c, d) (d | (((uint32) c) << 8) | (((uint32) b) << 16) | (((uint32) a) << 24))
#endif
...
{
 #if HMAC
  extern API void initialiseMacVST();
  extern API void* attachComponentToWindowRefVST (Component*, void* parent, bool isNSView);
  extern API void detachComponentFromWindowRefVST (Component*, void* window, bool isNSView);
  extern API void setNativeHostWindowSizeVST (void* window, Component*, int newWidth, int newHeight, bool isNSView);
  extern API void checkWindowVisibilityVST (void* window, Component*, bool isNSView);
  extern API bool forwardCurrentKeyEventToHostVST (Component*, bool isNSView);
 #if ! H64BIT
  extern API void updateEditorCompBoundsVST (Component*);
 #endif
 #endif

  extern API bool handleManufacturerSpecificVST2Opcode (int32, pointer_sized_int, void*, float);
}
...
struct SharedMessageThread  : public Thread
{
    ...
    HDECLARE_SINGLETON (SharedMessageThread, false)

    bool initialised = false;
};

HIMPLEMENT_SINGLETON (SharedMessageThread)
...
private:
    ...
    struct VstTempBuffers
    {
        ...
        AudioBuffer<FloatType> processTempBuffer;
    };
...
public:
         ...
         processor (af)
    {
        ...
        HAssert (processor->isMidiEffect() || (maxNumInChannels > 0 || maxNumOutChannels > 0));
        ...
        if (auto* HParam = processor->getBypassParameter())
            HParam->addListener (this);

        HParameters.update (*processor, false);
        ...
        vstEffect.numParams = HParameters.getNumParameters();
                                   ...
                                   int32 numSamples, VstTempBuffers<FloatType>& tmpBuffers)
    {
       ...
       #if HDEBUG && ! (JucePlugin_ProducesMidiOutput || JucePlugin_IsMidiEffect)
        ...
        if (! midiEvents.isEmpty())
        {
            ...
            {
                HAssert (midiEventPosition >= 0 && midiEventPosition < numSamples);
            ...
            /*  This assertion is caused when you've added some events to the
                ...
                to the output.
            */
            HAssert (midiEvents.getNumEvents() <= numMidiEventsComingIn);
           #endif
    ...
    {
        HAssert (! processor->isUsingDoublePrecision());
    ...
    bool getCurrentPosition (AudioPlayHead::CurrentPositionInfo& info) override
    {
        ...
        if ((ti->flags & Vst2::kVstSmpteValid) != 0)
        {
            ...
            switch (ti->smpteFrameRate)
            {
                ...
                default:                          HAssertfalse; // unknown frame-rate..
            }
    ...
    {
        if (auto* param = HParameters.getParamForIndex (index))
    ...
    struct EditorCompWrapper  : public Component
    {
        ...
        void updateWindowSize (bool resizeEditor)
        {
            if (! isInSizeWindow)
            {
                if (auto* ed = getEditorComp())
                {
                   ...
                   #if ! HLINUX // setSize() on linux causes renoise and energyxt to fail.
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (EditorCompWrapper)
    };
...
private:
    ...
    {
       #if HVST_RETURN_HEX_VERSION_NUMBER_DIRECTLY
        ...
        // for details, see: https://forum.H.com/t/issues-with-version-integer-reported-by-vst2/23867
    ...
    {
        if (auto* param = HParameters.getParamForIndex (args.index))
        {
    ...
    pointer_sized_int handleManufacturerSpecific (VstOpCodeArguments args)
    {
        ...
        if (args.index == HMULTICHAR_CONSTANT ('P', 'r', 'e', 'S')
             && args.value == HMULTICHAR_CONSTANT ('A', 'e', 'C', 's'))
                                                    ...
                                                    float value)
    {
        ...
        {
            if (auto* param = HParameters.getParamForIndex ((int) paramIndex))
            {
                if (! LegacyAudioParameter::isLegacy (param))
                {
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JuceVSTWrapper)
};
...
// Mac startup code..
    ...
    HEXPORTED_FUNCTION Vst2::AEffect* VSTPluginMain (Vst2::audioMasterCallback audioMaster);
    HEXPORTED_FUNCTION Vst2::AEffect* VSTPluginMain (Vst2::audioMasterCallback audioMaster)
    {
        PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_VST;

        initialiseMacVST();
        return pluginEntryPoint (audioMaster);
    }

    HEXPORTED_FUNCTION Vst2::AEffect* main_macho (Vst2::audioMasterCallback audioMaster);
    HEXPORTED_FUNCTION Vst2::AEffect* main_macho (Vst2::audioMasterCallback audioMaster)
    {
        PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_VST;
...
// Linux startup code..
    ...
    HEXPORTED_FUNCTION Vst2::AEffect* main_plugin (Vst2::audioMasterCallback audioMaster) asm ("main");
    HEXPORTED_FUNCTION Vst2::AEffect* main_plugin (Vst2::audioMasterCallback audioMaster)
    {
        PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_VST;
...
// Win32 startup code..
   ...
   #ifndef H64BIT // (can't compile this on win64, but it's not needed anyway with VST2.4)
