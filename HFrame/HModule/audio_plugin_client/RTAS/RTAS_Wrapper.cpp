...
#include "../utility/IncludeModuleHeaders.h"
 ...
 // This HRTAS_LINK_TO_DEBUG_LIB setting can be used to force linkage
 // against only the release build of the RTAS lib, since in older SDKs there
 // can be problems with the debug build.
 #if HDEBUG && ! defined (HRTAS_LINK_TO_DEBUG_LIB)
  #define HRTAS_LINK_TO_DEBUG_LIB 1
 #endif

 #if HRTAS_LINK_TO_DEBUG_LIB
...
//==============================================================================
#if HWINDOWS
  extern void CALLTYPE attachSubWindow (void* hostWindow, int& titleW, int& titleH, Component* comp);
  extern void CALLTYPE resizeHostWindow (void* hostWindow, int& titleW, int& titleH, Component* comp);
 #if ! JucePlugin_EditorRequiresKeyboardFocus
  extern void CALLTYPE passFocusToHostWindow (void* hostWindow);
 #endif
#else
...
#if ! (HDEBUG || defined (HRTAS_PLUGINGESTALT_IS_CACHEABLE))
 #define HRTAS_PLUGINGESTALT_IS_CACHEABLE 1
#endif

const int midiBufferSize = 1024;
const OSType HChunkType = 'H';
...
public:
    //==============================================================================
    // RTAS builds will be removed from H in the next release
    HDEPRECATED_WITH_BODY (JucePlugInProcess(),
    {
        HFilter.reset (createPluginFilterOfType (AudioProcessor::wrapperType_RTAS));

        AddChunk (HChunkType, "Juce Audio Plugin Data");
    ...
    private:
        ...
        public:
                  ...
                  titleH (0)
            {
               ...
               #if HWINDOWS && ! JucePlugin_EditorRequiresKeyboardFocus
           ...
           #if HMAC && JucePlugin_EditorRequiresKeyboardFocus
    ...
    {
        return new JuceCustomUIView (HFilter.get(), this);
    }
    ...
    ComponentResult GetDelaySamplesLong (long* aNumSamples) override
    {
        if (aNumSamples != nullptr)
            *aNumSamples = HFilter != nullptr ? HFilter->getLatencySamples() : 0;
    ...
    void EffectInit() override
    {
        ...
        HAssert (maxBlockSize > 0);

        SFicPlugInStemFormats stems;
        GetProcessType()->GetStemFormats (&stems);

        HFilter->setPlayConfigDetails (fNumInputs, fNumOutputs, sampleRate, maxBlockSize);

        AddControl (new CPluginControl_OnOff ('bypa', "Master Bypass\nMastrByp\nMByp\nByp", false, true));
        DefineMasterBypassControlIndex (bypassControlIndex);

        const int numParameters = HFilter->getNumParameters();

       #if HFORCE_USE_LEGACY_PARAM_IDS
        const bool usingManagedParameters = false;
       #else
        const bool usingManagedParameters = (HFilter->getParameters().size() == numParameters);
       #endif

        for (int i = 0; i < numParameters; ++i)
        {
            OSType rtasParamID = static_cast<OSType> (usingManagedParameters ? HFilter->getParameterID (i).hashCode() : i);
            AddControl (new JucePluginControl (*HFilter, i, rtasParamID));
        }
        ...
        channels.calloc (jmax (HFilter->getTotalNumInputChannels(),
                               HFilter->getTotalNumOutputChannels()));

        HFilter->setPlayHead (this);
        HFilter->addListener (this);

        HFilter->prepareToPlay (sampleRate, maxBlockSize);
    }
    ...
    void RenderAudio (float** inputs, float** outputs, long numSamples) override
    {
        ...
        if (midiBufferNode != nullptr)
        {
            ...
            {
                HAssert (midiBufferNode->GetBufferPtr() != nullptr);
                ...
                for (int i = 0; i < numMidiEvents; ++i)
                {
                    ...
                    HAssert ((int) m.mTimestamp < numSamples);
...

        {
            ...
            const int numIn  = HFilter->getTotalNumInputChannels();
            const int numOut = HFilter->getTotalNumOutputChannels();
            ...
            else
            {
                ...
                if (mBypassed)
                    HFilter->processBlockBypassed (chans, midiEvents);
                else
                    HFilter->processBlock (chans, midiEvents);
            }
        }
        ...
        if (! midiEvents.isEmpty())
        {
            ...
            {
                //HAssert (midiEventPosition >= 0 && midiEventPosition < (int) numSamples);
            }
           #elif HDEBUG || HLOG_ASSERTIONS
    ...
    ComponentResult GetChunkSize (OSType chunkID, long* size) override
    {
        if (chunkID == HChunkType)
        {
            ...
            HFilter->getStateInformation (tempFilterData);
    ...
    ComponentResult SetChunk (OSType chunkID, SFicPlugInChunk* chunk) override
    {
        if (chunkID == HChunkType)
        {
            ...
            {
                HFilter->setStateInformation ((void*) chunk->fData,
    ...
    ComponentResult UpdateControlValue (long controlIndex, long value) override
    {
        if (controlIndex != bypassControlIndex)
        {
            ...
            if (auto* param = HFilter->getParameters()[paramIndex])
            {
            ...
            {
                HFilter->setParameter (paramIndex, floatValue);
            }
        }
        else
        {
...
private:
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (JucePluginControl)
    };
};
...
public:
    ...
    JucePlugInGroup()
    {
       ...
       #if HRTAS_PLUGINGESTALT_IS_CACHEABLE
    ...
    void CreateEffectTypes()
    {
        ...
        #error You need to set the "Plugin Channel Configurations" field in the Projector to build RTAS plug-ins
       #endif
...
private:
    static CEffectProcess* createNewProcess()
    {
       ...
       #endif
        PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_RTAS;
        initialiseJuce_GUI();
    ...
    static EPlugIn_StemFormat getFormatForChans (const int numChans) noexcept
    {
        switch (numChans)
        {
            ...
            default:  HAssertfalse; break; // hmm - not a valid number of chans for RTAS..
        }
