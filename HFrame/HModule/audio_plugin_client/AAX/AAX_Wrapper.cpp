...
#if JucePlugin_Build_AAX && (HINCLUDED_AAX_IN_MM || defined (_WIN32) || defined (_WIN64))
...
static_assert (AAX_SDK_CURRENT_REVISION >= AAX_SDK_2p3p0_REVISION, "H requires AAX SDK version 2.3.0 or higher");
...
#include <AAX_UIDs.h>
 ...
 #pragma clang diagnostic pop
  ...
  #error "You need to define the JucePlugin_AAXLibs_path macro. (This is best done via the Projector)"
 #endif

 #if H64BIT
  #define HAAX_LIB "AAXLibrary_x64"
 #else
  #define HAAX_LIB "AAXLibrary"
 #endif

 #if HDEBUG
  #define HAAX_LIB_PATH   "\\Debug\\"
  #define HAAX_LIB_SUFFIX "_D"
 #else
  #define HAAX_LIB_PATH   "\\Release\\"
  #define HAAX_LIB_SUFFIX ""
 #endif

 #pragma comment(lib, JucePlugin_AAXLibs_path HAAX_LIB_PATH HAAX_LIB HAAX_LIB_SUFFIX ".lib")
#endif

#undef check

#include "AAX_Modifier_Injector.h"

using namespace H;

#ifndef JucePlugin_AAX_Chunk_Identifier
 #define JucePlugin_AAX_Chunk_Identifier     'H'
#endif

const int32_t HChunkType = JucePlugin_AAX_Chunk_Identifier;
...
namespace AAXClasses
{
    ...
    {
        HAssert (result == AAX_SUCCESS); ignoreUnused (result);
    }

    // maps a channel index of an AAX format to an index of a H format
    ...
    static int HChannelIndexToAax (int HIndex, const AudioChannelSet& channelSet)
    {
        ...
        for (layoutIndex = 0; aaxChannelOrder[layoutIndex].aaxStemFormat != currentLayout; ++layoutIndex)
            if (aaxChannelOrder[layoutIndex].aaxStemFormat == 0) return HIndex;

        auto& channelOrder = aaxChannelOrder[layoutIndex];
        auto channelType = channelSet.getTypeOfChannel (static_cast<int> (HIndex));
    ...
    struct PluginInstanceInfo
    {
        ...
        HDECLARE_NON_COPYABLE (PluginInstanceInfo)
    };

    //==============================================================================
    struct HAlgorithmContext
    {
        float** inputChannels;
    ...
    struct HAlgorithmIDs
    {
        enum
        {
            inputChannels     = AAX_FIELD_INDEX (HAlgorithmContext, inputChannels),
            outputChannels    = AAX_FIELD_INDEX (HAlgorithmContext, outputChannels),
            bufferSize        = AAX_FIELD_INDEX (HAlgorithmContext, bufferSize),
            bypass            = AAX_FIELD_INDEX (HAlgorithmContext, bypass),

           #if JucePlugin_WantsMidiInput || JucePlugin_IsMidiEffect
            midiNodeIn        = AAX_FIELD_INDEX (HAlgorithmContext, midiNodeIn),
           #endif

           #if JucePlugin_ProducesMidiOutput || JucePlugin_IsSynth || JucePlugin_IsMidiEffect
            midiNodeOut       = AAX_FIELD_INDEX (HAlgorithmContext, midiNodeOut),
           #endif

            pluginInstance    = AAX_FIELD_INDEX (HAlgorithmContext, pluginInstance),
            preparedFlag      = AAX_FIELD_INDEX (HAlgorithmContext, isPrepared),

            meterTapBuffers   = AAX_FIELD_INDEX (HAlgorithmContext, meterTapBuffers),

            sideChainBuffers  = AAX_FIELD_INDEX (HAlgorithmContext, sideChainBuffers)
        };
    };

   #if JucePlugin_WantsMidiInput || JucePlugin_IsMidiEffect
    static AAX_IMIDINode* getMidiNodeIn (const HAlgorithmContext& c) noexcept   { return c.midiNodeIn; }
   #else
    static AAX_IMIDINode* getMidiNodeIn (const HAlgorithmContext&) noexcept     { return nullptr; }
   #endif

   #if JucePlugin_ProducesMidiOutput || JucePlugin_IsSynth || JucePlugin_IsMidiEffect
    AAX_IMIDINode* midiNodeOut;
    static AAX_IMIDINode* getMidiNodeOut (const HAlgorithmContext& c) noexcept  { return c.midiNodeOut; }
   #else
    static AAX_IMIDINode* getMidiNodeOut (const HAlgorithmContext&) noexcept    { return nullptr; }
   #endif
    ...
    private:
        ...
        struct ContentWrapperComponent  : public Component
        {
            ...
            HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ContentWrapperComponent)
        };

        std::unique_ptr<ContentWrapperComponent> component;
        ScopedJuceInitialiser_GUI libraryInitialiser;

        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JuceAAX_GUI)
    };

    static void AAX_CALLBACK algorithmProcessCallback (HAlgorithmContext* const instancesBegin[], const void* const instancesEnd);
    ...
    public:
            ...
            : pluginInstance (createPluginFilterOfType (AudioProcessor::wrapperType_AAX))
        {
            ...
            AAX_CEffectParameters::GetNumberOfChunks (&HChunkIndex);
        ...
        {
            PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_AAX;

            if (PluginHostType::HPlugInIsRunningInAudioSuiteFn == nullptr)
            {
                PluginHostType::HPlugInIsRunningInAudioSuiteFn = [] (AudioProcessor& processor)
                {
        ...
        AAX_Result SetChunk (AAX_CTypeID chunkID, const AAX_SPlugInChunk* chunk) override
        {
            ...
            auto numParameters = HParameters.getNumParameters();

            for (int i = 0; i < numParameters; ++i)
                if (auto paramID = getAAXParamIDFromJuceIndex(i))
                    SetParameterNormalizedValue (paramID, HParameters.getParamForIndex (i)->getValue());
        ...
        AAX_Result ResetFieldData (AAX_CFieldIndex fieldIndex, void* data, uint32_t dataSize) const override
        {
            switch (fieldIndex)
            {
                ...
                case HAlgorithmIDs::preparedFlag:
                {
                ...
                case HAlgorithmIDs::meterTapBuffers:
                {
                    // this is a dummy field only when there are no aaxMeters
                    HAssert (aaxMeters.size() == 0);
        ...
        void audioProcessorChanged (AudioProcessor* processor) override
        {
            ...
            for (int i = 0; i < numParameters; ++i)
            {
                ...
                {
                    auto newName = HParameters.getParamForIndex (i)->getName (31);
    ...
    private:
        ...
        friend void AAX_CALLBACK AAXClasses::algorithmProcessCallback (HAlgorithmContext* const instancesBegin[], const void* const instancesEnd);
                      ...
                      const bool bypass, AAX_IMIDINode* midiNodeIn, AAX_IMIDINode* midiNodesOut)
        {
           ...
           #if JucePlugin_WantsMidiInput || JucePlugin_IsMidiEffect
            {
                ...
                for (uint32_t i = 0; i < numMidiEvents; ++i)
                {
                    ...
                    HAssert ((int) m.mTimestamp < bufferSize);
           ...
           #if JucePlugin_ProducesMidiOutput || JucePlugin_IsMidiEffect
            {
                ...
                {
                    HAssert (isPositiveAndBelow (midiEventPosition, bufferSize));
        ...
        bool isBypassPartOfRegularParemeters() const
        {
            ...
            int n = HParameters.getNumParameters();

            if (auto* bypassParam = audioProcessor.getBypassParameter())
                for (int i = 0; i < n; ++i)
                    if (HParameters.getParamForIndex (i) == bypassParam)
                        return true;
        ...
        void addAudioProcessorParameters()
        {
            ...
            HParameters.update (audioProcessor, forceLegacyParamIDs);
            ...
            if (! bypassPartOfRegularParams)
                HParameters.params.add (bypassParameter);

            int parameterIndex = 0;

            for (auto* HParam : HParameters.params)
            {
                auto isBypassParameter = (HParam == bypassParameter);

                auto category = HParam->getCategory();
                ...
                {
                    aaxMeters.add (HParam);
                    continue;
                }

                auto parameter = new AAX_CParameter<float> (aaxParamID,
                                                            AAX_CString (HParam->getName (31).toRawUTF8()),
                                                            HParam->getDefaultValue(),
                                                            AAX_CLinearTaperDelegate<float, 0>(),
                                                            AAX_CNumberDisplayDelegate<float, 3>(),
                                                            HParam->isAutomatable());

                parameter->AddShortenedName (HParam->getName (4).toRawUTF8());

                auto parameterNumSteps = getSafeNumberOfParameterSteps (*HParam);
               ...
               #else
                parameter->SetType (HParam->isDiscrete() ? AAX_eParameterType_Discrete
                                                            : AAX_eParameterType_Continuous);
               #endif

                parameter->SetOrientation (HParam->isOrientationInverted()
                ...
                if (isBypassParameter)
                    mPacketDispatcher.RegisterPacket (aaxParamID, HAlgorithmIDs::bypass);
            }
        }
        ...
        bool getMainBusFormats (AudioChannelSet& inputSet, AudioChannelSet& outputSet)
        {
            ...
            HAssert (audioProcessor.getTotalNumInputChannels()  == 0
                ...
                for (int busIdx = 0; busIdx < numBuses; ++busIdx)
                {
                    ...
                    if (channelFormat != AudioChannelSet::disabled())
                    {
                        ...
                        for (int ch = 0; ch < numChannels; ++ch)
                            layoutMap.add (HChannelIndexToAax (ch, channelFormat) + chOffset);
        ...
        static void algorithmCallback (HAlgorithmContext* const instancesBegin[], const void* const instancesEnd)
        {
        ...
        //==============================================================================
        static AudioProcessor::CurveData::Type aaxCurveTypeToH (AAX_CTypeID type) noexcept
        {
            switch (type)
            {
        ...
        {
            auto curveType = aaxCurveTypeToH (iCurveType);
        ...
        static AudioProcessor::BusesLayout getDefaultLayout (AudioProcessor& p)
        {
            ...
            HAssert (p.checkBusesLayoutSupported (defaultLayout));
            return defaultLayout;
        }
        ...
        HDECLARE_NON_COPYABLE (JuceAAX_Processor)
    };
                                  ...
                                  const int numMeters)
    {
        ...
        check (desc.AddAudioIn  (HAlgorithmIDs::inputChannels));
        check (desc.AddAudioOut (HAlgorithmIDs::outputChannels));

        check (desc.AddAudioBufferLength (HAlgorithmIDs::bufferSize));
        check (desc.AddDataInPort (HAlgorithmIDs::bypass, sizeof (int32_t)));

       #if JucePlugin_WantsMidiInput || JucePlugin_IsMidiEffect
        check (desc.AddMIDINode (HAlgorithmIDs::midiNodeIn, AAX_eMIDINodeType_LocalInput,
                                 JucePlugin_Name, 0xffff));
       #endif

       #if JucePlugin_ProducesMidiOutput || JucePlugin_IsSynth || JucePlugin_IsMidiEffect
        check (desc.AddMIDINode (HAlgorithmIDs::midiNodeOut, AAX_eMIDINodeType_LocalOutput,
                                 JucePlugin_Name " Out", 0xffff));
       #endif

        check (desc.AddPrivateData (HAlgorithmIDs::pluginInstance, sizeof (PluginInstanceInfo)));
        check (desc.AddPrivateData (HAlgorithmIDs::preparedFlag, sizeof (int32_t)));
        ...
        if (numMeters > 0)
        {
            ...
            check (desc.AddMeters (HAlgorithmIDs::meterTapBuffers, meterIDs.getData(), static_cast<uint32_t> (numMeters)));
        }
        else
        {
            ...
            check (desc.AddPrivateData (HAlgorithmIDs::meterTapBuffers, sizeof (uintptr_t)));
        }

        // Create a property map
        AAX_IPropertyMap* const properties = desc.NewPropertyMap();
        HAssert (properties != nullptr);
        ...
        HAssert (! pluginIds.contains (pluginID));
        ...
        {
            check (desc.AddSideChainIn (HAlgorithmIDs::sideChainBuffers));
        ...
        else
        {
            ...
            check (desc.AddPrivateData (HAlgorithmIDs::sideChainBuffers, sizeof (uintptr_t)));
        }
    ...
    static void getPlugInDescription (AAX_IEffectDescriptor& descriptor, const AAX_IFeatureInfo* featureInfo)
    {
       ...
       #if JucePlugin_IsMidiEffect
        // MIDI effect plug-ins do not support any audio channels
        HAssert (numInputBuses == 0 && numOutputBuses == 0);
        ...
        HAssert (pluginIds.size() > 0);
       #endif
    }
}

void AAX_CALLBACK AAXClasses::algorithmProcessCallback (HAlgorithmContext* const instancesBegin[], const void* const instancesEnd)
{
    AAXClasses::JuceAAX_Processor::algorithmCallback (instancesBegin, instancesEnd);
}

//==============================================================================
AAX_Result HCDECL GetEffectDescriptions (AAX_ICollection*);
AAX_Result HCDECL GetEffectDescriptions (AAX_ICollection* collection)
{
    ScopedJuceInitialiser_GUI libraryInitialiser;
    ...
    if (auto* descriptor = collection->NewDescriptor())
    {
        ...
        collection->AddEffect (HSTRINGIFY (JucePlugin_AAXIdentifier), descriptor);
