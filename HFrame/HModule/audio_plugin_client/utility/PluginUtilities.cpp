...
#include "IncludeModuleHeaders.h"

using namespace H;

AudioProcessor::WrapperType PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_Undefined;
std::function<bool(AudioProcessor&)> PluginHostType::HPlugInIsRunningInAudioSuiteFn = nullptr;
 ...
 bool shouldDoubleScaleNativeGLWindow()
 {
...
#if JucePlugin_Build_VST3 && (__APPLE_CPP__ || __APPLE_CC__ || _WIN32 || _WIN64) && HVST3_CAN_REPLACE_VST2
...
void API getUUIDForVST2ID (bool forControllerUID, uint8 uuid[16])
{
    char uidString[33];
    ...
    {
        uint8 c = i < len ? static_cast<uint8> (JucePlugin_Name[i]) : 0;
...
#if JucePlugin_Build_VST
bool API handleManufacturerSpecificVST2Opcode (int32 index, pointer_sized_int value, void* ptr, float)
{
...
#if JucePlugin_Enable_IAA && JucePlugin_Build_Standalone && HIOS && (! HUSE_CUSTOM_PLUGIN_STANDALONE_APP)
 extern bool CALLTYPE isInterAppAudioConnected();
 extern void CALLTYPE switchToHostApplication();
 extern Image CALLTYPE getIAAHostIcon (int);
#endif

AudioProcessor* API CALLTYPE createPluginFilterOfType (AudioProcessor::WrapperType type)
{
    ...
    HAssert (pluginInstance != nullptr && pluginInstance->wrapperType == type);
...
bool PluginHostType::isInterAppAudioConnected() const
{
    ...
    if (getPluginLoadedAs() == AudioProcessor::wrapperType_Standalone)
        return isInterAppAudioConnected();
   #endif
...
bool PluginHostType::isInAAXAudioSuite (AudioProcessor& processor)
{
    ...
    if (PluginHostType::getPluginLoadedAs() == AudioProcessor::wrapperType_AAX
        && HPlugInIsRunningInAudioSuiteFn != nullptr)
    {
        return HPlugInIsRunningInAudioSuiteFn (processor);
    }
   #endif
...
extern Image API getIconFromApplication (const String&, const int);
