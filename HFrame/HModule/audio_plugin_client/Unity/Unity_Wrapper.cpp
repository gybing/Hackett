...
extern createUnityPeerFunctionType createUnityPeerFn;
...
private:
    ...
    struct UnityBitmapImage    : public ImagePixelData
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (UnityBitmapImage)
    };
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (UnityPeer)
};
...
public:
    AudioProcessorUnityWrapper (bool isTemporary)
    {
        ...
        HParameters.update (*pluginInstance, false);
    }
    ...
    void declareParameters (UnityAudioEffectDefinition& definition)
    {
        ...
        {
            numParams = HParameters.params.size();
            ...
            {
                auto* parameter = HParameters.params[i];
    ...
    void setParameter (int index, float value)       { HParameters.getParamForIndex (index)->setValueNotifyingHost (value); }
    float getParameter (int index) const noexcept    { return HParameters.getParamForIndex (index)->getValue(); }

    String getParameterString (int index) const noexcept
    {
        auto* param = HParameters.getParamForIndex (index);
...
private:
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioProcessorUnityWrapper)
};
...
UNITY_INTERFACE_EXPORT int UNITY_INTERFACE_API UnityGetAudioEffectDefinitions (UnityAudioEffectDefinition*** definitionsPtr)
{
    ...
    if (! hasInitialised)
    {
        PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_Unity;
        createUnityPeerFn = createUnityPeer;
...
//==============================================================================
static ModifierKeys unityModifiersToH (UnityEventModifiers mods, bool mouseDown, int mouseButton = -1)
{
    int flags = 0;
...
//==============================================================================
static AudioProcessorUnityWrapper* getWrapperChecked (int id)
{
    auto* wrapper = getWrapperMap()[id];
    HAssert (wrapper != nullptr);
...
{
    getWrapperChecked (id)->getEditorPeer().forwardMouseEvent ({ x, y }, unityModifiersToH (unityMods, true, button));
}
...
{
    getWrapperChecked (id)->getEditorPeer().forwardMouseEvent ({ x, y }, unityModifiersToH (unityMods, false));
}

UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API unityKeyEvent (int id, int code, UnityEventModifiers mods, const char* name)
{
    getWrapperChecked (id)->getEditorPeer().forwardKeyPress (code, name, unityModifiersToH (mods, false));
}
