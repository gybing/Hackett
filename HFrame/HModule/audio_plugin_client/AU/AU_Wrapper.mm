...
#if __LP64__
 #undef HSUPPORT_CARBON
 #define HSUPPORT_CARBON 0
#endif
...
*/
#if ! (defined (BUILD_AU_CARBON_UI) || H64BIT)
...
#include "../../audio_basics/native/mac_CoreAudioLayouts.h"
#include "../../audio_processors/format_types/LegacyAudioParameter.cpp"
#include "../../audio_processors/format_types/AU_Shared.h"
...
static const AudioUnitPropertyID HFilterObjectPropertyID = 0x1a45ffe9;
...
struct AudioProcessorHolder
{
    AudioProcessorHolder (bool initialiseGUI)
    {
        ...
        HFilter.reset (createPluginFilterOfType (AudioProcessor::wrapperType_AudioUnit));

        // audio units do not have a notion of enabled or un-enabled buses
        HFilter->enableAllBuses();
    }

    std::unique_ptr<AudioProcessor> HFilter;
};
...
public:
    JuceAU (AudioUnit component)
        : AudioProcessorHolder(activePlugins.size() + activeUIs.size() == 0),
          MusicDeviceBase (component,
                           (UInt32) AudioUnitHelpers::getBusCount (HFilter.get(), true),
                           (UInt32) AudioUnitHelpers::getBusCount (HFilter.get(), false)),
          mapper (*HFilter)
    {
        ...
        HAssert (numConfigs > 0 && (configs[0][0] > 0 || configs[0][1] > 0));
        HFilter->setPlayConfigDetails (configs[0][0], configs[0][1], 44100.0, 1024);
       ...
       #else
        channelInfo = AudioUnitHelpers::getAUChannelInfo (*HFilter);
       #endif

        AddPropertyListener (kAudioUnitProperty_ContextName, auPropertyListenerDispatcher, this);

        totalInChannels  = HFilter->getTotalNumInputChannels();
        totalOutChannels = HFilter->getTotalNumOutputChannels();

        HFilter->setPlayHead (this);
        HFilter->addListener (this);
    ...
    ~JuceAU()
    {
        ...
        HAssert (activePlugins.contains (this));
    ...
    ComponentResult Initialize() override
    {
        ...
        mapper.alloc();
        pulledSucceeded.calloc (static_cast<size_t> (AudioUnitHelpers::getBusCount (HFilter.get(), true)));
    ...
    void prepareToPlay()
    {
        if (HFilter != nullptr)
        {
            HFilter->setRateAndBufferSizeDetails (getSampleRate(), (int) GetMaxFramesPerSlice());

            audioBuffer.prepare (totalInChannels, totalOutChannels, (int) GetMaxFramesPerSlice() + 32);
            HFilter->prepareToPlay (getSampleRate(), (int) GetMaxFramesPerSlice());
    ...
    bool BusCountWritable (AudioUnitScope scope) override
    {
        ...
        const int busCount = AudioUnitHelpers::getBusCount (HFilter.get(), isInput);
        return (HFilter->canAddBus (isInput) || (busCount > 0 && HFilter->canRemoveBus (isInput)));
       #endif
    }
    ...
    OSStatus SetBusCount (AudioUnitScope scope, UInt32 count) override
    {
        ...
        if (count != (UInt32) AudioUnitHelpers::getBusCount (HFilter.get(), isInput))
        {
           #ifdef JucePlugin_PreferredChannelConfigurations
            ...
            if  ((! HFilter->canAddBus (isInput)) && ((busCount == 0) || (! HFilter->canRemoveBus (isInput))))
                return kAudioUnitErr_PropertyNotWritable;
            ...
            const int requestedNumBus = static_cast<int> (count);
            {
                ...
                for (busNr = (busCount - 1); busNr != (requestedNumBus - 1); busNr += (requestedNumBus > busCount ? 1 : -1))
                {
                    ...
                    {
                        if (! HFilter->addBus (isInput))
                            break;

                        err = syncAudioUnitWithChannelSet (isInput, busNr,
                                                           HFilter->getBus (isInput, busNr + 1)->getDefaultLayout());
                        if (err != noErr)
                            break;
                    }
                    else
                    {
                        if (! HFilter->removeBus (isInput))
                            break;
                    }
                }
            ...
            {
                // restore bus state
                const int newBusCount = AudioUnitHelpers::getBusCount (HFilter.get(), isInput);
                for (int i = newBusCount; i != busCount; i += (busCount > newBusCount ? 1 : -1))
                {
                    if (busCount > newBusCount)
                        HFilter->addBus (isInput);
                    else
                        HFilter->removeBus (isInput);
                }
                                 ...
                                 void* outData) override
    {
        if (inScope == kAudioUnitScope_Global)
        {
            switch (inID)
            {
                case kAudioUnitProperty_ParameterClumpName:
                    ...
                    if (auto* clumpNameInfo = (AudioUnitParameterNameInfo*) outData)
                    {
                        if (HFilter != nullptr)
                        {
                            ...
                            while (group->getParent() != &HFilter->getParameterTree())
                            {
                ...
                case HFilterObjectPropertyID:
                    ((void**) outData)[0] = (void*) static_cast<AudioProcessor*> (HFilter.get());
                    ((void**) outData)[1] = (void*) this;
                    return noErr;

                case kAudioUnitProperty_OfflineRender:
                    *(UInt32*) outData = (HFilter != nullptr && HFilter->isNonRealtime()) ? 1 : 0;
                    return noErr;
                ...
                case kAudioUnitProperty_SupportsMPE:
                    *(UInt32*) outData = (HFilter != nullptr && HFilter->supportsMPE()) ? 1 : 0;
                    return noErr;
                ...
                case kAudioUnitProperty_CocoaUI:
                    {
                        HAUTORELEASEPOOL
                        {
                            ...
                            info->mCocoaAUViewClass[0] = (CFStringRef) [HStringToNS (class_getName (cls.cls)) retain];
        ...
        if (inScope == kAudioUnitScope_Global)
        {
            switch (inID)
            {
                ...
                case kAudioUnitProperty_OfflineRender:
                {
                    ...
                    if (HFilter != nullptr)
                    {
                        auto isCurrentlyRealtime = HFilter->isNonRealtime();

                        if (isCurrentlyRealtime != shouldBeRealtime)
                        {
                            const ScopedLock sl (HFilter->getCallbackLock());

                            HFilter->setNonRealtime (shouldBeRealtime);
                            HFilter->prepareToPlay (getSampleRate(), (int) GetMaxFramesPerSlice());
                        }
                    }
    ...
    ComponentResult SaveState (CFPropertyListRef* outData) override
    {
        ...
        HAssert (CFGetTypeID (*outData) == CFDictionaryGetTypeID());
        ...
        {
            MemoryBlock state;
            HFilter->getCurrentProgramStateInformation (state);

            if (state.getSize() > 0)
            {
                CFDataRef ourState = CFDataCreate (kCFAllocatorDefault, (const UInt8*) state.getData(), (CFIndex) state.getSize());

                CFStringRef key = CFStringCreateWithCString (kCFAllocatorDefault, HSTATE_DICTIONARY_KEY, kCFStringEncodingUTF8);
        ...
        if (HFilter != nullptr)
        {
            ...
            if (valuePresent)
            {
                if (data != 0)
                {
                    ...
                    if (numBytes > 0)
                        HFilter->setCurrentProgramStateInformation (rawBytes, numBytes);
                }
            }
        }
    ...
    bool busIgnoresLayout (bool isInput, int busNr) const
    {
        ...
        return true;
       #else
        if (const AudioProcessor::Bus* bus = HFilter->getBus (isInput, busNr))
        {
    ...
    OSStatus SetAudioChannelLayout (AudioUnitScope scope, AudioUnitElement element, const AudioChannelLayout* inLayout) override
    {
        ...
        if (const AUIOElement* ioElement = GetIOElement (isInput ? kAudioUnitScope_Input : kAudioUnitScope_Output, element))
        {
            ...
            if (! AudioUnitHelpers::isLayoutSupported (*HFilter, isInput, busNr, newChannelNum, configs))
                return kAudioUnitErr_FormatNotSupported;
           #else
            if (! HFilter->getBus (isInput, busNr)->isLayoutSupported (newChannelSet))
                return kAudioUnitErr_FormatNotSupported;
           #endif
    ...
    float getMaximumParameterValue (AudioProcessorParameter* HParam)
    {
       #if HFORCE_LEGACY_PARAMETER_AUTOMATION_TYPE
        ignoreUnused (HParam);
        return 1.0f;
       #else
        return HParam->isDiscrete() ? (float) (HParam->getNumSteps() - 1) : 1.0f;
       #endif
    }
    ...
    {
        if (inScope == kAudioUnitScope_Global && HFilter != nullptr)
        {
            if (auto* param = getParameterForAUParameterID (inParameterID))
            {
                ...
                HAssert (outParameterInfo.defaultValue >= outParameterInfo.minValue
                                              ...
                                              CFArrayRef *outStrings) override
    {
        ...
        if (inScope == kAudioUnitScope_Global && HFilter != nullptr)
        {
            if (auto* param = getParameterForAUParameterID (inParameterID))
            {
                if (param->isDiscrete())
                {
                    auto index = LegacyAudioParameter::getParamIndex (*HFilter, param);
    ...
    Float64 GetTailTime() override                       { return HFilter->getTailLengthSeconds(); }
    double getSampleRate()                               { return AudioUnitHelpers::getBusCount (HFilter.get(), false) > 0 ? GetOutput (0)->GetStreamFormat().mSampleRate : 44100.0; }

    Float64 GetLatency() override
    {
        const double rate = getSampleRate();
        HAssert (rate > 0);
        return rate > 0 ? HFilter->getLatencySamples() / rate : 0;
    }
    ...
    void sendAUEvent (const AudioUnitEventType type, const int HParamIndex)
    {
        auEvent.mEventType = type;
        auEvent.mArgument.mParameter.mParameterID = getAUParameterIDForIndex (HParamIndex);
    ...
    bool ValidFormat (AudioUnitScope scope, AudioUnitElement element, const CAStreamBasicDescription& format) override
    {
        ...
        const int oldNumChannels = HFilter->getChannelCountOfBus (isInput, busNr);

        if (newNumChannels == oldNumChannels)
            return true;

        if (AudioProcessor::Bus* bus = HFilter->getBus (isInput, busNr))
        {
            ...
            ignoreUnused (bus);
            return AudioUnitHelpers::isLayoutSupported (*HFilter, isInput, busNr, newNumChannels, configs);
           #else
    ...
    OSStatus ChangeStreamFormat (AudioUnitScope scope, AudioUnitElement element, const CAStreamBasicDescription& old, const CAStreamBasicDescription& format) override
    {
        ...
        if (! AudioUnitHelpers::isLayoutSupported (*HFilter, isInput, busNr, newNumChannels, configs))
            return kAudioUnitErr_FormatNotSupported;
       #endif

        // predict channel layout
        AudioChannelSet set = (newNumChannels != oldNumChannels) ? HFilter->getBus (isInput, busNr)->supportedLayoutWithChannels (newNumChannels)
                                                                 : HFilter->getChannelLayoutOfBus (isInput, busNr);
                            ...
                            const UInt32 nFrames) override
    {
        ...
        // set buffer pointers to minimize copying
        {
            ...
            {
                int channelIndexInBus = HFilter->getOffsetInBusBufferForAbsoluteChannelIndex (true, chIdx, busIdx);
                ...
                const int n = HFilter->getChannelCountOfBus (true, busIdx);
    ...
    OSStatus HandleMidiEvent (UInt8 nStatus, UInt8 inChannel, UInt8 inData1, UInt8 inData2, UInt32 inStartFrame) override
    {
        ...
        const uint8 data[] = { (uint8) (nStatus | inChannel),
                                     (uint8) inData1,
                                     (uint8) inData2 };
    ...
    ComponentResult GetPresets (CFArrayRef* outData) const override
    {
        if (outData != nullptr)
        {
            const int numPrograms = HFilter->getNumPrograms();
    ...
    OSStatus NewFactoryPresetSet (const AUPreset& inNewFactoryPreset) override
    {
        ...
        chosenPreset.presetName = HFilter->getProgramName (chosenPresetNumber).toCFString();

        HFilter->setCurrentProgram (chosenPresetNumber);
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (EditorCompHolder)
    };
    ...
    {
        JuceUIViewClass()  : ObjCClass<NSView> ("HAUView_")
        {
    ...
    private:
        ...
        static void shutdown (id self)
        {
            ...
            HAssert (activeUIs.contains (self));
            ...
            if (activePlugins.size() + activeUIs.size() == 0)
            {
                ...
                HAssert (Component::getCurrentlyModalComponent() == nullptr);
    ...
    {
        JuceUICreationClass()  : ObjCClass<NSObject> ("HAUCocoaViewClass_")
        {
    ...
    private:
        ...
        static NSView* uiViewForAudioUnit (id, SEL, AudioUnit inAudioUnit, NSSize)
        {
            ...
            if (AudioUnitGetProperty (inAudioUnit, HFilterObjectPropertyID,
...
private:
    ...
    //==============================================================================
   #if HFORCE_USE_LEGACY_PARAM_IDS
    ...
    //==============================================================================
    LegacyAudioParametersWrapper HParameters;
    ...
    void processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiBuffer) noexcept
    {
        ...
        if (HFilter->isSuspended())
        {
            buffer.clear();
        }
        else if (bypassParam == nullptr && isBypassed)
        {
            HFilter->processBlockBypassed (buffer, midiBuffer);
        }
        else
        {
            HFilter->processBlock (buffer, midiBuffer);
        }
    }
    ...
    void pushMidiOutput (UInt32 nFrames) noexcept
    {
        ...
        {
            HAssert (isPositiveAndBelow (midiEventPosition, nFrames));
    ...
    void GetAudioBufferList (bool isInput, int busIdx, AudioBufferList*& bufferList, bool& interleaved, int& numChannels)
    {
        ...
        HAssert (bufferList->mNumberBuffers > 0);
    ...
    OSStatus elementToBusIdx (AudioUnitScope scope, AudioUnitElement element, bool& isInput, int& busIdx) noexcept
    {
        ...
        if (isPositiveAndBelow (busIdx, AudioUnitHelpers::getBusCount (HFilter.get(), isInput))) return noErr;
    ...
    void addParameters()
    {
        parameterGroups = HFilter->getParameterTree().getSubgroups (true);

        HParameters.update (*HFilter, forceUseLegacyParamIDs);
        const int numParams = HParameters.getNumParameters();
        ...
        {
            for (auto* param : HParameters.params)
            {
                ...
                HAssert (! paramMap.contains (static_cast<int32> (auParamID)));
        ...
        for (auto* param : HParameters.params)
            if (param->isDiscrete())
                HAssert (param->getNumSteps() != AudioProcessor::getDefaultNumParameterSteps());
       #endif
        ...
        if ((bypassParam = HFilter->getBypassParameter()) != nullptr)
    ...
    {
        const String& HParamID = LegacyAudioParameter::getParamID (param, forceUseLegacyParamIDs);
        AudioUnitParameterID paramHash = static_cast<AudioUnitParameterID> (HParamID.hashCode());

       #if HUSE_STUDIO_ONE_COMPATIBLE_PARAMETERS
        // studio one doesn't like negative parameters
        paramHash &= ~(1 << (sizeof (AudioUnitParameterID) * 8 - 1));
       #endif

        return forceUseLegacyParamIDs ? static_cast<AudioUnitParameterID> (HParamID.getIntValue())
                                      : paramHash;
    }
    ...
    AudioProcessorParameter* getParameterForAUParameterID (AudioUnitParameterID address) const noexcept
    {
        ...
        return forceUseLegacyParamIDs ? HParameters.getParamForIndex (index)
                                      : paramMap[index];
    }
    ...
    {
        OSStatus err = noErr;
        const int enabledInputs  = AudioUnitHelpers::getBusCount (HFilter.get(), true);
        const int enabledOutputs = AudioUnitHelpers::getBusCount (HFilter.get(), false);
        ...
        for (int i = 0; i < enabledInputs; ++i)
            if ((err = syncAudioUnitWithChannelSet (true,  i, HFilter->getChannelLayoutOfBus (true,  i))) != noErr) return err;

        for (int i = 0; i < enabledOutputs; ++i)
            if ((err = syncAudioUnitWithChannelSet (false, i, HFilter->getChannelLayoutOfBus (false, i))) != noErr) return err;
    ...
    {
        const int numInputBuses  = AudioUnitHelpers::getBusCount (HFilter.get(), true);
        const int numOutputBuses = AudioUnitHelpers::getBusCount (HFilter.get(), false);
        ...
        if (! AudioUnitHelpers::setBusesLayout (HFilter.get(), requestedLayouts))
            return kAudioUnitErr_FormatNotSupported;
    ...
    OSStatus syncAudioUnitWithChannelSet (bool isInput, int busNr, const AudioChannelSet& channelSet)
    {
        ...
        {
            element->SetName ((CFStringRef) HStringToNS (HFilter->getBus (isInput, busNr)->getName()));
    ...
    void refreshCurrentPreset()
    {
        ...
        const int currentProgramNumber = HFilter->getCurrentProgram();
        const String currentProgramName = HFilter->getProgramName (currentProgramNumber);
    ...
    {
        if (AudioProcessor::Bus* bus = HFilter->getBus (isInput, busNum))
        {
            ...
            for (int ch = 0; ch < n; ++ch)
            {
                ...
                if (AudioUnitHelpers::isLayoutSupported (*HFilter, isInput, busNum, ch, configs))
    ...
    void addSupportedLayoutTagsForDirection (bool isInput)
    {
        ...
        layouts.clear();
        auto numBuses = AudioUnitHelpers::getBusCount (HFilter.get(), isInput);
    ...
    void addSupportedLayoutTags()
    {
        ...
        currentInputLayout. resize (AudioUnitHelpers::getBusCount (HFilter.get(), true));
        currentOutputLayout.resize (AudioUnitHelpers::getBusCount (HFilter.get(), false));
    ...
    void auPropertyListener (AudioUnitPropertyID propId, AudioUnitScope scope, AudioUnitElement)
    {
        if (scope == kAudioUnitScope_Global && propId == kAudioUnitProperty_ContextName
             && HFilter != nullptr && mContextName != nullptr)
        {
            AudioProcessor::TrackProperties props;
            props.name = String::fromCFString (mContextName);

            HFilter->updateTrackProperties (props);
        }
    }
        ...
        HAUTORELEASEPOOL
        {
            if (HFilter == nullptr)
            {
                ...
                AudioUnitGetProperty (GetEditAudioUnit(),
                                      HFilterObjectPropertyID,
                                      kAudioUnitScope_Global,
                                      0,
                                      pointers,
                                      &propertySize);

                HFilter = (AudioProcessor*) pointers[0];
            }
            ...
            if (HFilter != nullptr)
            {
                ...
                if (AudioProcessorEditor* editorComp = HFilter->createEditorIfNeeded())
                {
            ...
            {
                HAssertfalse; // can't get a pointer to our effect
            }
        }
        ...
        if (windowComp != nullptr)
        {
            ...
            if (JuceAU::EditorCompHolder* editorCompHolder = dynamic_cast<JuceAU::EditorCompHolder*> (windowComp->getChildComponent(0)))
                if (AudioProcessorEditor* audioProcessEditor = dynamic_cast<AudioProcessorEditor*> (editorCompHolder->getChildComponent(0)))
                    HFilter->editorBeingDeleted (audioProcessEditor);
    ...
    public:
              ...
              recursive (false)
        {
            HAUTORELEASEPOOL
            {
                ...
                // but H plug-ins crash without it
...
//==============================================================================
#define HCOMPONENT_ENTRYX(Class, Name, Suffix) \
    extern "C" __attribute__((visibility("default"))) ComponentResult Name ## Suffix (ComponentParameters* params, Class* obj); \
    extern "C" __attribute__((visibility("default"))) ComponentResult Name ## Suffix (ComponentParameters* params, Class* obj) \
    { \
        PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_AudioUnit; \
...
#define HFACTORY_ENTRYX(Class, Name) \
...
#define HCOMPONENT_ENTRY(Class, Name, Suffix)   HCOMPONENT_ENTRYX(Class, Name, Suffix)
#define HFACTORY_ENTRY(Class, Name)             HFACTORY_ENTRYX(Class, Name)

//==============================================================================
HCOMPONENT_ENTRY (JuceAU, JucePlugin_AUExportPrefix, Entry)

#ifndef AUDIOCOMPONENT_ENTRY
 #define HDISABLE_AU_FACTORY_ENTRY 1
#endif

#if ! HDISABLE_AU_FACTORY_ENTRY  // (You might need to disable this for old Xcode 3 builds)
HFACTORY_ENTRY   (JuceAU, JucePlugin_AUExportPrefix)
#endif

#if BUILD_AU_CARBON_UI
 HCOMPONENT_ENTRY (JuceAUView, JucePlugin_AUExportPrefix, ViewEntry)
#endif

#if ! HDISABLE_AU_FACTORY_ENTRY
