...
#import <AVFoundation/AVFoundation.h>
  ...
  #define HAUV3_MIDI_OUTPUT_SUPPORTED 1
  #define HAUV3_VIEW_CONFIG_SUPPORTED 1
 #endif
#endif
...
#include "../../graphics/native/mac_CoreGraphicsHelpers.h"
...
#define HVIEWCONTROLLER_OBJC_NAME(x) HJOIN_MACRO (x, FactoryAUv3)
...
#define HAUDIOUNIT_OBJC_NAME(x) HJOIN_MACRO (x, AUv3)
...
public:
    ...
    //==============================================================================
   #if HAUV3_VIEW_CONFIG_SUPPORTED
    ...
    struct Class  : public ObjCClass<AUAudioUnit>
    {
        Class() : ObjCClass<AUAudioUnit> ("AUAudioUnit_")
        {
           ...
           #pragma clang diagnostic ignored "-Wundeclared-selector"
            addMethod (@selector (initWithComponentDescription:options:error:HClass:),
           ...
           #if HAUV3_MIDI_OUTPUT_SUPPORTED
        ...
        static id initWithComponentDescription (id _self, SEL, AudioComponentDescription descr, AudioComponentInstantiationOptions options, NSError** error)
        {
            ...
            JuceAudioUnitv3Base* HAU = JuceAudioUnitv3Base::create (self, descr, options, error);

            setThis (self, HAU);
            return self;
        }

        static id initWithComponentDescriptionAndJuceClass (id _self, SEL, AudioComponentDescription descr, AudioComponentInstantiationOptions options, NSError** error, JuceAudioUnitv3Base* HAU)
        {
            AUAudioUnit* self = _self;
...
public:
    ...
    NSDictionary<NSString*, id>* getFullState() override
    {
        ...
        if (state.getSize() > 0)
        {
            ...
            NSString* nsKey = [[NSString alloc] initWithUTF8String: HSTATE_DICTIONARY_KEY];
    ...
    void setFullState (NSDictionary<NSString*, id>* state) override
    {
        ...
        if (obj != nullptr)
        {
            if ([obj isKindOfClass:[NSData class]])
            {
                ...
                const uint8* const rawBytes = reinterpret_cast< const uint8* const> ([data bytes]);
    ...
    //==============================================================================
    NSString* getContextName() const    override              { return HStringToNS (contextName); }
    ...
    bool allocateRenderResourcesAndReturnError (NSError **outError) override
    {
        ...
        for (int dir = 0; dir < 2; ++dir)
        {
            ...
            HAssert ([auBuses count] == static_cast<NSUInteger> (n));
    ...
    NSIndexSet* getSupportedViewConfigurations (NSArray<AUAudioUnitViewConfiguration*>* configs) override
    {
        ...
        if (auto* editor = getAudioProcessor().createEditorIfNeeded())
        {
            ...
            HAssert (editor->supportsHostMIDIControllerPresence (true) || editor->supportsHostMIDIControllerPresence (false));
...
private:
    ...
    std::unique_ptr<AUParameter, NSObjectDeleter> createParameter (AudioProcessorParameter* parameter)
    {
        ...
        else
        {
            if (! forceLegacyParamIDs)
            {
                if (parameter->isDiscrete())
                {
                    ...
                    HAssert (numSteps != AudioProcessor::getDefaultNumParameterSteps());

                    valueStrings.reset ([NSMutableArray new]);

                    for (int i = 0; i < numSteps; ++i)
                        [valueStrings.get() addObject: HStringToNS (parameter->getText ((float) i / maxValue, 0))];
                }
            }
        }
        ...
        HAssert (! paramMap.contains (static_cast<int64> (address)));
        ...
        @try
        {
            ...
            param.reset([[AUParameterTree createParameterWithIdentifier: HStringToNS (getParameterIdentifier())
                                                                   name: HStringToNS (name)
                                                                address: address
                                                                    min: 0.0f
    ...
    std::unique_ptr<AUParameterGroup, NSObjectDeleter> createParameterGroup (AudioProcessorParameterGroup* group)
    {
        ...
        {
            // Create methods in AUParameterTree return unretained objects (!) -> see Apple header AUAudioUnitImplementation.h
            result.reset ([[AUParameterTree createGroupWithIdentifier: HStringToNS (group->getID())
                                                                 name: HStringToNS (group->getName())
    ...
    {
        auto& processor = getAudioProcessor();
        HParameters.update (processor, forceLegacyParamIDs);
    ...
    void setAudioProcessorParameter (AudioProcessorParameter* HParam, float value)
    {
        if (value != HParam->getValue())
        {
            HParam->setValue (value);

            inParameterChangedCallback = true;
            HParam->sendValueChangedMessageToListeners (value);
        }
    }
    ...
    void addPresets()
    {
        ...
        for (int idx = 0; idx < n; ++idx)
        {
            ...
            [preset.get() setName: HStringToNS (name)];
    ...
    {
        auto& processor = getAudioProcessor();
        HAssert (static_cast<int> (frameCount) <= getAudioProcessor().getBlockSize());
        ...
        if (lastTimeStamp.mSampleTime != timestamp->mSampleTime)
        {
            ...
            // send MIDI
           #if JucePlugin_ProducesMidiOutput && HAUV3_MIDI_OUTPUT_SUPPORTED
    ...
    {
        const String& HParamID = LegacyAudioParameter::getParamID (param, forceLegacyParamIDs);

        return static_cast<AUParameterAddress> (forceLegacyParamIDs ? HParamID.getIntValue()
                                                                    : HParamID.hashCode64());
    }

    AudioProcessorParameter* getJuceParameterForAUAddress (AUParameterAddress address) const noexcept
    {
        return HParameters.getParamForIndex (getJuceParameterIndexForAUAddress (address));
    }
...
{
    PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_AudioUnitv3;
        ...
        if (AudioProcessor* p = createPluginFilterOfType (AudioProcessor::wrapperType_AudioUnitv3))
        {
            ...
            if (processor.hasEditor())
            {
                if (AudioProcessorEditor* editor = processor.createEditorIfNeeded())
                {
                    ...
                    HIOS_MAC_VIEW* view = [[[HIOS_MAC_VIEW alloc] initWithFrame: convertToCGRect (editor->getBounds())] autorelease];
                    [myself setView: view];
                   ...
                   #if HIOS
                    if (HIOS_MAC_VIEW* peerView = [[[myself view] subviews] objectAtIndex: 0])
...
//==============================================================================
// necessary glue code
@interface HVIEWCONTROLLER_OBJC_NAME (JucePlugin_AUExportPrefix) : AUViewController<AUAudioUnitFactory>
@end

@implementation HVIEWCONTROLLER_OBJC_NAME (JucePlugin_AUExportPrefix)
{
...
//==============================================================================
#if HIOS
bool CALLTYPE isInterAppAudioConnected() { return false; }
void CALLTYPE switchToHostApplication()  {}
Image CALLTYPE getIAAHostIcon (int)      { return {}; }
#endif
