...
//==============================================================================
 ...
 #undef HVST3HEADERS_INCLUDE_HEADERS_ONLY
 #define HVST3HEADERS_INCLUDE_HEADERS_ONLY 1
#endif

#include "../../audio_processors/format_types/VST3Headers.h"
...
#include "../../audio_processors/format_types/VST3Common.h"

#ifndef HVST3_CAN_REPLACE_VST2
 #define HVST3_CAN_REPLACE_VST2 1
#endif

#if HVST3_CAN_REPLACE_VST2
namespace Vst2
{
...
#ifndef HVST3_EMULATE_MIDI_CC_WITH_PARAMETERS
 #define HVST3_EMULATE_MIDI_CC_WITH_PARAMETERS 1
#endif
...
//==============================================================================
  ...
  extern API void* attachComponentToWindowRefVST (Component*, void* parentWindowOrView, bool isNSView);
  extern API void detachComponentFromWindowRefVST (Component*, void* nsWindow, bool isNSView);
#endif
...
public:
    ...
    HDECLARE_VST3_COM_QUERY_METHODS
    HDECLARE_VST3_COM_REF_METHODS
    ...
    bool isUsingManagedParameters() const noexcept    { return HParameters.isUsingManagedParameters(); }
...
private:
    ...
    void setupParameters()
    {
        ...
        {
            bool isBypassParameter = (HParam == bypassParameter);

            Vst::ParamID vstParamID = forceLegacyParamIDs ? static_cast<Vst::ParamID> (i++)
                                                          : generateVSTParamIDForParam (HParam);
            ...
            paramMap.set (static_cast<int32> (vstParamID), HParam);
        }
    }

    Vst::ParamID generateVSTParamIDForParam (AudioProcessorParameter* param)
    {
        auto HParamID = LegacyAudioParameter::getParamID (param, false);

      #if HFORCE_USE_LEGACY_PARAM_IDS
        return static_cast<Vst::ParamID> (HParamID.getIntValue());
      #else
        auto paramHash = static_cast<Vst::ParamID> (HParamID.hashCode());
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JuceAudioProcessor)
};
...
public:
    ...
    struct Param  : public Vst::Parameter
    {
            ...
            : owner (editController), param (p)
        {
            ...
            HAssert (info.defaultNormalizedValue >= 0 && info.defaultNormalizedValue <= 1.0f);
        ...
        void toString (Vst::ParamValue value, Vst::String128 result) const override
        {
            if (LegacyAudioParameter::isLegacy (&param))
                // remain backward-compatible with old H code
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Param)
    };
    ...
    struct ProgramChangeParameter  : public Vst::Parameter
    {
        ...
        {
            HAssert (owner.getNumPrograms() > 1);
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ProgramChangeParameter)
    };
...
private:
        ...
        if (auto* pluginInstance = getPluginInstance())
        {
            ...
            if (parameters.getParameterCount() <= 0)
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    auto* HParam = audioProcessor->getParamForVSTParamID (vstParamID);
                    auto* parameterGroup = pluginInstance->getParameterTree().getGroupsForParameter (HParam).getLast();
                    auto unitID = JuceAudioProcessor::getUnitID (parameterGroup);

                    parameters.addParameter (new Param (*this, *HParam, vstParamID, unitID,
           ...
           #if HVST3_EMULATE_MIDI_CC_WITH_PARAMETERS
        ...
        tresult PLUGIN_API onSize (ViewRect* newSize) override
        {
            if (newSize != nullptr)
            {
                ...
                if (component != nullptr)
                {
                   ...
                   #if HWINDOWS && HWIN_PER_MONITOR_DPI_AWARE
    ...
    private:
        ...
        //==============================================================================
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JuceVST3Editor)
    };

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JuceVST3EditController)
};
...
public:
        ...
        host (h)
    {
        ...
        HAssert (checkBusFormatsAreNotDiscrete());
    ...
    ~JuceVST3Component()
    {
        if (HVST3EditController != nullptr)
            HVST3EditController->vst3IsPlaying = 0;
    ...
    tresult PLUGIN_API connect (IConnectionPoint* other) override
    {
        if (other != nullptr && HVST3EditController == nullptr)
            HVST3EditController.loadFrom (other);
    ...
    tresult PLUGIN_API disconnect (IConnectionPoint*) override
    {
        ...
        HVST3EditController = nullptr;
        return kResultTrue;
    }

    tresult PLUGIN_API notify (Vst::IMessage* message) override
    {
        if (message != nullptr && HVST3EditController == nullptr)
        {
            Steinberg::int64 value = 0;

            if (message->getAttributes()->getInt ("JuceVST3EditController", value) == kResultTrue)
            {
                HVST3EditController = (JuceVST3EditController*) (pointer_sized_int) value;

                if (HVST3EditController != nullptr)
                    HVST3EditController->setAudioProcessor (comPluginInstance);
                else
                    HAssertfalse;
            }
        }
    ...
    void getStateInformation (MemoryBlock& destData)
    {
        ...
        // With bypass support, H now needs to store private state data.
        // Put this at the end of the plug-in state and add a few null characters
        // so that plug-ins built with older versions of H will hopefully ignore
        // this data. Additionally, we need to add some sort of magic identifier
        // at the very end of the private data so that H has some sort of
        // way to figure out if the data was stored with a newer H version.
    ...
    void setStateInformation (const void* data, int sizeAsInt)
    {
        ...
        // Check if this data was written with a newer H version
        // and if it has the H private data magic code at the end
        auto HPrivDataIdentifierSize = std::strlen (kJucePrivateDataIdentifier);

        if ((size_t) size >= HPrivDataIdentifierSize + sizeof (int64))
        {
            ...
            {
                // found a H private data section
                uint64 privateDataSize;

                std::memcpy (&privateDataSize,
                             buffer + ((size_t) size - HPrivDataIdentifierSize - sizeof (uint64)),
                             sizeof (uint64));

                privateDataSize = ByteOrder::swapIfBigEndian (privateDataSize);
                size -= privateDataSize + HPrivDataIdentifierSize + sizeof (uint64);
    ...
    {
        HAssert ('VstW' == htonl (*(int32*) data));
        HAssert (1 == htonl (*(int32*) (data + 8))); // version should be 1 according to Steinberg's docs

        auto headerLen = (int) htonl (*(int32*) (data + 4)) + 8;
    ...
    bool loadVST2CcnKBlock (const char* data, int size)
    {
        ...
        HAssert ('CcnK' == htonl (bank->chunkMagic));
        HAssert ('FBCh' == htonl (bank->fxMagic));
        HAssert (htonl (bank->version) == 1 || htonl (bank->version) == 2);
        HAssert (JucePlugin_VSTUniqueID == htonl (bank->fxID));
    ...
    bool loadVST3PresetFile (const char* data, int size)
    {
        ...
        for (int i = 0; i < entryCount; ++i)
        {
            ...
            if (memcmp (data + entryOffset, "Comp", 4) == 0)
            {
                ...
                if (chunkOffset + chunkSize > static_cast<uint64> (size))
                {
                    HAssertfalse;
                    return false;
                }
    ...
    {
        HAssert (pluginInstance != nullptr);
        ...
        for (Steinberg::int32 i = 0; i < numParamsChanged; ++i)
        {
            if (auto* paramQueue = paramChanges.getParameterData (i))
            {
                ...
                if (paramQueue->getPoint (numPoints - 1, offsetSamples, value) == kResultTrue)
                {
                   ...
                   #if HVST3_EMULATE_MIDI_CC_WITH_PARAMETERS
                    else if (HVST3EditController->isMidiControllerParamID (vstParamID))
    ...
    void addParameterChangeToMidiBuffer (const Steinberg::int32 offsetSamples, const Vst::ParamID id, const double value)
    {
        ...
        if (HVST3EditController->getMidiControllerForParameter (id, channel, ctrlNumber))
        {
    ...
    tresult PLUGIN_API process (Vst::ProcessData& data) override
    {
        ...
        if (data.processContext != nullptr)
        {
            ...
            if (HVST3EditController != nullptr)
                HVST3EditController->vst3IsPlaying = processContext.state & Vst::ProcessContext::kPlaying;
        }
        else
        {
            zerostruct (processContext);
...
private:
    ...
    void processAudio (Vst::ProcessData& data, Array<FloatType*>& channelList)
    {
...

        {
           ...
           #if HDEBUG && ! JucePlugin_ProducesMidiOutput
            ...
            /*  This assertion is caused when you've added some events to the
                ...
                to the output.
            */
            HAssert (midiBuffer.getNumEvents() <= numMidiEventsComingIn);
           #endif
        }
    }
    ...
    ComSmartPtr<JuceVST3EditController> HVST3EditController;
    ...
    */
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (JuceVST3Component)
};

const char* JuceVST3Component::kJucePrivateDataIdentifier = "HPrivateData";
...
DEF_CLASS_IID (JuceAudioProcessor)
 ...
 {
     TUID uuid;
     extern API void getUUIDForVST2ID (bool, uint8[16]);
...
#undef HEXPORTED_FUNCTION
    ...
    #define HEXPORTED_FUNCTION

#else
    #define HEXPORTED_FUNCTION extern "C" __attribute__ ((visibility ("default")))

    CFBundleRef globalBundleInstance = nullptr;
    uint32 numBundleRefs = 0;
    Array<CFBundleRef> bundleRefs;

    enum { MaxPathLength = 2048 };
    char modulePath[MaxPathLength] = { 0 };
    void* moduleHandle = nullptr;

    HEXPORTED_FUNCTION bool bundleEntry (CFBundleRef ref)
    {
        if (ref != nullptr)
        {
            ++numBundleRefs;
    ...
    HEXPORTED_FUNCTION bool bundleExit()
    {
        if (shutdownModule())
        {
            if (--numBundleRefs == 0)
...
struct JucePluginFactory  : public IPluginFactory3
{
    ...
    tresult PLUGIN_API createInstance (FIDString cid, FIDString sourceIid, void** obj) override
    {
        ...
        *obj = nullptr;
        ...
        {
            HAssertfalse; // The host you're running in has severe implementation issues!
            return kInvalidArgument;
        }
...
private:
    ...
    private:
        HDECLARE_NON_COPYABLE (ClassEntry)
    };
    ...
    HDECLARE_NON_COPYABLE (JucePluginFactory)
};
...
HEXPORTED_FUNCTION IPluginFactory* PLUGIN_API GetPluginFactory()
{
    PluginHostType::HPlugInClientCurrentWrapperType = AudioProcessor::wrapperType_VST3;
