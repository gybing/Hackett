                ...
                if (status == noErr)
                {
                    ...
                    if (hasLayout && caLayout != nullptr)
                    {
                        ...
                        for (int i = 0; i < static_cast<int> (numChannels); ++i)
                        {
                            ...
                            HAssert (isPositiveAndBelow (idx, static_cast<int> (numChannels)));
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CoreAudioReader)
};
...
public:
    CoreAudioLayoutsUnitTest()
        : UnitTest ("Core Audio Layout <-> H channel layout conversion", UnitTestCategories::audio)
    {}
    ...
    void runTest() override
    {
        ...
        {
            // Check that all known tags defined in CoreAudio SDK version 10.12.4 are known to H
            ...
            for (auto tagEntry : knownTags)
            {
                ...
                expect (! labels.isDiscreteLayout(), "Tag \"" + String (tagEntry.name) + "\" is not handled by H");
            }
        }
        ...
        {
            beginTest ("CA speaker list and H layouts are consistent");
            ...
            for (auto tagEntry : knownTags)
                expect (AudioChannelSet::channelSetWithChannels (CoreAudioLayouts::getSpeakerLayoutForCoreAudioTag (tagEntry.tag))
                        ...
                        "Tag \"" + String (tagEntry.name) + "\" is not converted consistently by H");
        }
