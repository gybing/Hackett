...
private:
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AiffAudioFormatReader)
};
...
public:
        ...
        : AudioFormatWriter (out, aiffFormatName, rate, numChans, bits)
    {
        ...
        if (metadataValues.size() > 0)
        {
            ...
            HAssert (metadataValues.getValue ("MetaDataSource", "None") != "WAV");
...
private:
    ...
    void writeHeader()
    {
        ...
        HAssert (output->getPosition() == headerLen);
    }

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AiffAudioFormatWriter)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MemoryMappedAiffReader)
};
