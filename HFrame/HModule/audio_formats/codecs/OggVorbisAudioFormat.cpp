...
#if HMAC && ! defined (__MACOSX__)
 #define __MACOSX__ 1
#endif

namespace OggVorbisNamespace
{
#if HINCLUDE_OGGVORBIS_CODE || ! defined (HINCLUDE_OGGVORBIS_CODE)
 #if HMSVC
  #pragma warning (push)
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OggReader)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OggWriter)
};
