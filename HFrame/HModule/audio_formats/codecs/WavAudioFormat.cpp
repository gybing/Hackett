...
{
    HCONSTEXPR inline int chunkName (const char* name) noexcept         { return (int) ByteOrder::littleEndianInt (name); }
    HCONSTEXPR inline size_t roundUpSize (size_t sz) noexcept           { return (sz + 3) & ~3u; }
        ...
        static MemoryBlock createFrom (const StringPairArray& values)
        {
            ...
            if (numCues > 0)
            {
                ...
                for (int i = 0; i < numCues; ++i)
                {
                   ...
                   #if HDEBUG
                    HAssert (! identifiers.contains (identifier));
...
private:
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WavAudioFormatReader)
};
...
public:
        ...
        : AudioFormatWriter (out, wavFormatName, rate, channelLayoutToUse, bits)
    {
        ...
        if (metadataValues.size() > 0)
        {
            ...
            HAssert (metadataValues.getValue ("MetaDataSource", "None") != "AIFF");
    ...
    bool write (const int** data, int numSamples) override
    {
        ...
        HAssert (data != nullptr && *data != nullptr); // the input must contain at least one channel!
...
private:
    ...
    void writeHeader()
    {
        ...
        if (! isRF64)
        {
           #if ! HWAV_DO_NOT_PAD_HEADER_SIZE
            /* NB: This junk chunk is added for padding, so that the header is a fixed size
               ...
               The HWAV_DO_NOT_PAD_HEADER_SIZE macro allows you to disable this feature in case
    ...
    static int getChannelMaskFromChannelLayout (const AudioChannelSet& channelLayout)
    {
        ...
        for (auto channel : channels)
        {
            ...
            HAssert (wavChannelBit >= 0 && wavChannelBit <= 31);
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WavAudioFormatWriter)
};
...
public:
                      ...
                      int64 startSampleInFile, int numSamples) override
    {
        ...
        {
            HAssertfalse; // you must make sure that the window contains all the samples you're going to attempt to read.
            return false;
        }
    ...
    void readMaxLevels (int64 startSampleInFile, int64 numSamples, Range<float>* results, int numChannelsToRead) override
    {
        ...
        {
            HAssert (numSamples <= 0); // you must make sure that the window contains all the samples you're going to attempt to read.
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MemoryMappedWavReader)
};
    ...
    if (reader != nullptr)
    {
        ...
        if (bwavSize > 0)
        {
            ...
            if (chunk.getSize() <= (size_t) bwavSize)
            {
                ...
                HAssert (wavFile.getSize() == oldSize);
                return true;
            }
        }
    }
...
private:
    ...
    HDECLARE_NON_COPYABLE (WaveAudioFormatTests)
};
