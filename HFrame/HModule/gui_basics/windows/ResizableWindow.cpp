...
ResizableWindow::~ResizableWindow()
{
    ...
    HAssert (resizableCorner == nullptr || getIndexOfChildComponent (resizableCorner.get()) >= 0);
    HAssert (resizableBorder == nullptr || getIndexOfChildComponent (resizableBorder.get()) >= 0);
    ...
    HAssert (getNumChildComponents() == 0);
}
...
{
    HAssert (width > 0 && height > 0); // not a great idea to give it a zero size..
...
void ResizableWindow::resized()
{
    ...
    if (contentComponent != nullptr)
    {
        ...
        HAssert (! contentComponent->isTransformed());
...
void ResizableWindow::childBoundsChanged (Component* child)
{
    if ((child == contentComponent) && (child != nullptr) && resizeToFitContent)
    {
        ...
        HAssert (child->getWidth() > 0);
        HAssert (child->getHeight() > 0);
...
void ResizableWindow::paint (Graphics& g)
{
    ...
    /* If this fails, then you've probably written a subclass with a resized()
       ...
       layout.
    */
    HAssert (hasBeenResized || (getWidth() == 0 && getHeight() == 0));
   #endif
}
