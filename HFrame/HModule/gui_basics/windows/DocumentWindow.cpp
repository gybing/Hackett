...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ButtonListenerProxy)
};
...
DocumentWindow::~DocumentWindow()
{
    ...
    HAssert (menuBar == nullptr || getIndexOfChildComponent (menuBar.get()) >= 0);
    HAssert (titleBarButtons[0] == nullptr || getIndexOfChildComponent (titleBarButtons[0].get()) >= 0);
    HAssert (titleBarButtons[1] == nullptr || getIndexOfChildComponent (titleBarButtons[1].get()) >= 0);
    HAssert (titleBarButtons[2] == nullptr || getIndexOfChildComponent (titleBarButtons[2].get()) >= 0);
...
void DocumentWindow::closeButtonPressed()
{
    /*  If you've got a close button, you have to override this method to get
        ...
        to call HApplicationBase::quit(), and leave the window to be deleted later by your
        HApplicationBase::shutdown() method. (Doing it this way means that your window will
