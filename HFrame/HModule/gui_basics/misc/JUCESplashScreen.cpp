...
#if ! defined (HREPORT_APP_USAGE)
 #define HREPORT_APP_USAGE 1
#endif

#if ! defined (HDISPLAY_SPLASH_SCREEN)
 #define HDISPLAY_SPLASH_SCREEN 1
#endif

#if ! defined (HUSE_DARK_SPLASH_SCREEN)
                                   ...
                                   public DeletedAtShutdown
{
    ...
    HDECLARE_SINGLETON_SINGLETHREADED_MINIMAL (ReportingThreadContainer)
};

HIMPLEMENT_SINGLETON (ReportingThreadContainer)
...
//==============================================================================
HSplashScreen::HSplashScreen (Component& parent)
{
    ignoreUnused (parent);

   #if HREPORT_APP_USAGE
    if (! appUsageReported)
    {
        ...
        if (appUsageReportingLock.isLocked() && ! appUsageReported)
        {
            ...
            data.set ("cd1", SystemStats::getHVersion());
            data.set ("cd2", osName);
...
std::unique_ptr<Drawable> HSplashScreen::getSplashScreenLogo()
{
    const char* svgData = R"HSPLASHSCREEN(
      ...
      <svg width="123px" height="63px" viewBox="0 0 123 63" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        ...
        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" opacity="0.8">
          ...
          <rect stroke="#B144C8" stroke-width="2" x="1.25311203" y="1.25311203" width="120" height="60" rx="10" fill=")HSPLASHSCREEN"
   #if HUSE_DARK_SPLASH_SCREEN
    "#000000"
   #else
    "#FFFFFF"
   #endif
    ...
    HAssert (svgXml != nullptr);
    return Drawable::createFromSVG (*svgXml);
}

void HSplashScreen::paint (Graphics& g)
{
...
void HSplashScreen::timerCallback()
{
   #if HDISPLAY_SPLASH_SCREEN
...
void HSplashScreen::parentSizeChanged()
{
...
void HSplashScreen::parentHierarchyChanged()
{
    toFront (false);
}

bool HSplashScreen::hitTest (int x, int y)
{
...
void HSplashScreen::mouseUp (const MouseEvent&)
{
    URL HWebsite ("https://H.com");
    HWebsite.launchInDefaultBrowser();
}
