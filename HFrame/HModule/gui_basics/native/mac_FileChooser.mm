...
static NSMutableArray* createAllowedTypesArray (const StringArray& filters)
{
    ...
    for (int i = 0; i < filters.size(); ++i)
    {
        ...
        [filterArray addObject: HStringToNS (f)];
    }
...
public:
          ...
          panel (isSave ? [[NSSavePanel alloc] init] : [[NSOpenPanel alloc] init])
    {
        ...
        [panel setTitle: HStringToNS (owner.title)];
        ...
        [panel setNameFieldStringValue: HStringToNS (filename)];
    }
...
private:
    ...
    bool shouldShowFilename (const String& filenameToTest)
    {
        ...
        auto nsFilename = HStringToNS (filenameToTest);
    ...
    {
        DelegateClass()  : ObjCClass <DelegateType> ("HFileChooser_")
        {
