...
#if HDEBUG && ! defined (HDEBUG_XERRORS)
 #define HDEBUG_XERRORS 1
#endif

#define HX11_SUPPORTS_XEMBED 1

#if HX11_SUPPORTS_XEMBED
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (XBitmapImage)
};
...
public:
    ...
    ~LinuxComponentPeer()
    {
       ...
       #if HX11_SUPPORTS_XEMBED
        handleXEmbedEvent (this, nullptr);
       #endif
    ...
    {
       #if HX11_SUPPORTS_XEMBED
        if (Window w = (Window) getCurrentFocusWindow (this))
            return w;
       #endif
    ...
    void handleClientMessageEvent (XClientMessageEvent& clientMsg, XEvent& event)
    {
        if (clientMsg.message_type == atoms->protocols && clientMsg.format == 32)
        {
            ...
            {
                if ((getStyleFlags() & ComponentPeer::windowIgnoresKeyPresses) == 0)
                {
                    XWindowAttributes atts;
...
private:
    ...
    private:
       ...
       #endif
        HDECLARE_NON_COPYABLE (LinuxRepaintManager)
    };
    ...
    void sendExternalDragAndDropPosition (Window targetWindow)
    {
        ...
        msg.data.l[4] = (long) atoms->XdndActionCopy; // this is all H currently supports
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LinuxComponentPeer)
};
...
namespace WindowingHelpers
{
    static void windowMessageReceive (XEvent& event)
    {
        ...
        {
           #if HX11_SUPPORTS_XEMBED
            if (! handleXEmbedEvent (nullptr, &event))
           #endif
            {
...
void Displays::findDisplays (float masterScale)
{
    ...
    if (auto display = xDisplay.display)
    {
        ...
        for (auto& d : displays)
            d.userArea = d.totalArea; // H currently does not support requesting the user area on Linux
...
//==============================================================================
Rectangle<int> LinuxScaledToPhysicalBounds (ComponentPeer* peer, Rectangle<int> bounds)
{
...
void LinuxAddRepaintListener (ComponentPeer* peer, Component* dummy)
{
    if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*> (peer))
        linuxPeer->addOpenGLRepaintListener (dummy);
}

void LinuxRemoveRepaintListener (ComponentPeer* peer, Component* dummy)
{
    if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*> (peer))
        linuxPeer->removeOpenGLRepaintListener (dummy);
}

unsigned long createKeyProxyWindow (ComponentPeer* peer)
{
    if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*> (peer))
...
void deleteKeyProxyWindow (ComponentPeer* peer)
{
    if (auto* linuxPeer = dynamic_cast<LinuxComponentPeer*> (peer))
...
static LinuxComponentPeer* getPeerForDragEvent (Component* sourceComp)
{
    ...
    HAssertfalse;  // This method must be called in response to a component's mouseDown or mouseDrag event!
    return nullptr;
}
