...
private:
    ...
    NSInteger getRawResult() const
    {
        ...
        [alert setMessageText:     HStringToNS (title)];
        [alert setInformativeText: HStringToNS (message)];
    ...
    {
        if (button != nullptr)
            [alert addButtonWithTitle: HStringToNS (TRANS (button))];
    }
};
...
{
    NSDraggingSourceHelper() : ObjCClass<NSObject<NSDraggingSource>> ("HNSDraggingSourceHelper_")
    {
    ...
    static void provideDataForType (id self, SEL, NSPasteboard* sender, NSPasteboardItem*, NSString* type)
    {
        if ([type compare: NSPasteboardTypeString] == NSOrderedSame)
            if (auto* text = getIvar<String*> (self, "text"))
                [sender setData: [HStringToNS (*text) dataUsingEncoding: NSUTF8StringEncoding]
    ...
    if (auto* view = getNSViewForDragEvent (sourceComponent))
    {
        HAUTORELEASEPOOL
        {
            if (auto event = [[view window] currentEvent])
            {
                ...
                {
                    auto* nsFilename = HStringToNS (filename);
    ...
    struct PMAssertion
    {
        PMAssertion()  : assertionID (kIOPMNullAssertionID)
        {
            IOReturn res = IOPMAssertionCreateWithName (kIOPMAssertionTypePreventUserIdleDisplaySleep,
                                                        ...
                                                        CFSTR ("H Playback"),
                                                        &assertionID);
            HAssert (res == kIOReturnSuccess); ignoreUnused (res);
        }
...
struct DisplaySettingsChangeCallback  : private DeletedAtShutdown
{
    ...
    HDECLARE_SINGLETON_SINGLETHREADED_MINIMAL (DisplaySettingsChangeCallback)

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DisplaySettingsChangeCallback)
};

HIMPLEMENT_SINGLETON (DisplaySettingsChangeCallback)
...
static void selectImageForDrawing (const Image& image)
{
    ...
    [NSGraphicsContext setCurrentContext: [NSGraphicsContext graphicsContextWithGraphicsPort: getImageContext (image)
                                                                                     flipped: false]];
}
...
Image createIconForFile (const File& file)
{
    HAUTORELEASEPOOL
    {
        NSImage* image = [[NSWorkspace sharedWorkspace] iconForFile: HStringToNS (file.getFullPathName())];
