...
public:
    ...
    void setTitle (const String& title) override
    {
        ...
        {
            if (! isSharedWindow)
                [window setTitle: HStringToNS (title)];
        }
    }
    ...
    void setRepresentedFile (const File& file) override
    {
        if (! isSharedWindow)
        {
            [window setRepresentedFilename: HStringToNS (file != File()
    ...
    void setIcon (const Image& newIcon) override
    {
        if (! isSharedWindow)
        {
            ...
            if (! windowRepresentsFile)
                [window setRepresentedFilename:HStringToNS (" ")]; // can't just use an empty string for some reason...
    ...
    void drawRect (NSRect r)
    {
       ...
       #if USE_COREGRAPHICS_RENDERING && HCOREGRAPHICS_RENDER_WITH_MULTIPLE_PAINT_CALLS
    ...
    void drawRect (CGContextRef cg, NSRect r, float displayScale)
    {
       ...
       #endif
        {
            ...
            if (! clip.isEmpty())
            {
                ...
                CGImageRef image = createCoreGraphicsImage (temp, colourSpace, false);
    ...
    {
        return areAnyWindowsInLiveResize() || ! HApplication::isStandaloneApp();
    }
    ...
    {
        return (getStyleFlags() & ComponentPeer::windowIgnoresKeyPresses) == 0;
    }
    ...
    bool worksWhenModal() const
    {
        ...
        return ! HApplication::isStandaloneApp();
    }
    ...
    {
        return (isSharedWindow || ! HApplication::isStandaloneApp())
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NSViewComponentPeer)
};
...
{
    JuceNSViewClass()  : ObjCClass<NSView> ("HView_")
    {
    ...
    static NSAttributedString* attributedSubstringFromRange (id self, SEL, NSRange theRange)
    {
        if (auto* owner = getOwner (self))
        {
            if (auto* target = owner->findCurrentTextInputTarget())
            {
                ...
                return [[[NSAttributedString alloc] initWithString: HStringToNS (target->getTextInRange (r))] autorelease];
            }
        }
...
{
    JuceNSWindowClass()  : ObjCClass<NSWindow> ("HWindow_")
    {
        addIvar<NSViewComponentPeer*> ("owner");
...
void Desktop::setKioskComponent (Component* kioskComp, bool shouldBeEnabled, bool allowMenusAndBars)
{
    ...
    HAssert (peer != nullptr); // (this should have been checked by the caller)
