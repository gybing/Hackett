...
extern ::Window messageWindowHandle;
...
namespace ClipboardHelpers
{
    ...
    static String readWindowProperty (::Display* display, Window window, Atom prop)
    {
        ...
        if (display != nullptr)
        {
                                    ...
                                    (unsigned char**) &clipData) == Success)
            {
                ...
                HAssert (bytesLeft == 0 || numItems == 1000000);
            }
    ...
    {
        Atom property_name = XInternAtom (display, "HSEL", false);

        // The selection owner will be asked to set the HSEL property on the
        // messageWindowHandle with the selection content
        XConvertSelection (display, selection, requestedFormat, property_name,
                           messageWindowHandle, CurrentTime);
        ...
        while (--count >= 0)
        {
            ...
            if (XCheckTypedWindowEvent (display, messageWindowHandle, SelectionNotify, &event))
            {
                if (event.xselection.property == property_name)
                {
                    HAssert (event.xselection.requestor == messageWindowHandle);
    ...
    //==============================================================================
    // Called from the event loop in linux_Messaging in response to SelectionRequest events
...
void SystemClipboard::copyTextToClipboard (const String& clipText)
{
    ...
    if (auto display = xDisplay.display)
    {
        ...
        XSetSelectionOwner (display, XA_PRIMARY, messageWindowHandle, CurrentTime);
        XSetSelectionOwner (display, ClipboardHelpers::atom_CLIPBOARD, messageWindowHandle, CurrentTime);
    }
}
...
String SystemClipboard::getTextFromClipboard()
{
    ...
    if (auto display = xDisplay.display)
    {
        ...
        */
        ...
        {
            if (selectionOwner == messageWindowHandle)
            {
