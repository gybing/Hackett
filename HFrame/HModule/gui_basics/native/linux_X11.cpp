...
void XWindowSystem::initialiseXDisplay() noexcept
{
    ...
    messageWindowHandle = XCreateWindow (display, RootWindow (display, screen),
    ...
    LinuxEventLoop::setWindowSystemFd (fd,
         [this](int /*fd*/)
         {
            do
            {
                ...
                if (evt.type == SelectionRequest && evt.xany.window == messageWindowHandle
                     && handleSelectionRequest != nullptr)
                {
                    handleSelectionRequest (evt.xselectionrequest);
                }
                else if (evt.xany.window != messageWindowHandle
...
{
    ScopedXLock xlock (display);
    XDestroyWindow (display, messageWindowHandle);
    messageWindowHandle = 0;
    XSync (display, True);
    LinuxEventLoop::removeWindowSystemFd();
}

HIMPLEMENT_SINGLETON (XWindowSystem)
