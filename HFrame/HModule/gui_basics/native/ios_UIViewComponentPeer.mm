...
namespace Orientations
{
    static Desktop::DisplayOrientation convertToJuce (UIInterfaceOrientation orientation)
    {
        switch (orientation)
        {
            ...
            default:                                        HAssertfalse; // unknown orientation!
        }
...
private:
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (UIViewComponentPeer)
...
{
    JuceUIView* HView = (JuceUIView*) [c view];

    if (HView != nil && HView->owner != nullptr)
        HView->owner->updateTransformAndScreenBounds();
}

static bool isKioskModeView (JuceUIViewController* c)
{
    JuceUIView* HView = (JuceUIView*) [c view];
    HAssert (HView != nil && HView->owner != nullptr);

    return Desktop::getInstance().getKioskModeComponent() == &(HView->owner->getComponent());
}
...
void UIViewComponentPeer::handleTouches (UIEvent* event, const bool isDown, const bool isUp, bool isCancel)
{
    ...
    for (unsigned int i = 0; i < [touches count]; ++i)
    {
        ...
        lastMousePos = pos + getBounds (true).getPosition().toFloat();
...
void UIViewComponentPeer::updateHiddenTextContent (TextInputTarget* target)
{
    ...
    view->hiddenTextView.text = HStringToNS (target->getTextInRange (Range<int> (0, target->getHighlightedRegion().getStart())));
