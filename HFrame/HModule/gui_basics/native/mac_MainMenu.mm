...
struct JuceMainMenuBarHolder : private DeletedAtShutdown
{
    ...
    HDECLARE_SINGLETON_SINGLETHREADED (JuceMainMenuBarHolder, true)
};

HIMPLEMENT_SINGLETON (JuceMainMenuBarHolder)
...
public:
    ...
    {
        NSMenuItem* item = [parent addItemWithTitle: HStringToNS (name)
                                             action: nil
                                      keyEquivalent: nsEmptyString()];
    ...
    void updateTopLevelMenu (NSMenuItem* parentItem, const PopupMenu& menuToCopy, const String& name, int menuId, int topLevelIndex)
    {
        ...
        NSMenu* menu = [[NSMenu alloc] initWithTitle: HStringToNS (name)];
                      ...
                      const int topLevelMenuId, const int topLevelIndex)
    {
        ...
        NSString* text = HStringToNS (i.text);
        ...
        else
        {
            ...
            auto* HItem = new PopupMenu::Item (i);
            HItem->customComponent = nullptr;

            [item setRepresentedObject: [createNSObjectFromJuceClass (HItem) autorelease]];
    ...
    {
        NSMenu* m = [[NSMenu alloc] initWithTitle: HStringToNS (menuName)];
...
private:
    struct RecentFilesMenuItem
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (RecentFilesMenuItem)
    };
    ...
    static unsigned int HModsToNSMods (const ModifierKeys mods)
    {
        unsigned int m = 0;
    ...
    {
        JuceMenuCallbackClass()  : ObjCClass<NSObject> ("HMainMenu_")
        {
    ...
    private:
        static void menuItemInvoked (id self, SEL, NSMenuItem* item)
        {
            ...
            if (auto* HItem = getJuceClassFromNSObject<PopupMenu::Item> ([item representedObject]))
            {
                ...
                owner->invoke (*HItem, static_cast<int> ([item tag]));
            }
        }
...
namespace MainMenuHelpers
{
    ...
    {
        return NSLocalizedString (HStringToNS (TRANS (name)), nil);
    }
    ...
    static void rebuildMainMenu (const PopupMenu* extraItems)
    {
        ...
        HAssert (HApplicationBase::isStandaloneApp());
    ...
    if (getMacMainMenu() != newMenuBarModel)
    {
        HAUTORELEASEPOOL
        {
            if (newMenuBarModel == nullptr)
            {
                ...
                HAssert (JuceMainMenuHandler::instance == nullptr); // should be zeroed in the destructor
                HAssert (extraAppleMenuItems == nullptr); // you can't specify some extra items without also supplying a model
...
NSMenu* createNSMenu (const PopupMenu& menu, const String& name, int topLevelMenuId, int topLevelIndex, bool addDelegate)
{
    ...
    HAssertfalse; // calling this before making sure the OSX main menu stuff was initialised?
    return nil;
}
