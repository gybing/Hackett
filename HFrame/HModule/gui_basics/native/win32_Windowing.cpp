...
#if HMODULE_AVAILABLE_audio_plugin_client
 #include <audio_plugin_client/AAX/AAX_Modifier_Injector.h>
#endif

#if HWIN_PER_MONITOR_DPI_AWARE
 #include <gui_extra/embedding/ScopedDPIAwarenessDisabler.h>
#endif
...
extern void repeatLastProcessPriority();
extern void checkCurrentlyFocusedTopLevelWindow();  // in TopLevelWindow.cpp
extern bool isRunningInWine();
...
public:
    ...
    {
        HAssert (format == Image::RGB || format == Image::ARGB);
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WindowsBitmapImage)
};
...
//==============================================================================
HCOMCLASS (ITipInvocation, "37c994e7-432b-4834-a2f7-dce1f13b834b")  : public IUnknown
{
                            ...
                            private Timer
{
    ...
    HDECLARE_SINGLETON_SINGLETHREADED (OnScreenKeyboard, false)
...
HIMPLEMENT_SINGLETON (OnScreenKeyboard)
...
HCOMCLASS (IUIViewSettingsInterop, "3694dbf9-8f68-44be-8ff5-195c98ede8a6")  : public IInspectable
{
    virtual HRESULT STDMETHODCALLTYPE GetForWindow (HWND, REFIID, void**) = 0;
};

HCOMCLASS (IUIViewSettings, "c63657f6-8850-470d-88f8-455e16ea2c26")  : public IInspectable
{
...
public:
    ...
    ~HWNDComponentPeer()
    {
        ...
        JuceWindowIdentifier::setAsHWindow (hwnd, false);
    ...
    void toBehind (ComponentPeer* other) override
    {
        ...
        {
            HAssertfalse; // wrong type of window?
        }
    }
    ...
    {
        if (h != 0 && JuceWindowIdentifier::isHWindow (h))
    ...
    struct FileDropTarget    : public ComBaseClassHelper<IDropTarget>
    {
        ...
        HCOMRESULT DragEnter (IDataObject* pDataObject, DWORD grfKeyState, POINTL mousePos, DWORD* pdwEffect) override
        {
        ...
        HCOMRESULT DragOver (DWORD /*grfKeyState*/, POINTL mousePos, DWORD* pdwEffect) override
        {
            if (peerIsDeleted)
                return S_FALSE;
        ...
        HCOMRESULT Drop (IDataObject* pDataObject, DWORD /*grfKeyState*/, POINTL mousePos, DWORD* pdwEffect) override
        {
            auto hr = updateFileList (pDataObject);
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (FileDropTarget)
    };

    static bool offerKeyMessageToHWindow (MSG& m)
    {
...
private:
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (TemporaryImage)
    };
    ...
    public:
        ...
        HDECLARE_SINGLETON_SINGLETHREADED_MINIMAL (WindowClassHolder)
    ...
    private:
        ...
        {
            if (Component::getNumCurrentlyModalComponents() == 0 || JuceWindowIdentifier::isHWindow (m.hwnd))
                return false;
        ...
        HDECLARE_NON_COPYABLE (WindowClassHolder)
    };
    ...
    void createWindow()
    {
        ...
        if (hasTitleBar())
        {
            ...
            else
            {
                ...
                HAssert ((styleFlags & (windowHasMinimiseButton | windowHasMaximiseButton)) == 0);
            }
        ...
        if (hwnd != 0)
        {
            ...
            JuceWindowIdentifier::setAsHWindow (hwnd, true);
            ...
            if (! HApplication::isStandaloneApp())
...
private:
    ...
    LRESULT peerWindowProc (HWND h, UINT message, WPARAM wParam, LPARAM lParam)
    {
        switch (message)
        {
           ...
           #if HREMOVE_COMPONENT_FROM_DESKTOP_ON_WM_DESTROY
            case WM_DESTROY:
            ...
            case WM_SYSCOMMAND:
                ...
                case SC_KEYMENU:
                   #if ! HWINDOWS_ALT_KEY_TRIGGERS_MENU
                    ...
                    // HWINDOWS_ALT_KEY_TRIGGERS_MENU=1 in your app.
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (IMEHandler)
    };
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (HWNDComponentPeer)
};
...
API ComponentPeer* createNonRepaintingEmbeddedWindowsPeer (Component& component, void* parentHWND)
{
    return new HWNDComponentPeer (component, ComponentPeer::windowIgnoresMouseClicks,
                                  (HWND) parentHWND, true);
}

API bool shouldScaleGLWindow (void* hwnd)
{
    return isPerMonitorDPIAwareWindow ((HWND) hwnd);
}

#if HWIN_PER_MONITOR_DPI_AWARE
 API void setProcessDPIAwarenessIfNecessary (void* hwnd)
 {
     if (getProcessDPIAwareness == nullptr)
         return;
...
HIMPLEMENT_SINGLETON (HWNDComponentPeer::WindowClassHolder)
...
bool offerKeyMessageToHWindow (MSG& m)   { return HWNDComponentPeer::offerKeyMessageToHWindow (m); }

//==============================================================================
bool CALLTYPE Process::isForegroundProcess()
{
...
void CALLTYPE Process::makeForegroundProcess() {}
void CALLTYPE Process::hide() {}
...
private:
    ...
    static UINT getMessageBoxFlags (AlertWindow::AlertIconType iconType) noexcept
    {
        ...
        // this window can get lost behind H windows which are set to be alwaysOnTop
        // so if there are any set it to be topmost
        if (areThereAnyAlwaysOnTopWindows())
            flags |= MB_TOPMOST;
...
Image createIconForFile (const File& file)
{
    Image image;
