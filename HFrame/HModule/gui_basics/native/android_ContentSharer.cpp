...
*/
//==============================================================================
// This byte-code is generated from native/javacore/app/com/roli/H/JuceSharingContentProvider.java with min sdk version 16
...
private:
    ...
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     ...
     METHOD (constructor, "<init>", "(Lcom/roli/H/JuceSharingContentProvider;J[Ljava/lang/String;)V") \
     CALLBACK (contentSharerCursorClosed, "contentSharerCursorClosed", "(J)V") \

    DECLARE_JNI_CLASS (JuceContentProviderCursor, "com/roli/H/JuceSharingContentProvider$ProviderCursor")
    #undef JNI_CLASS_MEMBERS
...
private:
    ...
    //==============================================================================
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     METHOD (constructor,   "<init>",        "(Lcom/roli/H/JuceSharingContentProvider;JLjava/lang/String;I)V") \
    ...
    DECLARE_JNI_CLASS (JuceContentProviderFileObserver, "com/roli/H/JuceSharingContentProvider$ProviderFileObserver")
    #undef JNI_CLASS_MEMBERS
...
public:
    ContentSharerNativeImpl (ContentSharer& cs)
        : owner (cs),
          packageName (HString (LocalRef<jstring> ((jstring) getEnv()->CallObjectMethod (getAppContext().get(), AndroidContext.getPackageName)))),
    ...
    void shareFiles (const Array<URL>& files) override
    {
        ...
        {
            // You need to enable "Content Sharing" in Projector's Android exporter.
            HAssertfalse;
...
private:
    bool isContentSharingEnabled() const
    {
        ...
        for (int i = 0; i < numProviders; ++i)
        {
            ...
            if (HString (authority) == sharingContentProviderAuthority)
                return true;
        }
    ...
    {
        HAssert (prepareFilesThread != nullptr);
    ...
    ContentUriElements getContentUriElements (JNIEnv* env, const LocalRef<jobject>& uri) const
    {
        ...
        auto fullUri = HString ((jstring) env->CallObjectMethod (uri.get(), AndroidUri.toString));
    ...
    DECLARE_JNI_CLASS_WITH_BYTECODE (JuceSharingContentProvider, "com/roli/H/JuceSharingContentProvider", 16, javaJuceSharingContentProvider, sizeof(javaJuceSharingContentProvider))
    #undef JNI_CLASS_MEMBERS
