...
bool performDragDropFiles (const StringArray&, const bool copyFiles, bool& shouldStop);
bool performDragDropText (const String&, bool& shouldStop);
...
private:
    ...
    HDECLARE_NON_COPYABLE (DragImageComponent)
};
                                          ...
                                          const MouseInputSource* inputSourceCausingDrag)
{
    ...
    {
        HAssertfalse;   // You must call startDragging() from within a mouseDown or mouseDrag callback!
        return;
    }
    ...
    else
    {
        ...
        {
            HAssertfalse;   // Your DragAndDropContainer needs to be a Component!
            return;
        }
    }
...
var DragAndDropContainer::getCurrentDragDescription() const
{
    ...
    HAssert (dragImageComponents.size() < 2);
                                                                          ...
                                                                          const MouseInputSource* inputSourceCausingDrag)
{
    ...
    HAssert (inputSourceCausingDrag != nullptr && inputSourceCausingDrag->isDragging());
