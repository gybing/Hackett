...
struct HeaderItemComponent  : public PopupMenu::CustomComponent
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (HeaderItemComponent)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ItemComponent)
};
...
struct MenuWindow  : public Component
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MenuWindow)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MouseSourceState)
};
...
struct NormalComponentWrapper : public PopupMenu::CustomComponent
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NormalComponentWrapper)
};
...
{
    HAssert (commandManager != nullptr && commandID != 0);
...
struct PopupMenuCompletionCallback  : public ModalComponentManager::Callback
{
    ...
    HDECLARE_NON_COPYABLE (PopupMenuCompletionCallback)
};
                                         ...
                                         const bool canBeModal)
{
    ...
    if (auto* window = createWindow (options, &(callback->managerOfChosenCommand)))
    {
        ...
        HAssert (! (userCallback == nullptr && canBeModal));
       #endif
    }
...
bool CALLTYPE PopupMenu::dismissAllActiveMenus()
{
