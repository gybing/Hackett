...
Desktop::~Desktop()
{
    ...
    // be leaking memory!
    HAssert (desktopComponents.size() == 0);
}

Desktop& CALLTYPE Desktop::getInstance()
{
    if (instance == nullptr)
...
{
    HAssert (c != nullptr);
    HAssert (! desktopComponents.contains (c));
...
void Desktop::setKioskModeComponent (Component* componentToUse, bool allowMenusAndBars)
{
    ...
    if (kioskModeComponent != componentToUse)
    {
        ...
        HAssert (kioskModeComponent == nullptr || ComponentPeer::getPeerFor (kioskModeComponent) != nullptr);
        ...
        if (kioskModeComponent != nullptr)
        {
            ...
            HAssert (ComponentPeer::getPeerFor (kioskModeComponent) != nullptr);
...
void Desktop::setOrientationsEnabled (int newOrientations)
{
    if (allowedOrientations != newOrientations)
    {
        ...
        HAssert (newOrientations != 0 && (newOrientations & ~allOrientations) == 0);
...
bool Desktop::isOrientationEnabled (DisplayOrientation orientation) const noexcept
{
    ...
    HAssert (orientation == upright || orientation == upsideDown
