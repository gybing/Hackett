...
private:
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (BailOutChecker2)
    };

    HDECLARE_NON_COPYABLE (MouseListenerList)
};
...
struct FocusRestorer
{
    ...
    HDECLARE_NON_COPYABLE (FocusRestorer)
};
...
Component::~Component()
{
    ...
    HAssert (childComponentList.size() == 0);
}
...
void Component::setName (const String& name)
{
    ...
    HASSERT_MESSAGE_MANAGER_IS_LOCKED_OR_OFFSCREEN
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (StandardCachedComponentImage)
};
...
void Component::setBufferedToImage (bool shouldBeBuffered)
{
    ...
    HAssert (cachedImage == nullptr || dynamic_cast<StandardCachedComponentImage*> (cachedImage.get()) != nullptr);
...
void Component::toBehind (Component* other)
{
    if (other != nullptr && other != this)
    {
        ...
        HAssert (parentComponent == other->parentComponent);
        ...
        {
            HAssert (other->isOnDesktop());

            if (other->isOnDesktop())
            {
                auto* us = getPeer();
                auto* them = other->getPeer();
                HAssert (us != nullptr && them != nullptr);
...
void Component::toBack()
{
    if (isOnDesktop())
    {
        HAssertfalse; //xxx need to add this to native window
    }
    else if (parentComponent != nullptr)
    {
        auto& childList = parentComponent->childComponentList;
...
void Component::setBounds (int x, int y, int w, int h)
{
    ...
    HAssert (! (flags.isInsidePaintCall && wasResized && isOnDesktop()));
   #endif
...
void Component::setTransform (const AffineTransform& newTransform)
{
    ...
    HAssert (! newTransform.isSingularity());
...
void Component::addChildComponent (Component& child, int zOrder)
{
    ...
    HAssert (this != &child); // adding a component to itself!?
...
int CALLTYPE Component::getNumCurrentlyModalComponents() noexcept
{
    return ModalComponentManager::getInstance()->getNumModalComponents();
}

Component* CALLTYPE Component::getCurrentlyModalComponent (int index) noexcept
{
...
void Component::setPositioner (Positioner* newPositioner)
{
    ...
    HAssert (newPositioner == nullptr || this == &(newPositioner->getComponent()));
                                  ...
                                  bool wantsEventsForAllNestedChildComponents)
{
    ...
    HAssert ((newListener != this) || wantsEventsForAllNestedChildComponents);
...
void CALLTYPE Component::beginDragAutoRepeat (int interval)
{
...
Component* CALLTYPE Component::getCurrentlyFocusedComponent() noexcept
{
    return currentlyFocusedComponent;
}

void CALLTYPE Component::unfocusAllComponents()
{
...
bool CALLTYPE Component::isMouseButtonDownAnywhere() noexcept
{
