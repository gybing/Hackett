...
private:
    ...
    HDECLARE_NON_COPYABLE (CommandMessage)
};
...
ApplicationCommandTarget* ApplicationCommandTarget::getTargetForCommand (const CommandID commandID)
{
    ...
    while (target != nullptr)
    {
        ...
        ++depth;
        HAssert (depth < 100); // could be a recursive command chain??
        HAssert (target != this); // definitely a recursive command chain!
    ...
    if (target == nullptr)
    {
        target = HApplication::getInstance();
