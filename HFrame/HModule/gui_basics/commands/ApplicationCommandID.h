...
/** A set of general-purpose application command IDs.
    ...
    H - e.g. the quit ID is recognised as a command by the HApplication class.
...
namespace StandardApplicationCommandIDs
{
    enum
    {
        /** This command ID should be used to send a "Quit the App" command.
            ...
            This command is recognised by the HApplication class, so if it is invoked
            and no other ApplicationCommandTarget handles the event first, the HApplication
            object will catch it and call HApplicationBase::systemRequestedQuit().
        */
        quit           = 0x1001,
