...
private:
    ...
    ConcertinaPanel& getPanel() const
    {
        ...
        HAssert (panel != nullptr);
        return *panel;
    }

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PanelHolder)
};
...
{
    HAssert (component != nullptr); // can't use a null pointer here!
    HAssert (indexOfComp (component) < 0); // You can't add the same component more than once!
...
bool ConcertinaPanel::setPanelSize (Component* panelComponent, int height, const bool animate)
{
    ...
    HAssert (index >= 0); // The specified component doesn't seem to have been added!
