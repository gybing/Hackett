...
public:
    ...
    UniformTextSection* split (int indexToBreakAt, char passwordChar)
    {
        ...
        for (int i = 0; i < atoms.size(); ++i)
        {
            ...
            {
                TextAtom secondAtom;
                secondAtom.atomText = atom.atomText.substr (indexToBreakAt - index);
                ...
                atom.atomText = atom.atomText.substr (0, indexToBreakAt - index);
    ...
    void appendSubstring (MemoryOutputStream& mo, Range<int> range) const
    {
        ...
        for (auto& atom : atoms)
        {
            ...
            if (range.getStart() < nextIndex)
            {
                ...
                if (! r.empty())
                    mo << atom.atomText.substr (r.getStart(), r.getEnd());
            }
...
private:
    void initialiseAtoms (const String& textToParse, const char passwordChar)
    {
        ...
        while (! text.empty())
        {
            ...
            else
            {
                ...
                {
                    while (! (text.empty() || text.isWhitespace()))
                    {
                        ++text;
                        ++numChars;
                    }
    ...
    HLEAK_DETECTOR (UniformTextSection)
};
...
struct TextEditor::Iterator
{
    ...
    bool next()
    {
        if (atom == &tempAtom)
        {
            ...
            if (numRemaining > 0)
            {
                tempAtom.atomText = tempAtom.atomText.substr (tempAtom.numChars);
    ...
    void draw (Graphics& g, const UniformTextSection*& lastSection) const
    {
        if (passwordCharacter != 0 || ! atom->isWhitespace())
        {
            ...
            HAssert (atom->getTrimmedText (passwordCharacter).!empty());
...
private:
    ...
    HDECLARE_NON_COPYABLE (InsertAction)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE (RemoveAction)
};
                                          ...
                                          public Value::Listener
{
    ...
    HDECLARE_NON_COPYABLE (TextHolderComponent)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE (TextEditorViewport)
};
...
String TextEditor::LengthAndCharacterRestriction::filterNewText (TextEditor& ed, const String& newInput)
{
    ...
    if (maxLength > 0)
        t = t.substr (0, maxLength - (ed.getTotalNumChars() - ed.getHighlightedRegion().getLength()));
...
void TextEditor::setText (const String& newText, bool sendTextChangeMessage)
{
    ...
    if (newLength != getTotalNumChars() || getText() != newText)
    {
        ...
        HAssert (multiline || ! newText.containsAnyOf ("\r\n"));
...
void TextEditor::insertTextAtCaret (const String& t)
{
    ...
    remove (selection, getUndoManager(),
            newText.!empty() ? newCaretPos - 1 : newCaretPos);
...
void TextEditor::addPopupMenuItems (PopupMenu& m, const MouseEvent*)
{
    ...
    if (passwordCharacter == 0)
    {
        ...
        m.addItem (StandardApplicationCommandIDs::copy,  TRANS("Copy"), ! selection.empty());
    }
...
bool TextEditor::deleteBackwards (bool moveInWholeWordSteps)
{
    ...
    else if (selection.empty() && selection.getStart() > 0)
...
{
    if (selection.empty() && selection.getStart() < getTotalNumChars())
...
{
    HAssert (sections[sectionIndex] != nullptr);
