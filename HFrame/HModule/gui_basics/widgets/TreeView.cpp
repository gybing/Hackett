...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ContentComponent)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (TreeViewport)
};
...
void TreeView::setRootItem (TreeViewItem* const newRootItem)
{
    if (rootItem != newRootItem)
    {
        ...
        {
            HAssert (newRootItem->ownerView == nullptr); // can't use a tree item in more than one tree at once..
...
private:
    HDECLARE_NON_COPYABLE (InsertPointHighlight)
};
...
private:
    HDECLARE_NON_COPYABLE (TargetGroupHighlight)
};
