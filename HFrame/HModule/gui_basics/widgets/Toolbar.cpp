...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Spacer)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MissingItemsComponent)
};
                               ...
                               const int insertIndex)
{
    ...
    if (auto* tc = createItem (factory, itemId))
    {
        ...
        HAssert (allowedIds.contains (itemId));
       #endif
...
{
    HAssert (missingItemsButton->isShowing());
...
void Toolbar::itemDragMove (const SourceDetails& dragSourceDetails)
{
    if (auto* tc = dynamic_cast<ToolbarItemComponent*> (dragSourceDetails.sourceComponent.get()))
    {
        if (! items.contains (tc))
        {
            ...
            {
                HAssert (tc->getEditingMode() == ToolbarItemComponent::editableOnToolbar);
            }
