...
public:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (RowComponent)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ListViewport)
};
...
struct ListBoxMouseMoveSelector  : public MouseListener
{
    ...
    ListBox& owner;
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ListBoxMouseMoveSelector)
};
...
Component* ListBoxModel::refreshComponentForRow (int, bool, Component* existingComponentToUpdate)
{
    ...
    HAssert (existingComponentToUpdate == nullptr); // indicates a failure in the code that recycles the components
    return nullptr;
}
