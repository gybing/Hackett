...
public:
    ...
    float getPositionOfValue (double value) const
    {
        ...
        HAssertfalse; // not a valid call on a slider that doesn't work linearly!
        return 0.0f;
    }
    ...
    double getValue() const
    {
        ...
        HAssert (style != TwoValueHorizontal && style != TwoValueVertical);
    ...
    void setValue (double newValue, NotificationType notification)
    {
        ...
        {
            HAssert (static_cast<double> (valueMin.getValue()) <= static_cast<double> (valueMax.getValue()));
    ...
    struct DragInProgress
    {
        ...
        HDECLARE_NON_COPYABLE (DragInProgress)
    };
    ...
    void showTextBox()
    {
        HAssert (editableText); // this should probably be avoided in read-only sliders.
    ...
    void paint (Graphics& g, LookAndFeel& lf)
    {
        if (style != IncDecButtons)
        {
            if (isRotary())
            {
                ...
                HAssert (sliderPos >= 0 && sliderPos <= 1.0f);
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PopupDisplayComponent)
    };
...
void Slider::setRotaryParameters (RotaryParameters p) noexcept
{
    ...
    HAssert (p.startAngleRadians >= 0 && p.endAngleRadians >= 0);
    HAssert (p.startAngleRadians < MathConstants<float>::pi * 4.0f
...
{
    HAssert (distanceForFullScaleDrag > 0);
