...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OpenGLFrameBufferImage)
};
...
ImagePixelData::Ptr OpenGLImageType::create (Image::PixelFormat, int width, int height, bool /*shouldClearImage*/) const
{
    ...
    HAssert (currentContext != nullptr); // an OpenGL image can only be created when a valid context is active!
