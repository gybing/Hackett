...
public:
          ...
          hasDepthBuffer (false), hasStencilBuffer (false)
    {
        ...
        if (wantsDepthBuffer || wantsStencilBuffer)
        {
            ...
            HAssert (context.extensions.glIsRenderbuffer (depthOrStencilBuffer));
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SavedState)
};
...
{
    HAssert (context.isActive()); // The context must be active when creating a framebuffer!
...
bool OpenGLFrameBuffer::readPixels (PixelARGB* target, const Rectangle<int>& area)
{
    ...
    glReadPixels (area.getX(), area.getY(), area.getWidth(), area.getHeight(),
                  HRGBA_FORMAT, GL_UNSIGNED_BYTE, target);
