...
extern void (*clearOpenGLGlyphCache)(); // declared in graphics
                          ...
                          private ImagePixelData::Listener
{
    ...
    struct CachedImage
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CachedImage)
    };
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CachedImageList)
};
...
struct Target
{
    ...
    {
        HAssert (frameBufferID != 0); // trying to render into an uninitialised framebuffer object.
    }
...
private:
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (EdgeTableAlphaMap)
    };
};
...
struct ShaderPrograms  : public ReferenceCountedObject
{
            ...
            : program (context)
        {
            ...
            if (vertexShader == nullptr)
                vertexShader = "attribute vec2 position;"
                               ...
                               "varying " HMEDIUMP " vec4 frontColour;"
                               "varying " HHIGHP " vec2 pixelPos;"
                               "void main()"
                               "{"
                                 "frontColour = colour;"
    ...
    //==============================================================================
    #define HDECLARE_VARYING_COLOUR   "varying " HMEDIUMP " vec4 frontColour;"
    #define HDECLARE_VARYING_PIXELPOS "varying " HHIGHP " vec2 pixelPos;"

    struct SolidColourProgram  : public ShaderBase
    {
        SolidColourProgram (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_VARYING_COLOUR
                          "void main() { gl_FragColor = frontColour; }")
        {}
    };

    #define HDECLARE_MASK_UNIFORMS  "uniform sampler2D maskTexture;" \
                                        "uniform ivec4 maskBounds;"
    #define HFRAGCOORD_TO_MASK_POS  "vec2 ((pixelPos.x - float (maskBounds.x)) / float (maskBounds.z)," \
                                              "1.0 - (pixelPos.y - float (maskBounds.y)) / float (maskBounds.w))"
    #define HGET_MASK_ALPHA         "texture2D (maskTexture, " HFRAGCOORD_TO_MASK_POS ").a"

    struct SolidColourMaskedProgram  : public ShaderBase
    {
        SolidColourMaskedProgram (OpenGLContext& context)
            : ShaderBase (context,
                          HDECLARE_MASK_UNIFORMS HDECLARE_VARYING_COLOUR HDECLARE_VARYING_PIXELPOS
                          "void main() {"
                            "gl_FragColor = frontColour * " HGET_MASK_ALPHA ";"
                          "}"),
              maskParams (program)
        {}
    ...
    #define HDECLARE_MATRIX_UNIFORM   "uniform " HHIGHP " float matrix[6];"
    #define HDECLARE_RADIAL_UNIFORMS  "uniform sampler2D gradientTexture;" HDECLARE_MATRIX_UNIFORM
    #define HMATRIX_TIMES_FRAGCOORD   "(mat2 (matrix[0], matrix[3], matrix[1], matrix[4]) * pixelPos" \
                                          " + vec2 (matrix[2], matrix[5]))"
    #define HGET_TEXTURE_COLOUR       "(frontColour.a * texture2D (gradientTexture, vec2 (gradientPos, 0.5)))"

    struct RadialGradientProgram  : public ShaderBase
    {
        RadialGradientProgram (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_VARYING_PIXELPOS
                          HDECLARE_RADIAL_UNIFORMS HDECLARE_VARYING_COLOUR
                          "void main()"
                          "{"
                            HMEDIUMP " float gradientPos = length (" HMATRIX_TIMES_FRAGCOORD ");"
                            "gl_FragColor = " HGET_TEXTURE_COLOUR ";"
                          "}"),
    ...
    struct RadialGradientMaskedProgram  : public ShaderBase
    {
        RadialGradientMaskedProgram (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_VARYING_PIXELPOS
                          HDECLARE_RADIAL_UNIFORMS HDECLARE_VARYING_COLOUR
                          HDECLARE_MASK_UNIFORMS
                          "void main()"
                          "{"
                            HMEDIUMP " float gradientPos = length (" HMATRIX_TIMES_FRAGCOORD ");"
                            "gl_FragColor = " HGET_TEXTURE_COLOUR " * " HGET_MASK_ALPHA ";"
                          "}"),
              gradientParams (program),
              maskParams (program)
        {}
    ...
    #define HDECLARE_LINEAR_UNIFORMS  "uniform sampler2D gradientTexture;" \
                                          "uniform " HMEDIUMP " vec4 gradientInfo;" \
                                          HDECLARE_VARYING_COLOUR HDECLARE_VARYING_PIXELPOS
    #define HCALC_LINEAR_GRAD_POS1    HMEDIUMP " float gradientPos = (pixelPos.y - (gradientInfo.y + (gradientInfo.z * (pixelPos.x - gradientInfo.x)))) / gradientInfo.w;"
    #define HCALC_LINEAR_GRAD_POS2    HMEDIUMP " float gradientPos = (pixelPos.x - (gradientInfo.x + (gradientInfo.z * (pixelPos.y - gradientInfo.y)))) / gradientInfo.w;"

    struct LinearGradient1Program  : public ShaderBase
    {
        LinearGradient1Program (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_LINEAR_UNIFORMS  // gradientInfo: x = x1, y = y1, z = (y2 - y1) / (x2 - x1), w = length
                          "void main()"
                          "{"
                            HCALC_LINEAR_GRAD_POS1
                            "gl_FragColor = " HGET_TEXTURE_COLOUR ";"
                          "}"),
              gradientParams (program)
        {}
    ...
    struct LinearGradient2Program  : public ShaderBase
    {
        LinearGradient2Program (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_LINEAR_UNIFORMS  // gradientInfo: x = x1, y = y1, z = (x2 - x1) / (y2 - y1), y = y1, w = length
                          "void main()"
                          "{"
                            HCALC_LINEAR_GRAD_POS2
                            "gl_FragColor = " HGET_TEXTURE_COLOUR ";"
                          "}"),
              gradientParams (program)
        {}
    ...
    #define HDECLARE_IMAGE_UNIFORMS "uniform sampler2D imageTexture;" \
                                        "uniform " HMEDIUMP " vec2 imageLimits;" \
                                        HDECLARE_MATRIX_UNIFORM HDECLARE_VARYING_COLOUR HDECLARE_VARYING_PIXELPOS
    #define HGET_IMAGE_PIXEL        "texture2D (imageTexture, vec2 (texturePos.x, 1.0 - texturePos.y))"
    #define HCLAMP_TEXTURE_COORD    HHIGHP " vec2 texturePos = clamp (" HMATRIX_TIMES_FRAGCOORD ", vec2 (0, 0), imageLimits);"
    #define HMOD_TEXTURE_COORD      HHIGHP " vec2 texturePos = mod (" HMATRIX_TIMES_FRAGCOORD ", imageLimits);"
    ...
    struct ImageProgram  : public ShaderBase
    {
        ImageProgram (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_VARYING_COLOUR
                          ...
                          "varying " HHIGHP " vec2 texturePos;"
                          "void main()"
                          "{"
                            "gl_FragColor = frontColour.a * " HGET_IMAGE_PIXEL ";"
                          "}",
                          "uniform " HMEDIUMP " vec2 imageLimits;"
                          HDECLARE_MATRIX_UNIFORM
                          ...
                          "{"
                            ...
                            "texturePos = clamp (" HMATRIX_TIMES_FRAGCOORD ", vec2 (0, 0), imageLimits);"
                            "vec2 scaledPos = adjustedPos / screenBounds.zw;"
                            "gl_Position = vec4 (scaledPos.x - 1.0, 1.0 - scaledPos.y, 0, 1.0);"
                          "}"),
              imageParams (program)
    ...
    struct ImageMaskedProgram  : public ShaderBase
    {
        ImageMaskedProgram (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_IMAGE_UNIFORMS HDECLARE_MASK_UNIFORMS
                          "void main()"
                          "{"
                            HCLAMP_TEXTURE_COORD
                            "gl_FragColor = frontColour.a * " HGET_IMAGE_PIXEL " * " HGET_MASK_ALPHA ";"
                          "}"),
              imageParams (program),
              maskParams (program)
        {}
    ...
    struct TiledImageProgram  : public ShaderBase
    {
        TiledImageProgram (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_IMAGE_UNIFORMS
                          "void main()"
                          "{"
                            HMOD_TEXTURE_COORD
                            "gl_FragColor = frontColour.a * " HGET_IMAGE_PIXEL ";"
    ...
    struct MaskTextureProgram  : public ShaderBase
    {
        MaskTextureProgram (OpenGLContext& context)
            : ShaderBase (context, HDECLARE_IMAGE_UNIFORMS
                          "void main()"
                          "{"
                            HHIGHP " vec2 texturePos = " HMATRIX_TIMES_FRAGCOORD ";"
                            HHIGHP " float roundingError = 0.00001;"
                            "if (texturePos.x >= -roundingError"
                             ...
                             "gl_FragColor = frontColour * " HGET_IMAGE_PIXEL ".a;"
                            "else "
...
struct StateHelpers
{
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (EdgeTableRenderer)
    };
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (FloatRectangleRenderer)
    };
    ...
    struct ActiveTextures
    {
        ...
        {
            HAssert (currentActiveTexture >= 0);
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (ShaderQuadQueue)
    };
...
struct GLState
{
          ...
          previousFrameBufferTarget (OpenGLFrameBuffer::getCurrentFrameBufferTarget())
    {
        ...
        HAssert (OpenGLHelpers::isContextActive());
...
struct SavedState  : public RenderingHelpers::SavedStateBase<SavedState>
{
    ...
    void endTransparencyLayer (SavedState& finishedLayerState)
    {
        if (clip != nullptr)
        {
            HAssert (finishedLayerState.previousTarget != nullptr);
...
struct ShaderContext   : public RenderingHelpers::StackBasedLowLevelGraphicsContext<SavedState>
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ShaderContext)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NonShaderContext)
};
                        ...
                        public OpenGLRendering::ShaderPrograms::ShaderBase
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CustomProgram)
};
