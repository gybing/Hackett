...
#if HIOS
struct AppInactivityCallback // NB: this is a duplicate of an internal declaration in core
{
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (iOSBackgroundProcessCheck)
};
...
public:
    ...
    void drawComponentBuffer()
    {
        ...
        HAssert (context.extensions.glActiveTexture != nullptr);
    ...
    void paintOwner (LowLevelGraphicsContext& llgc)
    {
      ...
      #if HENABLE_REPAINT_DEBUGGING
       #ifdef HIS_REPAINT_DEBUGGING_ACTIVE
        if (HIS_REPAINT_DEBUGGING_ACTIVE)
       #endif
        {
            g.saveState();
        }
    ...
    void execute (OpenGLContext::AsyncWorker::Ptr workerToUse, bool shouldBlock, bool calledFromDestructor = false)
    {
        ...
        {
            HAssertfalse; // you called execute AFTER you detached your openglcontext
        }
    }
...
{
    HAssert (name != nullptr);

    auto* c = getCachedImage();

    // This method must only be called from an openGL rendering callback.
    HAssert (c != nullptr && nativeContext != nullptr);
    HAssert (getCurrentContext() != nullptr);
...
void OpenGLContext::execute (OpenGLContext::AsyncWorker::Ptr workerToUse, bool shouldBlock)
{
    if (auto* c = getCachedImage())
        ...
        HAssertfalse; // You must have attached the context to a component
}
                                 ...
                                 bool flippedVertically)
{
    ...
    if (areShadersAvailable())
    {
        struct OverlayShaderProgram  : public ReferenceCountedObject
        {
            ...
            {
                static const char programValueID[] = "HGLComponentOverlayShader";
            ...
            struct ProgramBuilder
            {
                ProgramBuilder (OpenGLShaderProgram& prog)
                {
                    prog.addVertexShader (OpenGLHelpers::translateVertexShaderToV3 (
                        "attribute " HHIGHP " vec2 position;"
                        "uniform " HHIGHP " vec2 screenSize;"
                        "uniform " HHIGHP " float textureBounds[4];"
                        "uniform " HHIGHP " vec2 vOffsetAndScale;"
                        "varying " HHIGHP " vec2 texturePos;"
                        "void main()"
                        "{"
                          HHIGHP " vec2 scaled = position / (0.5 * screenSize.xy);"
    ...
    {
        HAssert (attachment == nullptr); // Running on an old graphics card!
    }
