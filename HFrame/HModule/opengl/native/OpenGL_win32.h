...
#else
 bool shouldDoubleScaleNativeGLWindow()  { return false; }
#endif
...
public:
    ...
    {
        HAssert (isActive()); // this can only be called when the context is active..
...
private:
    ...
    #define HDECLARE_WGL_EXTENSION_FUNCTION(name, returnType, params) \
        typedef returnType (__stdcall *type_ ## name) params; type_ ## name name;

    HDECLARE_WGL_EXTENSION_FUNCTION (wglChoosePixelFormatARB,  BOOL, (HDC, const int*, const FLOAT*, UINT, int*, UINT*))
    HDECLARE_WGL_EXTENSION_FUNCTION (wglSwapIntervalEXT,       BOOL, (int))
    HDECLARE_WGL_EXTENSION_FUNCTION (wglGetSwapIntervalEXT,    int, ())
    #undef HDECLARE_WGL_EXTENSION_FUNCTION
    ...
    void timerCallback() override
    {
        if (safeComponent != nullptr)
        {
            if (auto* peer = safeComponent->getTopLevelComponent()->getPeer())
            {
                ...
                if (shouldDoubleScaleNativeGLWindow())
                    newScale *= newScale;
    ...
    {
        #define HINIT_WGL_FUNCTION(name)    name = (type_ ## name) OpenGLHelpers::getExtensionFunction (#name);
        HINIT_WGL_FUNCTION (wglChoosePixelFormatARB);
        HINIT_WGL_FUNCTION (wglSwapIntervalEXT);
        HINIT_WGL_FUNCTION (wglGetSwapIntervalEXT);
        #undef HINIT_WGL_FUNCTION
    }
    ...
    int wglChoosePixelFormatExtension (const OpenGLPixelFormat& pixelFormat) const
    {
        ...
        if (wglChoosePixelFormatARB != nullptr)
        {
            ...
            atts[n++] = 0;
            HAssert (n <= numElementsInArray (atts));
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (NativeContext)
};
