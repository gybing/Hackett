...
private:
    ...
    {
        AppDelegateClass()  : ObjCClass<NSObject> ("HAppDelegate_")
        {
    ...
    if (quitMessagePosted.get() == 0) // check that the quit message wasn't already posted..
    {
        HAUTORELEASEPOOL
        {
            ...
            @catch (NSException* e)
            {
                ...
                HApplicationBase::sendUnhandledException (&ex, __FILE__, __LINE__);
            }
            @finally
            {
            }
...
{
    HAssert (millisecondsToRunFor >= 0);
...
{
    NSDictionary* info = [NSDictionary dictionaryWithObject: HStringToNS (message)
...
private:
    ...
    {
        ObserverClass()  : ObjCClass<NSObject> ("HDriveObserver_")
        {
            addIvar<Pimpl*> ("owner");
