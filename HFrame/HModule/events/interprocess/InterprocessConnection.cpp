...
{
    ConnectionThread (InterprocessConnection& c)  : Thread ("H IPC"), owner (c) {}
    void run() override     { owner.runThread(); }

    InterprocessConnection& owner;
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ConnectionThread)
};
...
{
    HAssert (socket == nullptr && pipe == nullptr);
...
struct ConnectionStateMessage  : public MessageManager::MessageBase
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ConnectionStateMessage)
};
...
{
    HAssert (callbackConnectionState);
