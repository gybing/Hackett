...
#include "VST3Headers.h"
#include "VST3Common.h"

using namespace Steinberg;

//==============================================================================
#ifndef HVST3_DEBUGGING
 #define HVST3_DEBUGGING 0
#endif
...
{
    HAssert (sampleRate > 0.0); //Must always be valid, as stated by the VST3 SDK
    ...
    if (playHead != nullptr)
    {
        ...
        switch (position.frameRate)
        {
            ...
            default:                         HAssertfalse; break; // New frame rate?
        }
                          ...
                          public Vst::IUnitHandler
{
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ContextMenu)
    };
...
private:
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Message)
    };
    ...
    struct AttributeList  : public Vst::IAttributeList
    {
        ...
        {
            HAssert (size >= 0 && (data != nullptr || size == 0));
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AttributeList)
    };

    ComSmartPtr<AttributeList> attributeList;

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VST3HostContext)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DescriptionFactory)
};
...
private:
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DescriptionLister)
};
...
struct DLLHandle
{
    ...
    /** @note The factory should begin with a refCount of 1,
        ...
        @see releaseFactory
    */
    IPluginFactory* CALLTYPE getPluginFactory()
    {
        if (factory == nullptr)
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DLLHandle)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VST3ModuleHandle)
};
...
private:
    ...
    struct ChildComponent  : public Component
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ChildComponent)
    };
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VST3PluginWindow)
};
...
struct VST3ComponentHolder
{
    ...
    {
        HAssert (module != nullptr && isComponentInitialised);
...
public:
    ...
    {
        HAssert (getActiveEditor() == nullptr); // You must delete any editors before deleting the plugin instance!
    ...
    void prepareToPlay (double newSampleRate, int estimatedSamplesPerBlock) override
    {
        ...
        HAssert (actualInArr == inputArrangements && actualOutArr == outputArrangements);
    ...
    struct TrackPropertiesAttributeList    : public Vst::IAttributeList
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (TrackPropertiesAttributeList)
    };
    ...
    struct ParamValueQueueList  : public Vst::IParameterChanges
    {
        ...
        private:
            ...
            HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ParamValueQueue)
        };
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ParamValueQueueList)
    };
...
private:
    ...
    */
    ...
    static ComSmartPtr<Steinberg::MemoryStream> createMemoryStreamForState (XmlElement& head, const String& identifier)
    {
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VST3PluginInstance)
};
