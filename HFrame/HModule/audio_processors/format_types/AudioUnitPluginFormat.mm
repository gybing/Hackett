...
#include "../../audio_devices/native/MidiDataConcatenator.h"
#include "AU_Shared.h"
...
#if AU_LOGGING
 #define HAU_LOG(a) Logger::writeToLog(a);
#else
 #define HAU_LOG(a)
#endif
...
public:
    struct AUInstanceParameter final  : public Parameter
    {
        ...
        {
           #if HMAC
            HAssert (pluginInstance.audioUnit != nullptr);
    ...
    ~AudioUnitPluginInstance() override
    {
        ...
        HAssert (AudioUnitFormatHelpers::insideCallback.get() == 0);
       #endif
    ...
    static AudioProcessor::BusesProperties getBusesProperties (AudioComponentInstance comp)
    {
        ...
        for (int dir = 0; dir < 2; ++dir)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                HAssert (! currentLayout.isDisabled());
...
private:
        ...
        void setValue (float newValue) override
        {
            ...
            if (newBypassValue != currentValue)
            {
                ...
                if (parent.auSupportsBypass)
                {
                   ...
                   #if HMAC
                    HAssert (parent.audioUnit != nullptr);
    ...
    void eventCallback (const AudioUnitEvent& event, AudioUnitParameterValue newValue)
    {
        ...
        switch (event.mEventType)
        {
            case kAudioUnitEvent_ParameterValueChange:
                ...
                {
                    HAssert (dynamic_cast<AUInstanceParameter*> (param) != nullptr);
                             ...
                             AudioBufferList* ioData) const
    {
        if (currentBuffer != nullptr)
        {
            ...
            HAssert (inNumberFrames == (UInt32) currentBuffer->getNumSamples());
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioUnitPluginInstance)
};
...
public:
   ...
   #if HSUPPORTS_AUv3
    void embedViewController (HIOS_MAC_VIEW* pluginView, const CGSize& size)
    {
    ...
    void requestViewControllerCallback (AUViewControllerBase* controller)
    {
        ...
        if (! MessageManager::getInstance()->isThisTheMessageThread())
        {
            struct AsyncViewControllerCallback : public CallbackMessage
            {
                ...
                HIOS_MAC_VIEW* controllerView;
                CGSize size;

                AsyncViewControllerCallback (AudioUnitPluginWindowCocoa* plugInWindow, HIOS_MAC_VIEW* inView,
...
public:
    ...
    void closeViewComponent()
    {
        ...
        {
            HAU_LOG ("Closing AU GUI: " + plugin.getName());
...
private:
    ...
    public:
        ...
        {
            HAU_LOG ("Opening AU GUI: " + owner.plugin.getName());
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (InnerWrapperComponent)
    };

    friend class InnerWrapperComponent;
    std::unique_ptr<InnerWrapperComponent> innerWrapper;

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioUnitPluginWindowCarbon)
};
