...
#ifdef HVSTINTERFACE_H_INCLUDED
    ...
    static void addEventsToMidiBuffer (const Vst2::VstEvents* events, MidiBuffer& dest)
    {
        for (int i = 0; i < events->numEvents; ++i)
        {
            ...
            if (e != nullptr)
            {
                ...
                {
                    dest.addEvent ((const uint8*) ((const Vst2::VstMidiEvent*) e)->midiData,
                                   4, e->deltaFrames);
                }
                else if (e->type == Vst2::kVstSysExType)
                {
                    dest.addEvent ((const uint8*) ((const Vst2::VstMidiSysexEvent*) e)->sysexDump,
...
#endif // HVSTINTERFACE_H_INCLUDED
