...
#define HLADSPA_LOGGING 1

#if HLADSPA_LOGGING
 #define HLADSPA_LOG(x) Logger::writeToLog (x);
#else
 #define HLADSPA_LOG(x)
#endif
...
public:
    ...
    static LADSPAModuleHandle* findOrCreateModule (const File& file)
    {
        ...
        HLADSPA_LOG ("Loading LADSPA module: " + file.getFullPathName());
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LADSPAModuleHandle)
};
...
public:
        ...
        : module (m)
    {
        ...
        HLADSPA_LOG ("Creating LADSPA instance: " + name);
        ...
        if (module->moduleMain != nullptr)
        {
            ...
            if (plugin == nullptr)
            {
                HLADSPA_LOG ("Cannot find any valid descriptor in shared library");
                --insideLADSPACallback;
                return;
            }
        }
        ...
        {
            HLADSPA_LOG ("Cannot find any valid plugin in shared library");
            --insideLADSPACallback;
            return;
        }
    ...
    ~LADSPAPluginInstance()
    {
        ...
        HAssert (insideLADSPACallback == 0);
    ...
    void initialise (double initialSampleRate, int initialBlockSize)
    {
        ...
        HLADSPA_LOG ("Initialising LADSPA: " + name);
    ...
    void processBlock (AudioBuffer<float>& buffer, MidiBuffer&)
    {
        ...
        if (initialised && plugin != nullptr && handle != nullptr)
        {
            ...
            HAssertfalse; // no callback to use?
        }
...
private:
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LADSPAPluginInstance)
};
