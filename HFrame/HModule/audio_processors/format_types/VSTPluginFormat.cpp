...
// paths or use the "VST (Legacy) SDK Folder" field in the Projector. The VST2
// SDK can be obtained from the vstsdk3610_11_06_2018_build_37 (or older) VST3
// SDK or H version 5.3.2.
...
#ifndef HVST_WRAPPER_LOAD_CUSTOM_MAIN
 #define HVST_WRAPPER_LOAD_CUSTOM_MAIN
#endif

#ifndef HVST_WRAPPER_INVOKE_MAIN
#define HVST_WRAPPER_INVOKE_MAIN  effect = module->moduleMain ((Vst2::audioMasterCallback) &audioMaster);
#endif
...
namespace
{
    ...
    struct IdleCallRecursionPreventer
    {
        ...
        HDECLARE_NON_COPYABLE (IdleCallRecursionPreventer)
    };
...
#if VST_LOGGING
 #define HVST_LOG(a) Logger::writeToLog(a);
#else
 #define HVST_LOG(a)
#endif
...
{
public:
    static VSTXMLInfo* createFor (const XmlElement& xml)
    {
    ...
    {
        String name;
        OwnedArray<Base> paramTree;
    };

    struct Range
    {
        Range() noexcept {}
        Range (const String& s)       { set (s); }
    ...
    const ValueType* getValueType (const String& name) const
    {
    ...
    OwnedArray<Base> paramTree;
    OwnedArray<ValueType> valueTypes;
    OwnedArray<Template> templates;

    ValueType switchValueType;

private:
    VSTXMLInfo (const XmlElement& xml)
    {
    ...
    void parseParam (const XmlElement& item, Group* group, Template* temp)
    {
    ...
    void parseValueType (const XmlElement& item)
    {
    ...
    void parseTemplate (const XmlElement& item)
    {
    ...
    void parseGroup (const XmlElement& item, Group* parentGroup)
    {
    ...
    int evaluate (String expr, const StringArray& variables) const
    {
        StringArray names;
        Array<int> vals;
...
struct ModuleHandle    : public ReferenceCountedObject
{
    ...
    static Ptr findOrCreateModule (const File& file)
    {
        ...
        HVST_LOG ("Attempting to load VST: " + file.getFullPathName());
        ...
        : file (f), moduleMain (customMainCall)
    {
       ...
       #if HWINDOWS || HLINUX || HIOS || HANDROID
...
private:
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ModuleHandle)
};
                               ...
                               private AsyncUpdater
{
        ...
        for (int i = 0; i < vstEffect->numParams; ++i)
        {
            ...
            if (xmlInfo != nullptr)
            {
                if (auto* param = xmlInfo->getParamForID (i, nullptr))
                {
                    ...
                    {
                        static int compareElements (const String& first, const String& second) noexcept
                        {
    ...
    void initialise (double initialSampleRate, int initialBlockSize)
    {
        ...
        HVST_LOG ("Initialising VST: " + vstModule->pluginName + " (" + getVersion() + ")");
        initialised = true;
    ...
    double getTailLengthSeconds() const override
    {
        ...
        // remain backward compatible with old H plug-ins: anything larger
        // than INT32_MAX is an invalid tail time but old H 64-bit plug-ins
    ...
    {
        HAssert (! isUsingDoublePrecision());
        ...
        switch (opcode)
        {
            case Vst2::audioMasterAutomate:
                if (auto* param = getParameters()[index])
                    ...
                    HAssertfalse; // Invalid parameter index!
...
private:
    ...
    static Vst2::AEffect* constructEffect (const ModuleHandle::Ptr& module)
    {
        ...
        try
        {
            ...
            HVST_LOG ("Creating VST instance: " + module->pluginName);
            ...
            {
                HVST_WRAPPER_INVOKE_MAIN
            }

            if (effect != nullptr && effect->magic == 0x56737450 /* 'VstP' */)
            {
                HAssert (effect->resvd2 == 0);
                HAssert (effect->object != 0);
    ...
    static BusesProperties queryBusIO (Vst2::AEffect* effect)
    {
        ...
        // Workaround for old broken H plug-ins which would return an invalid
    ...
    {
        // Workaround: unfortunately old H VST-2 plug-ins had a bug and would crash if
        // you try to get the speaker arrangement when there are no input channels present.
        // Hopefully, one day (when there are no more old H plug-ins around), we can
    ...
    String getTextForOpcode (const int index, const int opcode) const
    {
        ...
        HAssert (index >= 0 && index < vstEffect->numParams);
        char nm[256] = { 0 };
    ...
    String getVersion() const
    {
        ...
        if (v != 0)
        {
            ...
            // https://forum.H.com/t/issues-with-version-integer-reported-by-vst2/23867/6
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VSTPluginInstance)
};
...
public:
    ...
    void setScaleFactorAndDispatchMessage (double newScaleFactor)
    {
        ...
        if (pluginRespondsToDPIChanges)
            dispatch (Vst2::effVendorSpecific,
                      HMULTICHAR_CONSTANT ('P', 'r', 'e', 'S'),
                      HMULTICHAR_CONSTANT ('A', 'e', 'C', 's'),
    ...
    bool keyPressed (const KeyPress&) override     { return pluginWantsKeys; }
...
private:
    ...
    void openPluginWindow()
    {
        ...
        HVST_LOG ("Opening VST UI: " + plugin.getName());
        isOpen = true;
    ...
    void closePluginWindow()
    {
        if (isOpen)
        {
            ...
            HVST_LOG ("Closing VST UI: " + plugin.getName());
            isOpen = false;
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CarbonWrapperComponent)
    };
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (VSTPluginWindow)
};
#endif
#if HMSVC
 #pragma warning (pop)
...
pointer_sized_int CALLTYPE VSTPluginFormat::dispatcher (AudioPluginInstance* plugin, int32 opcode, int32 index, pointer_sized_int value, void* ptr, float opt)
{
    if (auto* vst = dynamic_cast<VSTPluginInstance*> (plugin))
