...
public:
    ...
    */
    ...
    /** AAX plug-ins need to report a unique "plug-in id" for every audio layout
        ...
        to stay backward compatible with older versions of H).
    ...
    */
    ...
    /** @internal */
    static void CALLTYPE setTypeOfNextNewPlugin (WrapperType);
...
protected:
    /** Callback to query if the AudioProcessor supports a specific layout.
        ...
        implementation will accept any layout. H does basic sanity checks so that
...
public:
    ...
    HDEPRECATED (virtual int getNumParameters());
    HDEPRECATED (virtual const String getParameterName (int parameterIndex));
    HDEPRECATED (virtual String getParameterID (int index));
    HDEPRECATED (virtual float getParameter (int parameterIndex));
    HDEPRECATED (virtual String getParameterName (int parameterIndex, int maximumStringLength));
    HDEPRECATED (virtual const String getParameterText (int parameterIndex));
    HDEPRECATED (virtual String getParameterText (int parameterIndex, int maximumStringLength));
    HDEPRECATED (virtual int getParameterNumSteps (int parameterIndex));
    HDEPRECATED (virtual bool isParameterDiscrete (int parameterIndex) const);
    HDEPRECATED (virtual float getParameterDefaultValue (int parameterIndex));
    HDEPRECATED (virtual String getParameterLabel (int index) const);
    HDEPRECATED (virtual bool isParameterOrientationInverted (int index) const);
    HDEPRECATED (virtual void setParameter (int parameterIndex, float newValue));
    HDEPRECATED (virtual bool isParameterAutomatable (int parameterIndex) const);
    HDEPRECATED (virtual bool isMetaParameter (int parameterIndex) const);
    HDEPRECATED (virtual AudioProcessorParameter::Category getParameterCategory (int parameterIndex) const);
    HDEPRECATED (void beginParameterChangeGesture (int parameterIndex));
    HDEPRECATED (void endParameterChangeGesture (int parameterIndex));
    HDEPRECATED (void setParameterNotifyingHost (int parameterIndex, float newValue));
    ...
    HDEPRECATED_WITH_BODY (int getNumInputChannels()  const noexcept, { return getTotalNumInputChannels(); })
    HDEPRECATED_WITH_BODY (int getNumOutputChannels() const noexcept, { return getTotalNumOutputChannels(); })
    HDEPRECATED_WITH_BODY (const String getInputSpeakerArrangement()  const noexcept, { return cachedInputSpeakerArrString; })
    HDEPRECATED_WITH_BODY (const String getOutputSpeakerArrangement() const noexcept, { return cachedOutputSpeakerArrString; })
    HDEPRECATED (virtual const String getInputChannelName  (int channelIndex) const);
    HDEPRECATED (virtual const String getOutputChannelName (int channelIndex) const);
    HDEPRECATED (virtual bool isInputChannelStereoPair  (int index) const);
    HDEPRECATED (virtual bool isOutputChannelStereoPair (int index) const);
   #endif
...
private:
   ...
   #if HDEBUG
    #if ! HDISABLE_AUDIOPROCESSOR_BEGIN_END_GESTURE_CHECKING
    ...
    HDEPRECATED_WITH_BODY (virtual bool silenceInProducesSilenceOut() const, { return false; })

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioProcessor)
};
