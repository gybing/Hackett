...
private:
    ...
    struct DelayChannelOp  : public RenderingOp
    {
        ...
        HDECLARE_NON_COPYABLE (DelayChannelOp)
    };
    ...
    struct ProcessOp   : public RenderingOp
    {
        ...
        HDECLARE_NON_COPYABLE (ProcessOp)
    };
};
...
struct RenderSequenceBuilder
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (RenderSequenceBuilder)
};
...
AudioProcessorGraph::Node::Ptr AudioProcessorGraph::addNode (AudioProcessor* newProcessor, NodeID nodeID)
{
    ...
    for (auto* n : nodes)
    {
        ...
        {
            HAssertfalse; // Cannot add two copies of the same processor, or duplicate node IDs!
            return {};
        }
    }
...
{
    HAssert (nodes.contains (&src));
    HAssert (nodes.contains (&dst));
