...
/**
    ...
    @tags{Audio}
*/
class API AudioPluginInstance   : public AudioProcessor
{
public:
    //==============================================================================
    /** Destructor.
    ...
    */
    ...
    // the first call.
    HDEPRECATED (String getParameterID (int index) override);
    HDEPRECATED (float getParameter (int parameterIndex) override);
    HDEPRECATED (void setParameter (int parameterIndex, float newValue) override);
    HDEPRECATED (const String getParameterName (int parameterIndex) override);
    HDEPRECATED (String getParameterName (int parameterIndex, int maximumStringLength) override);
    HDEPRECATED (const String getParameterText (int parameterIndex) override);
    HDEPRECATED (String getParameterText (int parameterIndex, int maximumStringLength) override);
    HDEPRECATED (int getParameterNumSteps (int parameterIndex) override);
    HDEPRECATED (bool isParameterDiscrete (int parameterIndex) const override);
    HDEPRECATED (bool isParameterAutomatable (int parameterIndex) const override);
    HDEPRECATED (float getParameterDefaultValue (int parameterIndex) override);
    HDEPRECATED (String getParameterLabel (int parameterIndex) const override);
    HDEPRECATED (bool isParameterOrientationInverted (int parameterIndex) const override);
    HDEPRECATED (bool isMetaParameter (int parameterIndex) const override);
    HDEPRECATED (AudioProcessorParameter::Category getParameterCategory (int parameterIndex) const override);
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioPluginInstance)
};
