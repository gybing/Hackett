...
bool RSAKey::applyToValue (BigInteger& value) const
{
    ...
    {
        HAssertfalse;   // using an uninitialised key
        value.clear();
        return false;
    }
...
{
    HAssert (numBits > 16); // not much point using less than this..
    HAssert (numRandomSeeds == 0 || numRandomSeeds >= 2); // you need to provide plenty of seeds here!
