...
public:
    ...
    {
        HAssert (thumbSampleIndex < data.size());
...
private:
                      ...
                      LevelDataSource* levelData, const OwnedArray<ThumbData>& chans)
    {
        ...
        {
            HAssert (chans.size() == numChannelsCached);
    ...
    {
        HAssert (isPositiveAndBelow (channelNum, numChannelsCached) && isPositiveAndBelow (cacheIndex, data.size()));
