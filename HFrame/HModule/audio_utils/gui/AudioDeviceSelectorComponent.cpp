                                             ...
                                             public Timer
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SimpleDeviceManagerInputLevelMeter)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiInputSelectorComponentListBox)
};
...
public:
    ...
    void updateAllControls()
    {
        ...
        {
            HAssert (setup.manager->getCurrentAudioDevice() == nullptr); // not the correct device type!
...
public:
    ...
    private:
        ...
        {
            HAssert (type == audioInputType || type == audioOutputType);
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ChannelSelectorListBox)
    };

private:
    std::unique_ptr<ChannelSelectorListBox> inputChanList, outputChanList;

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioDeviceSettingsPanel)
};
...
{
    HAssert (minOutputChannels >= 0 && minOutputChannels <= maxOutputChannels);
    HAssert (minInputChannels >= 0 && minInputChannels <= maxInputChannels);
