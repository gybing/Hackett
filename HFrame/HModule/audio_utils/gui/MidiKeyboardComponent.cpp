...
private:
    ...
    HDECLARE_NON_COPYABLE (UpDownButton)
};
...
{
    HAssert (lowestNote >= 0 && lowestNote <= 127);
    HAssert (highestNote >= 0 && highestNote <= 127);
    HAssert (lowestNote <= highestNote);
...
{
    HAssert (midiChannelNumber > 0 && midiChannelNumber <= 16);
...
{
    HAssert (note >= rangeStart && note <= rangeEnd);
...
{
    HAssert (newOctaveNumber >= 0 && newOctaveNumber <= 10);
