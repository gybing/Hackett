...
//==============================================================================
template <typename Type>
void LadderFilter<Type>::prepare (const dsp::ProcessSpec& spec)
{
...
{
    HAssert (newValue >= Type (0) && newValue <= Type (1));
    resonance = newValue;
    updateResonance();
}
...
{
    HAssert (newValue > Type (0));
    cutoffFreqScaler = Type (-2.0 * MathConstants<double>::pi) / newValue;
