...
struct OversamplingDummy   : public Oversampling<SampleType>::OversamplingStage
{
    ...
    {
        HAssert (inputBlock.getNumChannels() <= static_cast<size_t> (ParentType::buffer.getNumChannels()));
        HAssert (inputBlock.getNumSamples() * ParentType::factor <= static_cast<size_t> (ParentType::buffer.getNumSamples()));
    ...
    {
        HAssert (outputBlock.getNumChannels() <= static_cast<size_t> (ParentType::buffer.getNumChannels()));
        HAssert (outputBlock.getNumSamples() * ParentType::factor <= static_cast<size_t> (ParentType::buffer.getNumSamples()));

        outputBlock.copy (ParentType::getProcessedSamples (outputBlock.getNumSamples()));
    }

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OversamplingDummy)
};
...
private:
    ...
    //===============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Oversampling2TimesEquirippleFIR)
};
...
private:
    ...
    */
    ...
    //===============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Oversampling2TimesPolyphaseIIR)
};
...
Oversampling<SampleType>::Oversampling (size_t newNumChannels, size_t newFactor,
    ...
    : numChannels (newNumChannels)
{
    HAssert (isPositiveAndBelow (newFactor, 5) && numChannels > 0);
