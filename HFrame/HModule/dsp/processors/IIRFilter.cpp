...
{
    HAssert (sampleRate > 0.0);
    HAssert (frequency > 0 && frequency <= static_cast<float> (sampleRate * 0.5));
...
{
    HAssert (sampleRate > 0.0);
    HAssert (cutOffFrequency > 0.0 && cutOffFrequency <= sampleRate * 0.5);
    HAssert (Q > 0.0);
...
{
    HAssert (sampleRate > 0);
    HAssert (cutOffFrequency > 0 && cutOffFrequency <= static_cast<NumericType> (sampleRate * 0.5));
    HAssert (Q > 0);
...
{
    HAssert (sampleRate > 0);
    HAssert (frequency > 0 && frequency <= static_cast<NumericType> (sampleRate * 0.5));
    HAssert (Q > 0);
    HAssert (gainFactor > 0);
                                                                    ...
                                                                    size_t numSamples, double sampleRate) const noexcept
{
    ...
    {
        HAssert (frequencies[i] >= 0 && frequencies[i] <= sampleRate * 0.5);
