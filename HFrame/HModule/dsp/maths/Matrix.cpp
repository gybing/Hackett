...
{
    HAssert (vector.isOneColumnVector());
    HAssert (size <= vector.rows);
...
{
    HAssert(vector.isOneColumnVector());
    HAssert(vector.rows >= (2 * (size - 1) + 1));
...
{
    HAssert (columnOne < columns && columnTwo < columns);
...
{
    HAssert (rowOne < rows && rowTwo < rows);
...
{
    auto n = columns;
    HAssert (n == n && n == b.rows && b.isOneColumnVector());
