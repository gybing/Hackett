...
public:
    ...
    */
    ...
    {
        HAssert (isInitialised());  // Use the non-default constructor or call initialise() before first use
        HAssert (isPositiveAndBelow (index, FloatType (getNumPoints())));

        auto i = truncatePositiveToUnsignedInt (index);
        auto f = index - FloatType (i);
        HAssert (isPositiveAndBelow (f, FloatType (1)));
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LookupTable)
};
...
public:
    ...
    */
    ...
    {
        HAssert (value >= minInputValue && value <= maxInputValue);
    ...
    */
    FloatType processSample (FloatType value) const noexcept
    {
        ...
        HAssert (isPositiveAndBelow (index, FloatType (lookupTable.getNumPoints())));
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LookupTableTransform)
};
