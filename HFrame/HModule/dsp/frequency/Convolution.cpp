...
struct ConvolutionEngine
{
    ...
    */
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ConvolutionEngine)
};
...
struct Convolution::Pimpl  : private Thread
{
    ...
    void addToFifo (ChangeRequest type, var parameter)
    {
    ...
    void addToFifo (ChangeRequest* types, var* parameters, int numEntries)
    {
        int start1, size1, start2, size2;
        ...
        // If you hit this assertion then you have requested more impulse response
        // changes than the Convolution class can handle.
        HAssert (numEntries > 0 && size1 + size2 > 0);
    ...
    void readFromFifo (ChangeRequest& type, var& parameter)
    {
        int start1, size1, start2, size2;
...
private:
    ...
    */
    ...
    Array<var> fifoRequestsParameter;         // an array of change parameters

    Array<ChangeRequest> requestsType;              // an array of ChangeRequest
    Array<var> requestsParameter;             // an array of change parameters
...
Convolution::Convolution()
{
    ...
    pimpl->addToFifo (Convolution::Pimpl::ChangeRequest::changeEngine, var (0));
}
                                       ...
                                       bool wantsNormalisation)
{
    ...
    Array<var> sourceParameter;

    sourceParameter.add (var ((int) ConvolutionEngine::ProcessingInformation::SourceType::sourceBinaryData));
    sourceParameter.add (var (sourceData, sourceDataSize));

    var parameters[] = { var (sourceParameter),
                               var (static_cast<int64> (wantedSize)),
                               var (wantsStereo),
                               var (wantsTrimming),
                               var (wantsNormalisation) };
                                       ...
                                       bool wantsTrimming, size_t size, bool wantsNormalisation)
{
    ...
    sourceParameter.add (var ((int) ConvolutionEngine::ProcessingInformation::SourceType::sourceAudioFile));
    sourceParameter.add (var (fileImpulseResponse.getFullPathName()));
                                                       ...
                                                       bool wantsStereo, bool wantsTrimming, bool wantsNormalisation, size_t size)
{
    ...
    Array<var> sourceParameter;
    sourceParameter.add (var ((int) ConvolutionEngine::ProcessingInformation::SourceType::sourceAudioBuffer));
    sourceParameter.add (var (bufferSampleRate));
...
{
    HAssert (isPositiveAndBelow (spec.numChannels, static_cast<uint32> (3))); // only mono and stereo is supported

    Pimpl::ChangeRequest types[] = { Pimpl::ChangeRequest::changeSampleRate,
                                     Pimpl::ChangeRequest::changeMaximumBufferSize };

    var parameters[] = { var (spec.sampleRate),
                               var (static_cast<int> (spec.maximumBlockSize)) };
...
void Convolution::processSamples (const AudioBlock<const float>& input, AudioBlock<float>& output, bool isBypassed) noexcept
{
    ...
    HAssert (isPositiveAndBelow (input.getNumChannels(), static_cast<size_t> (3))); // only mono and stereo is supported
