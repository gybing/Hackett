...
struct FFT::Engine
{
    ...
    static FFT::Instance* createBestEngineForPlatform (int order)
    {
        ...
        HAssertfalse;  // This should never happen as the fallback engine should always work!
        return nullptr;
    }
    ...
    struct FFTConfig
    {
            ...
            : fftSize (sizeOfFFT), inverse (isInverse), twiddleTable ((size_t) sizeOfFFT)
        {
            ...
            for (int i = 0; i < numElementsInArray (factors); ++i)
            {
                ...
                HAssert (divisor == 1 || divisor == 2 || divisor == 4);
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (FFTConfig)
    };
...
//==============================================================================
//==============================================================================
#if HDSP_USE_SHARED_FFTW || HDSP_USE_STATIC_FFTW

#if HDSP_USE_STATIC_FFTW
extern "C"
{
...
{
   #if HDSP_USE_STATIC_FFTW
    // if the H developer has gone through the hassle of statically
