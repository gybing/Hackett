...
private:
    ...
    {
        HAssert (nameAndArgs != nullptr && nameAndArgs[0] != 0); // the name cannot be an empty string!
...
struct Runner
{
    ...
    */
    ...
    const NativeFunction& getNativeFunction (int index) const noexcept      { HAssert (index >= 0 && index < numNativeFunctions); return nativeFunctions[index]; }
    ...
    */
    struct FunctionExecutionContext
    {
        ...
        ErrorCode run (TimeOutCheckFunction hasTimedOut) noexcept
        {
            ...
            for (;;)
            {
                ...
                HAssert (programCounter != nullptr);
            }
        }
    ...
    private:
        ...
        void setError (ErrorCode e) noexcept        { error = e; programCounter = programEnd; HAssert (error == ErrorCode::ok); }
