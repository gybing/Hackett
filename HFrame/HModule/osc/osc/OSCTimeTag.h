...
/**
    ...
    For a more user-friendly time format, convert OSCTimeTag to a Time object
    using toTime().

    @tags{OSC}
*/
class API OSCTimeTag
{
public:
    //==============================================================================
    /** Default constructor.
    ...
    */
    ...
    /** Constructs an OSCTimeTag object from a Time object. */
    OSCTimeTag (Time time) noexcept;

    /** Returns a Time object representing the same time as the OSCTimeTag.

        If the OSCTimeTag has the special value representing "immediately", the
        resulting Time object will represent an arbitrary point of time (but
        guaranteed to be in the past), since Time does not have such a special value.
    */
