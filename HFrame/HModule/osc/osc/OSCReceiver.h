...
public:
    ...
    */
     ...
     */
    struct API  MessageLoopCallback {};
    ...
    */
    ...
    /** A class for receiving only those OSC messages from an OSCReceiver that match a
        ...
        (for example "/H/fader1", /H/knob2" etc.) and you want to route those to
        different objects. This class contains pre-build functionality for that OSC
        address routing, including wildcard pattern matching (e.g. "/H/fader[0-9]").
    ...
    */
    template <typename CallbackType = MessageLoopCallback>
    class API ListenerWithOSCAddress
    {
    public:
        /** Destructor. */
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OSCReceiver)
};
