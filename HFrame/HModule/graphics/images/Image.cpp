...
{
    HAssert (format == Image::RGB || format == Image::ARGB || format == Image::SingleChannel);
    HAssert (w > 0 && h > 0); // It's illegal to create a zero-sized image!
}
...
private:
    ...
    HLEAK_DETECTOR (SoftwarePixelData)
};
...
public:
    ...
    ImagePixelData::Ptr clone() override
    {
        HAssert (getReferenceCount() > 0); // (This method can't be used on an unowned pointer, as it will end up self-deleting)
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SubsectionPixelData)
};
...
HDECLARE_DEPRECATED_STATIC (const Image Image::null;)
    ...
    : width (w), height (h)
{
    ...
    HAssert (im.image != nullptr);
    HAssert (x >= 0 && y >= 0 && w > 0 && h > 0 && x + w <= im.getWidth() && y + h <= im.getHeight());

    im.image->initialiseBitmapData (*this, x, y, mode);
    HAssert (data != nullptr && pixelStride > 0 && lineStride != 0);
}
...
{
    HAssert (isPositiveAndBelow (x, width) && isPositiveAndBelow (y, height));
