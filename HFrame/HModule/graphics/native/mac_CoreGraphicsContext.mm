...
public:
        ...
        : ImagePixelData (format, w, h)
    {
       ...
       # if HMAC && defined (MAC_OS_X_VERSION_10_14) && MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_14
    ...
    //==============================================================================
    static CGImageRef getCachedImageRef (const Image& HImage, CGColorSpaceRef colourSpace)
    {
        auto cgim = dynamic_cast<CoreGraphicsImage*> (HImage.getPixelData());
        ...
        CGImageRef ref = createImage (HImage, colourSpace, false);
    ...
    static CGImageRef createImage (const Image& HImage, CGColorSpaceRef colourSpace, bool mustOutliveSource)
    {
        const Image::BitmapData srcData (HImage, Image::BitmapData::readOnly);
        ...
        CGImageRef imageRef = CGImageCreate ((size_t) srcData.width,
                                             ...
                                             colourSpace, getCGImageFlags (HImage.getFormat()), provider,
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CoreGraphicsImage)
};
      ...
      state (new SavedState())
{
    ...
    bool enableFontSmoothing
            #if HDISABLE_COREGRAPHICS_FONT_SMOOTHING
             = false;
            #else
             = true;
            #endif
...
void CoreGraphicsContext::addTransform (const AffineTransform& transform)
{
    ...
    HAssert (getPhysicalPixelScaleFactor() > 0.0f);
    HAssert (getPhysicalPixelScaleFactor() > 0.0f);
}
...
float CoreGraphicsContext::getPhysicalPixelScaleFactor()
{
    ...
    return targetScale * (float) (hypot (t.a, t.c) + hypot (t.b, t.d)) / 2.0f;
}
...
//==============================================================================
#if USE_COREGRAPHICS_RENDERING && HUSE_COREIMAGE_LOADER
Image loadWithCoreImage (InputStream& input)
{
   ...
   #endif
    {
        ...
        if (imageSource != nullptr)
        {
            ...
            if (loadedImage != nullptr)
            {
                ...
                HAssert (cgImage != nullptr); // if USE_COREGRAPHICS_RENDERING is set, the CoreGraphicsImage class should have been used.
...
Image createImageFromCIImage (CIImage*, int, int);
Image createImageFromCIImage (CIImage* im, int w, int h)
{
...
CGImageRef createCoreGraphicsImage (const Image& HImage, CGColorSpaceRef colourSpace,
                                         const bool mustOutliveSource)
{
    return CoreGraphicsImage::createImage (HImage, colourSpace, mustOutliveSource);
}

CGContextRef getImageContext (const Image& image)
{
 ...
 NSImage* imageToNSImage (const Image& image, float scaleFactor)
 {
     HAUTORELEASEPOOL
     {
         ...
         auto imageRef = createCoreGraphicsImage (image, colourSpace, true);
         CGColorSpaceRelease (colourSpace);
