...
private:
    ...
    static File getCacheDirectory()
    {
        static File result = [] ()
        {
            ...
            if (appContext != nullptr)
            {
                ...
                return File (HString (env, jPath.get()));
            }
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AndroidTypeface)
};
...
{
    HAssertfalse; // not available unless using FreeType
}
