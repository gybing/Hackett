...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (GlyphCache)
};
...
public:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CachedGlyphEdgeTable)
};
...
namespace GradientPixelIterators
{
    ...
    struct Radial
    {
              ...
              gy1 (gradient.point1.y)
        {
            ...
            HAssert (roundToInt (std::sqrt (maxDist) * invScale) <= numEntries);
        }
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (TransformedRadial)
    };
}

#define HPERFORM_PIXEL_OP_LOOP(op) \
{ \
...
namespace EdgeTableFillers
{
    ...
    private:
        ...
        {
            HPERFORM_PIXEL_OP_LOOP (blend (colour))
        }
        ...
        forcedinline void replaceLine (PixelRGB* dest, PixelARGB colour, int width) const noexcept
        {
            if ((size_t) destData.pixelStride == sizeof (*dest) && areRGBComponentsEqual)
                ...
                HPERFORM_PIXEL_OP_LOOP (set (colour));
        }
        ...
        forcedinline void replaceLine (PixelAlpha* dest, const PixelARGB colour, int width) const noexcept
        {
            if ((size_t) destData.pixelStride == sizeof (*dest))
                ...
                HPERFORM_PIXEL_OP_LOOP (setAlpha (colour.getAlpha()))
        }
        ...
        HDECLARE_NON_COPYABLE (SolidColour)
    };
    ...
    struct Gradient  : public GradientType
    {
        ...
        void handleEdgeTableLine (int x, int width, int alphaLevel) const noexcept
        {
            ...
            if (alphaLevel < 0xff)
                HPERFORM_PIXEL_OP_LOOP (blend (GradientType::getPixel (x++), (uint32) alphaLevel))
            else
                HPERFORM_PIXEL_OP_LOOP (blend (GradientType::getPixel (x++)))
        }
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (Gradient)
    };
    ...
    struct ImageFill
    {
        ...
        void handleEdgeTableLine (int x, int width, int alphaLevel) const noexcept
        {
            ...
            {
                if (alphaLevel < 0xfe)
                    HPERFORM_PIXEL_OP_LOOP (blend (*getSrcPixel (x++ % srcData.width), (uint32) alphaLevel))
                else
                    HPERFORM_PIXEL_OP_LOOP (blend (*getSrcPixel (x++ % srcData.width)))
            }
            else
            {
                HAssert (x >= 0 && x + width <= srcData.width);

                if (alphaLevel < 0xfe)
                    HPERFORM_PIXEL_OP_LOOP (blend (*getSrcPixel (x++), (uint32) alphaLevel))
                else
        ...
        void handleEdgeTableLineFull (int x, int width) const noexcept
        {
            ...
            {
                if (extraAlpha < 0xfe)
                    HPERFORM_PIXEL_OP_LOOP (blend (*getSrcPixel (x++ % srcData.width), (uint32) extraAlpha))
                else
                    HPERFORM_PIXEL_OP_LOOP (blend (*getSrcPixel (x++ % srcData.width)))
            }
            else
            {
                ...
                if (extraAlpha < 0xfe)
                    HPERFORM_PIXEL_OP_LOOP (blend (*getSrcPixel (x++), (uint32) extraAlpha))
                else
                    copyRow (dest, getSrcPixel (x), width);
            }
        }
        ...
        {
            HAssert (x - xOffset >= 0 && x + width - xOffset <= srcData.width);
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (ImageFill)
    };
    ...
    struct TransformedImageFill
    {
        ...
        void handleEdgeTableLine (int x, int width, int alphaLevel) noexcept
        {
            ...
            if (alphaLevel < 0xfe)
                HPERFORM_PIXEL_OP_LOOP (blend (*span++, (uint32) alphaLevel))
            else
                HPERFORM_PIXEL_OP_LOOP (blend (*span++))
        }
    ...
    private:
        ...
        private:
            ...
            HDECLARE_NON_COPYABLE (TransformedImageSpanInterpolator)
        };
        ...
        HDECLARE_NON_COPYABLE (TransformedImageFill)
    };
...
struct ClipRegions
{
    ...
    private:
        ...
        private:
            ...
            HDECLARE_NON_COPYABLE (SubRectangleIterator)
        };
        ...
        private:
            ...
            HDECLARE_NON_COPYABLE (SubRectangleIteratorFloat)
        };
...
public:
    ...
    void fillRect (Rectangle<int> r, bool replaceContents)
    {
        if (clip != nullptr)
        {
            ...
            {
                HAssert (! replaceContents); // not implemented..
                fillRectAsPath (r);
            }
        }
    }
    ...
    void fillShape (typename BaseRegionType::Ptr shapeToFill, bool replaceContents)
    {
        ...
        if (shapeToFill != nullptr)
        {
            ...
            {
                HAssert (! replaceContents); // that option is just for solid colours
...
public:
    ...
    void restore()
    {
        ...
        {
            HAssertfalse; // trying to pop with an empty stack!
        }
    }
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (SavedStateStack)
};
