...
namespace
{
    ...
    Rectangle<Type> coordsToRectangle (Type x, Type y, Type w, Type h) noexcept
    {
        ...
        HAssert ((int) x >= -maxVal && (int) x <= maxVal
...
{
    HAssert (imageToDrawOnto.isValid()); // Can't draw into a null image!
}
...
{
    HAssert (r.getWidth() >= 0.0f && r.getHeight() >= 0.0f);
...
{
    HAssert (checkWidth > 0 && checkHeight > 0); // can't be zero or less!
...
{
    HAssert (n >= 0 && n < numDashLengths); // your start index must be valid!
    ...
    if (totalLen >= 0.1)
    {
        ...
        {
            HAssert (dashLengths[n] > 0); // can't have zero-length dashes!
