...
{
#if HINCLUDE_ZLIB_CODE
  #undef OS_CODE
  #undef fdopen
  #include "../../core/zip/zlib/zlib.h"
  #undef OS_CODE
#else
  #include HZLIB_INCLUDE_PATH
#endif
}
...
#if HINCLUDE_PNGLIB_CODE || ! defined (HINCLUDE_PNGLIB_CODE)
...
namespace PNGHelpers
{
    ...
    static void HCDECL writeDataCallback (png_structp png, png_bytep data, png_size_t length)
    {
        static_cast<OutputStream*> (png_get_io_ptr (png))->write (data, length);
    }

   #if ! HUSING_COREIMAGE_LOADER
    static void HCDECL readCallback (png_structp png, png_bytep data, png_size_t length)
    {
    ...
    static void HCDECL errorCallback (png_structp p, png_const_charp)
    {
    ...
    static void HCDECL warningCallback (png_structp, png_const_charp) {}
    ...
    {
        // now convert the data to a H image format..
...
#if HUSING_COREIMAGE_LOADER
 Image loadWithCoreImage (InputStream&);
#endif

Image PNGImageFormat::decodeImage (InputStream& in)
{
   #if HUSING_COREIMAGE_LOADER
    return loadWithCoreImage (in);
   #else
