...
GetTypefaceForFont GetTypefaceForFont = nullptr;
...
public:
    ...
    HDECLARE_SINGLETON (TypefaceCache, false)
    ...
    Typeface::Ptr findTypefaceFor (const Font& font)
    {
        ...
        if (GetTypefaceForFont == nullptr)
            face.typeface = Font::getDefaultTypefaceForFont (font);
        else
            face.typeface = GetTypefaceForFont (font);

        HAssert (face.typeface != nullptr); // the look and feel must return a typeface!
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (TypefaceCache)
};

HIMPLEMENT_SINGLETON (TypefaceCache)
...
void Font::setFallbackFontName (const String& name)
{
   ...
   #if HMAC || HIOS
    HAssertfalse; // Note that use of a fallback font isn't currently implemented in OSX..
   #endif
}
