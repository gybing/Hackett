...
ColourGradient::ColourGradient() noexcept  : isRadial (false)
{
    ...
    #define HCOLOURGRADIENT_CHECK_COORDS_INITIALISED   HAssert (point1.x != 987654.0f);
   #else
    #define HCOLOURGRADIENT_CHECK_COORDS_INITIALISED
   #endif
}
...
int ColourGradient::addColour (const double proportionAlongGradient, Colour colour)
{
    ...
    HAssert (proportionAlongGradient >= 0 && proportionAlongGradient <= 1.0);
...
{
    HAssert (index > 0 && index < colours.size() - 1);
...
{
    HAssert (colours.getReference(0).position == 0.0); // the first colour specified has to go at position 0
...
{
    HCOLOURGRADIENT_CHECK_COORDS_INITIALISED // Trying to use this object without setting its coordinates?
    HAssert (colours.size() >= 2);
    HAssert (numEntries > 0);
    HAssert (colours.getReference(0).position == 0.0); // The first colour specified has to go at position 0
    ...
    for (int j = 1; j < colours.size(); ++j)
    {
        ...
        {
            HAssert (index >= 0 && index < numEntries);
