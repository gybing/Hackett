...
const int edgeTableDefaultEdgesPerLine = 32;
...
EdgeTable::EdgeTable (Rectangle<int> area, const Path& path, const AffineTransform& transform)
   : bounds (area),
     ...
     maxEdgesPerLine (jmax (edgeTableDefaultEdgesPerLine / 2,
     ...
     lineStrideElements ((edgeTableDefaultEdgesPerLine * 2) + 1)
{
    ...
    else
    {
        ...
        HAssert (lineY < bounds.getHeight());
        t[0] = 2;
        t[1] = x1;
        t[2] = y2 & 255;
        t[3] = x2;
...
inline void EdgeTable::remapWithExtraSpace (int numPoints)
{
    ...
    HAssert (numPoints < maxEdgesPerLine);
}
...
{
    HAssert (y >= 0 && y < bounds.getHeight());
...
void EdgeTable::intersectWithEdgeTableLine (const int y, const int* const otherLine)
{
    ...
    while (srcNum1 > 0 && srcNum2 > 0)
    {
        ...
        if (nextX > lastX)
        {
            ...
            HAssert (isPositiveAndBelow (nextLevel, 256));
