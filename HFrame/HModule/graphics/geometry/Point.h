...
public:
    ...
    HCONSTEXPR Point() = default;

    /** Creates a copy of another point. */
    HCONSTEXPR Point (const Point&) = default;

    /** Creates a point from an (x, y) position. */
    HCONSTEXPR Point (ValueType initialX, ValueType initialY) noexcept : x (initialX), y (initialY) {}

    //==============================================================================
    /** Copies this point from another one. */
    Point& operator= (const Point&) = default;

    HCONSTEXPR inline bool operator== (Point other) const noexcept      { return x == other.x && y == other.y; }
    HCONSTEXPR inline bool operator!= (Point other) const noexcept      { return x != other.x || y != other.y; }

    /** Returns true if the point is (0, 0). */
    HCONSTEXPR bool isOrigin() const noexcept                           { return x == ValueType() && y == ValueType(); }

    /** Returns true if the coordinates are finite values. */
    HCONSTEXPR inline bool isFinite() const noexcept                    { return isfinite(x) && isfinite(y); }

    /** Returns the point's x coordinate. */
    HCONSTEXPR inline ValueType getX() const noexcept                   { return x; }

    /** Returns the point's y coordinate. */
    HCONSTEXPR inline ValueType getY() const noexcept                   { return y; }
    ...
    HCONSTEXPR Point withX (ValueType newX) const noexcept              { return Point (newX, y); }

    /** Returns a point which has the same X position as this one, but a new Y. */
    HCONSTEXPR Point withY (ValueType newY) const noexcept              { return Point (x, newY); }
    ...
    HCONSTEXPR Point translated (ValueType deltaX, ValueType deltaY) const noexcept    { return Point (x + deltaX, y + deltaY); }

    /** Adds two points together */
    HCONSTEXPR Point operator+ (Point other) const noexcept             { return Point (x + other.x, y + other.y); }
    ...
    HCONSTEXPR Point operator* (Point<OtherType> other) const noexcept  { return Point ((ValueType) (x * other.x), (ValueType) (y * other.y)); }
    ...
    HCONSTEXPR Point operator* (FloatType multiplier) const noexcept    { return Point ((ValueType) (x * multiplier), (ValueType) (y * multiplier)); }

    /** Returns a point whose coordinates are divided by a given scalar value. */
    template <typename FloatType>
    HCONSTEXPR Point operator/ (FloatType divisor) const noexcept       { return Point ((ValueType) (x / divisor), (ValueType) (y / divisor)); }
    ...
    HCONSTEXPR Point operator-() const noexcept                         { return Point (-x, -y); }
    ...
    ValueType getDistanceFromOrigin() const noexcept                               { return hypot (x, y); }

    /** Returns the straight-line distance between this point and another one. */
    ValueType getDistanceFrom (Point other) const noexcept                         { return hypot (x - other.x, y - other.y); }

    /** Returns the square of the straight-line distance between this point and the origin. */
    HCONSTEXPR ValueType getDistanceSquaredFromOrigin() const noexcept         { return x * x + y * y; }

    /** Returns the square of the straight-line distance between this point and another one. */
    HCONSTEXPR ValueType getDistanceSquaredFrom (Point other) const noexcept   { return (*this - other).getDistanceSquaredFromOrigin(); }
    ...
    */
    ...
    HCONSTEXPR FloatType getDotProduct (Point other) const noexcept          { return x * other.x + y * other.y; }
    ...
    */
    ...
    HCONSTEXPR Point<int> toInt() const noexcept              { return Point<int> (static_cast<int> (x), static_cast<int> (y)); }

    /** Casts this point to a Point<float> object. */
    HCONSTEXPR Point<float> toFloat() const noexcept          { return Point<float> (static_cast<float> (x), static_cast<float> (y)); }

    /** Casts this point to a Point<double> object. */
    HCONSTEXPR Point<double> toDouble() const noexcept        { return Point<double> (static_cast<double> (x), static_cast<double> (y)); }

    /** Casts this point to a Point<int> object using roundToInt() to convert the values. */
    HCONSTEXPR Point<int> roundToInt() const noexcept         { return Point<int> (roundToInt (x), roundToInt (y)); }
