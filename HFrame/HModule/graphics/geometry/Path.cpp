...
#define HCHECK_COORDS_ARE_VALID(x, y) \
    HAssert (x == x && y == y);
...
{
    HAssert (numberOfSides > 1); // this would be silly.
...
{
    HAssert (numberOfPoints > 1); // this would be silly.
...
void Path::applyTransform (const AffineTransform& transform) noexcept
{
    ...
    while (d < end)
    {
        ...
        if (isMarker (type, moveMarker))
        {
            ...
            HCHECK_COORDS_ARE_VALID (d[0], d[1])
        ...
        else if (isMarker (type, quadMarker))
        {
            ...
            HCHECK_COORDS_ARE_VALID (d[0], d[1])
            HCHECK_COORDS_ARE_VALID (d[2], d[3])
        ...
        else if (isMarker (type, cubicMarker))
        {
            ...
            HCHECK_COORDS_ARE_VALID (d[0], d[1])
            HCHECK_COORDS_ARE_VALID (d[2], d[3])
            HCHECK_COORDS_ARE_VALID (d[4], d[5])
...
void Path::loadPathFromStream (InputStream& source)
{
    while (! source.isExhausted())
    {
        ...
        default:
            HAssertfalse; // illegal char in the stream
            break;
        }
    }
}
...
String Path::toString() const
{
    ...
    for (int i = 0; i < data.size();)
    {
        ...
        {
            HAssert (isMarker (type, closeSubPathMarker));
            markerChar = 'z';
        }
...
void Path::restoreFromString (StringRef stringVersion)
{
    ...
    for (;;)
    {
        ...
        switch (marker)
        {
            ...
            default:    HAssertfalse; break; // illegal string format?
        }
    }
}
