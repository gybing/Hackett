...
{
    HCONSTEXPR static const int magicNumber            = (int) ByteOrder::makeInt ('P', 'R', 'O', 'P');
    HCONSTEXPR static const int magicNumberCompressed  = (int) ByteOrder::makeInt ('C', 'P', 'R', 'P');

    HCONSTEXPR static const char* const fileTag        = "PROPERTIES";
    HCONSTEXPR static const char* const valueTag       = "VALUE";
    HCONSTEXPR static const char* const nameAttribute  = "name";
    HCONSTEXPR static const char* const valueAttribute = "val";
}
...
File PropertiesFile::Options::getDefaultFile() const
{
    ...
    HAssert (applicationName == File::createLegalFileName (applicationName));
        ...
        && ! osxLibrarySubFolder.startsWith ("Containers"))
    {
        /* The PropertiesFile class always used to put its settings files in "Library/Preferences", but Apple
           ...
           If your app needs to load settings files that were created by older versions of H and
...
bool PropertiesFile::saveAsBinary()
{
...

    {
        ...
        else
        {
            ...
            HAssert (options.storageFormat == storeAsBinary);
