...
public:
    ...
    ~SharedObject()
    {
        HAssert (parent == nullptr); // this should never happen unless something isn't obeying the ref-counting!
    ...
    void addChild (SharedObject* child, int index, UndoManager* undoManager)
    {
        if (child != nullptr && child->parent != this)
        {
            if (child != this && ! isAChildOf (child))
            {
                ...
                {
                    HAssert (child->parent->children.indexOf (child) >= 0);
    ...
    void moveChild (int currentIndex, int newIndex, UndoManager* undoManager)
    {
        ...
        HAssert (isPositiveAndBelow (currentIndex, children.size()));
    ...
    {
        HAssert (newOrder.size() == children.size());
    ...
    struct SetPropertyAction  : public UndoableAction
    {
        ...
        {
            HAssert (! (isAddingNewProperty && target->hasProperty (name)));
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (SetPropertyAction)
    };
    ...
    struct AddOrRemoveChildAction  : public UndoableAction
    {
        ...
        bool undo() override
        {
            ...
            else
            {
                ...
                HAssert (childIndex < target->children.size());
                target->removeChild (childIndex, nullptr);
            }
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (AddOrRemoveChildAction)
    };
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE (MoveChildAction)
    };
...
HDECLARE_DEPRECATED_STATIC (const ValueTree ValueTree::invalid;)
...
{
    HAssert (object != nullptr || source.object == nullptr); // Trying to add properties to a null ValueTree will fail!
...
{
    HAssert (object != nullptr || source.object == nullptr); // Trying to copy to a null ValueTree will fail!
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ValueTreePropertyValueSource)
};
...
{
    HAssert (object != nullptr); // Trying to add a child to a null ValueTree!
