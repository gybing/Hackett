...
#if HENABLE_LIVE_CONSTANT_EDITOR && ! DOXYGEN

//==============================================================================
/** You can safely ignore all the stuff in this namespace - it's a bunch of boilerplate
    code used to implement the HLIVE_CONSTANT functionality.
*/
namespace LiveConstantEditor
{
    ...
    inline String getAsCode (Colour v, bool)                { return "Colour (0x" + CharacterFunctions::hexToString ((int) v.getARGB()).paddedLeft ('0', 8) + ")"; }
    ...
    struct API  LiveValueBase
    {
        ...
        HDECLARE_NON_COPYABLE (LiveValueBase)
    };

    //==============================================================================
    struct API  LivePropertyEditorBase  : public Component
    {
        ...
        HDECLARE_NON_COPYABLE (LivePropertyEditorBase)
    };
    ...
    struct LiveValue  : public LiveValueBase
    {
        ...
        HDECLARE_NON_COPYABLE (LiveValue)
    };

    //==============================================================================
    class API ValueList  : private AsyncUpdater,
    ...
    public:
        ...
        HDECLARE_SINGLETON (ValueList, false)
    ...
    inline LiveValue<Type>& getValue (const char* file, int line, const Type& initialValue)
    {
        ...
        HAssert (File::isAbsolutePath (file));
...
//==============================================================================
 ...
 /**
    ...
    wraps, but if HENABLE_LIVE_CONSTANT_EDITOR is enabled, it injects a class wrapper
    ...
    The editor will also load the original source-file that contains each HLIVE_CONSTANT
    ...
    {
        g.fillAll (HLIVE_CONSTANT (Colour (0xffddddff)));

        Colour fontColour = HLIVE_CONSTANT (Colour (0xff005500));
        float fontSize = HLIVE_CONSTANT (16.0f);

        g.setColour (fontColour);
        g.setFont (fontSize);

        g.drawFittedText (HLIVE_CONSTANT ("Hello world!"),
                          getLocalBounds(), Justification::centred, 2);
    }
    @endcode
 */
 #define HLIVE_CONSTANT(initialValue) \
