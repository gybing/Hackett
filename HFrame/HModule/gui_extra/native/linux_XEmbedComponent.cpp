...
Window getCurrentFocusWindow (ComponentPeer*);

//==============================================================================
unsigned long createKeyProxyWindow (ComponentPeer*);
void deleteKeyProxyWindow (ComponentPeer*);
...
public:
    ...
    struct SharedKeyWindow : public ReferenceCountedObject
    {
        SharedKeyWindow (ComponentPeer* peerToUse)
            : keyPeer (peerToUse),
              keyProxy (createKeyProxyWindow (keyPeer))
        {}

        ~SharedKeyWindow()
        {
            deleteKeyProxyWindow (keyPeer);
...
private:
    ...
    void configureNotify()
    {
        ...
        if (XGetWindowAttributes (dpy, client, &attr))
        {
            ...
            HAssert (newBounds.getX() == 0 && newBounds.getY() == 0);
...
//==============================================================================
bool handleXEmbedEvent (ComponentPeer* p, void* e)
{
    return XEmbedComponent::Pimpl::dispatchX11Event (p, reinterpret_cast<const XEvent*> (e));
}

unsigned long getCurrentFocusWindow (ComponentPeer* peer)
{
