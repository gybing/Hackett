...
//==========================================================================
#if defined(HFIREBASE_INSTANCE_ID_SERVICE_CLASSNAME)
 #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
...
#if defined(HFIREBASE_MESSAGING_SERVICE_CLASSNAME)
 #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
...
struct PushNotifications::Pimpl
{
    ...
    void sendLocalNotification (const PushNotifications::Notification& n)
    {
        ...
        if (notificationManager.get() != 0)
        {
            auto notification = HNotificationToJavaNotification (n);
    ...
    void getDeliveredNotifications() const
    {
        if (getAndroidSDKVersion() >= 23)
        {
            ...
            HAssert (notificationManager.get() != 0);
    ...
    void notifyListenersAboutLocalNotification (const LocalRef<jobject>& intent)
    {
        ...
        auto packageName  = HString ((jstring) env->CallObjectMethod (context.get(), AndroidContext.getPackageName));

        String notificationString                = packageName + ".HNOTIFICATION.";
        String notificationButtonActionString    = packageName + ".HNOTIFICATION_BUTTON_ACTION.";
        String notificationTextInputActionString = packageName + ".HNOTIFICATION_TEXT_INPUT_ACTION.";

        auto actionString = HString ((jstring) env->CallObjectMethod (intent, AndroidIntent.getAction));
        ...
        else if (getAndroidSDKVersion() >= 20 && actionString.contains (notificationTextInputActionString))
        {
            ...
            if (remoteInputResult.get() != 0)
            {
                ...
                responseString = HString (responseStringRef.get());
            }
    ...
    String getDeviceToken() const
    {
        ...
        return HString ((jstring) env->CallObjectMethod (instanceId, FirebaseInstanceId.getToken));
      #else
        return {};
      #endif
    }
    ...
    void notifyListenersAboutUpstreamMessageSent (const LocalRef<jstring>& messageId)
    {
        ...
        {
            auto midString = HString ((jstring) mid.get());
                                                          ...
                                                          const LocalRef<jstring>& error)
    {
        ...
        {
            auto midString = HString ((jstring) mid.get());
            auto eString   = HString ((jstring) e.get());
    ...
    static LocalRef<jobject> HNotificationToJavaNotification (const PushNotifications::Notification& n)
    {
        auto* env = getEnv();
    ...
    static void setupRequiredFields (const PushNotifications::Notification& n, LocalRef<jobject>& notificationBuilder)
    {
        ...
        auto actionStringSuffix = javaString (".HNOTIFICATION." + n.identifier);
        ...
        env->CallObjectMethod (notifyIntent, AndroidIntent.putExtras, HNotificationToBundle (n).get());
        ...
        if (getAndroidSDKVersion() >= 21 && n.publicVersion != nullptr)
        {
            ...
            HAssert (n.publicVersion->publicVersion == nullptr);
    ...
    static LocalRef<jobject> HNotificationToBundle (const PushNotifications::Notification& n)
    {
        auto* env = getEnv();
                                                  ...
                                                  LocalRef<jobject>& notificationBuilder)
    {
        ...
        auto packageNameString  = LocalRef<jstring> ((jstring) (env->CallObjectMethod (context.get(), AndroidContext.getPackageName)));
        auto actionStringSuffix = javaString (".HNOTIFICATION_DELETED." + n.identifier);
        auto actionString       = LocalRef<jstring> ((jstring)env->CallObjectMethod (packageNameString, JavaString.concat, actionStringSuffix.get()));

        env->CallObjectMethod (deleteIntent, AndroidIntent.setAction, actionString.get());
        env->CallObjectMethod (deleteIntent, AndroidIntent.putExtras, HNotificationToBundle (n).get());
    ...
    static void setupActions (const PushNotifications::Notification& n, LocalRef<jobject>& notificationBuilder)
    {
        ...
        for (const auto& action : n.actions)
        {
            ...
            auto packageNameString   = LocalRef<jstring> ((jstring) (env->CallObjectMethod (context.get(), AndroidContext.getPackageName)));
            const String notificationActionString = isTextStyle ? ".HNOTIFICATION_TEXT_INPUT_ACTION." : ".HNOTIFICATION_BUTTON_ACTION.";
    ...
    static LocalRef<jobject> HUrlToAndroidUri (const URL& url)
    {
        auto* env = getEnv();
        LocalRef<jobject> context (getMainActivity());
        ...
        if (bundle.get() != 0)
        {
            ...
            for (;;)
            {
                ...
                if (object.get() != nullptr)
                {
                    ...
                    var value = HString (classAsString) == "java.lang.Bundle" ? bundleToVar (object) : var (HString (objectAsString.get()));
                    dynamicObject->setProperty (HString (key.get()), value);
                }
                else
                {
                    dynamicObject->setProperty (HString (key.get()), {});
                }
            }
    ...
    static PushNotifications::Notification firebaseRemoteNotificationToJuceNotification (jobject remoteNotification)
    {
        ...
        for (;;)
        {
            ...
            dataDynamicObject->setProperty (HString (key.get()), HString (value.get()));
        }

        var dataVar (dataDynamicObject.get());

        DynamicObject::Ptr propertiesDynamicObject = new DynamicObject();
        propertiesDynamicObject->setProperty ("collapseKey", HString (collapseKey.get()));
        propertiesDynamicObject->setProperty ("from", HString (from.get()));
        propertiesDynamicObject->setProperty ("messageId", HString (messageId.get()));
        propertiesDynamicObject->setProperty ("messageType", HString (messageType.get()));
        propertiesDynamicObject->setProperty ("to", HString (to.get()));
        ...
        if (notification != 0)
        {
            ...
            n.identifier = HString (tag.get());
            n.title      = HString (title.get());
            ...
            propertiesDynamicObject->setProperty ("clickAction",           HString (clickAction.get()));
            propertiesDynamicObject->setProperty ("bodyLocalizationKey",   HString (bodyLocalizationKey.get()));
            propertiesDynamicObject->setProperty ("titleLocalizationKey",  HString (titleLocalizationKey.get()));
            ...
            propertiesDynamicObject->setProperty ("link",                  link.get() != 0 ? HString ((jstring) env->CallObjectMethod (link, AndroidUri.toString)) : String());
        }
    ...
    void setupChannels (const Array<ChannelGroup>& groups, const Array<Channel>& channels)
    {
        ...
        for (const auto& c : channels)
        {
            ...
            env->CallVoidMethod (channel, NotificationChannel.setSound, HUrlToAndroidUri (c.soundToPlay).get(), audioAttributes.get());
    ...
    static bool intentActionContainsAnyOf (jobject intent, const StringArray& strings, bool includePackageName)
    {
        ...
        String packageName = includePackageName ? HString ((jstring) env->CallObjectMethod (context.get(),
                                                                                               AndroidContext.getPackageName))
                                                : String{};

        String intentAction = HString ((jstring) env->CallObjectMethod (intent, AndroidIntent.getAction));
    ...
    {
        return intentActionContainsAnyOf (intent, StringArray (".HNOTIFICATION_DELETED"), true);
    }

    static bool isLocalNotificationIntent (jobject intent)
    {
        return intentActionContainsAnyOf (intent, { ".HNOTIFICATION.",
                                                    ".HNOTIFICATION_BUTTON_ACTION.",
                                                    ".HNOTIFICATION_TEXT_INPUT_ACTION." },
                                          true);
    }
...
struct JuceFirebaseInstanceIdService
{
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     ...
     DECLARE_JNI_CLASS (InstanceIdService, "com/roli/H/JuceFirebaseInstanceIdService")
    #undef JNI_CLASS_MEMBERS

    static void JNICALL tokenRefreshed (void* token)
    {
        if (auto* instance = PushNotifications::getInstanceWithoutCreating())
            instance->pimpl->notifyListenersTokenRefreshed (HString (static_cast<jstring> (token)));
    }
};
...
struct JuceFirebaseMessagingService
{
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     ...
     DECLARE_JNI_CLASS (MessagingService, "com/roli/H/JuceFirebaseMessagingService")
    #undef JNI_CLASS_MEMBERS
...
//==============================================================================
bool handleNotificationIntent (void* intent)
{
...
struct JuceActivityNewIntentListener
{
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     ...
     DECLARE_JNI_CLASS (JavaActivity, HPUSH_NOTIFICATIONS_ACTIVITY)
    #undef JNI_CLASS_MEMBERS

    static void JNICALL appNewIntent (JNIEnv*, jobject /*activity*/, jobject intentData)
    {
       #if HPUSH_NOTIFICATIONS
        handleNotificationIntent(static_cast<void*>(intentData));
       #else
