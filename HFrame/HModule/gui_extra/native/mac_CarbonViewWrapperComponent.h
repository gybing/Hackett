...
public:
    ...
    ~CarbonViewWrapperComponent()
    {
        HAssert (embeddedView == 0); // must call deleteWindow() in the subclass's destructor!
    }
    ...
    void setEmbeddedWindowToOurSize()
    {
        if (! recursiveResize)
        {
            ...
            if (wrapperWindow != nil)
            {
                HAssert (getTopLevelComponent()->getDesktopScaleFactor() == 1.0f);
