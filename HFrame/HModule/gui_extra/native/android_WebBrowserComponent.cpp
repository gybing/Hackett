...
//==============================================================================
// This byte-code is generated from native/java/com/roli/H/JuceWebView.java with min sdk version 16
...
//==============================================================================
// This byte-code is generated from native/javacore/app/com/roli/H/JuceWebView21.java with min sdk version 21
// See core/native/java/README.txt on how to generate this byte-code.
...
public:
        ...
        : owner (o)
    {
        ...
        HWebChromeClient = GlobalRef (LocalRef<jobject> (env->NewObject (JuceWebChromeClient, JuceWebChromeClient.constructor,
                                                                            reinterpret_cast<jlong> (this))));
        env->CallVoidMethod ((jobject) getView(), AndroidWebView.setWebChromeClient, HWebChromeClient.get());

        auto sdkVersion = getAndroidSDKVersion();

        if (sdkVersion >= 21)
            HWebViewClient = GlobalRef (LocalRef<jobject> (env->NewObject (JuceWebViewClient21, JuceWebViewClient21.constructor,
                                                                              reinterpret_cast<jlong> (this))));
        else
            HWebViewClient = GlobalRef (LocalRef<jobject> (env->NewObject (JuceWebViewClient16, JuceWebViewClient16.constructor,
                                                                              reinterpret_cast<jlong> (this))));

        env->CallVoidMethod ((jobject) getView(), AndroidWebView.setWebViewClient, HWebViewClient.get());
    }
    ...
    ~Pimpl()
    {
        ...
        env->CallVoidMethod (HWebViewClient, getAndroidSDKVersion() >= 21 ? JuceWebViewClient21.hostDeleted
    ...
    {
        HAssert (connectionThread != nullptr);
...
private:
    ...
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     ...
     DECLARE_JNI_CLASS_WITH_BYTECODE (JuceWebViewClient21, "com/roli/H/JuceWebView21$Client", 21, JuceWebView21ByteCode, sizeof (JuceWebView21ByteCode))
    #undef JNI_CLASS_MEMBERS
    ...
    #define JNI_CLASS_MEMBERS(METHOD, STATICMETHOD, FIELD, STATICFIELD, CALLBACK) \
     ...
     DECLARE_JNI_CLASS_WITH_BYTECODE (JuceWebViewClient16, "com/roli/H/JuceWebView$Client", 16, JuceWebView16ByteCode, sizeof (JuceWebView16ByteCode))
    #undef JNI_CLASS_MEMBERS
    ...
    static jboolean JNICALL webViewPageLoadStarted (JNIEnv*, jobject /*activity*/, jlong host, jobject /*webView*/, jstring url)
    {
        if (auto* myself = reinterpret_cast<WebBrowserComponent::Pimpl*> (host))
            return myself->handlePageAboutToLoad (HString (url));
    ...
    {
        if (auto* myself = reinterpret_cast<WebBrowserComponent::Pimpl*> (host))
            myself->owner.pageFinishedLoading (HString (url));
    }
    ...
    static void JNICALL webViewReceivedSslError (JNIEnv*, jobject /*activity*/, jlong host, jobject /*webView*/, jobject /*sslErrorHandler*/, jobject sslError)
    {
        ...
        if (auto* myself = reinterpret_cast<WebBrowserComponent::Pimpl*> (host))
        {
            ...
            myself->owner.pageLoadHadNetworkError (HString (errorString));
        }
    }
    ...
    DECLARE_JNI_CLASS (JuceWebChromeClient, "com/roli/H/JuceWebView$ChromeClient")
    #undef JNI_CLASS_MEMBERS
    ...
    void webReceivedHttpError (jobject errorResponse)
    {
        ...
        if (responseClass != 0)
        {
            ...
            if (method != 0)
            {
                ...
                owner.pageLoadHadNetworkError (HString (errorString));
                return;
            }
        }
    ...
    //==============================================================================
    GlobalRef HWebChromeClient;
    GlobalRef HWebViewClient;
