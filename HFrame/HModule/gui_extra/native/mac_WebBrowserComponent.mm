...
{
    DownloadClickDetectorClass()  : ObjCClass<NSObject> ("HWebClickDetector_")
    {
    ...
    static void runOpenPanel (id, SEL, WebView*, id<WebOpenPanelResultListener> resultListener, BOOL allowMultipleFiles)
    {
                               ...
                               : chooser.browseForFileToOpen())
        {
            for (auto& f : chooser.getResults())
                [resultListener chooseFilename: HStringToNS (f.getFullPathName())];
        }
       #else
        ignoreUnused (resultListener, allowMultipleFiles);
        HAssertfalse; // Can't use this without modal loops being enabled!
       #endif
    }
};
...
{
    WebBrowserComponent* ownerComponent;
}

- (WebViewURLChangeDetector*) initWithWebBrowserOwner: (WebBrowserComponent*) ownerComponent;
...
- (WebViewURLChangeDetector*) initWithWebBrowserOwner: (WebBrowserComponent*) ownerComp
{
    [super init];
                  ...
                  const MemoryBlock* postData)
    {
        ...
        if (url.trimStart().startsWithIgnoreCase ("javascript:"))
        {
            [webView stringByEvaluatingJavaScriptFromString:
                HStringToNS (url.fromFirstOccurrenceOf (":", false, false))];
        }
        else
        {
            NSString* urlString = HStringToNS (url);

           #if (HMAC && (defined (MAC_OS_X_VERSION_10_9) && MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_9)) || (HIOS && (defined (__IPHONE_7_0) && __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_7_0))
            ...
            if (NSURL* nsURL = [NSURL URLWithString: urlString])
            {
                ...
                if (headers != nullptr)
                {
                    for (int i = 0; i < headers->size(); ++i)
                    {
                        ...
                        [r setValue: HStringToNS (headerValue)
                           forHTTPHeaderField: HStringToNS (headerName)];
                    }
                }
