...
namespace PushNotificationsDelegateDetails
{
    ...
    void* actionToNSAction (const Action& a, bool iOSEarlierThan10)
    {
        if (iOSEarlierThan10)
        {
            ...
            action.identifier     = HStringToNS (a.identifier);
            action.title          = HStringToNS (a.title);
        ...
        else
        {
            ...
            {
                return [UNTextInputNotificationAction actionWithIdentifier: HStringToNS (a.identifier)
                                                                     title: HStringToNS (a.title)
                                                                   options: NSUInteger (a.destructive << 1 | (! a.triggerInBackground) << 2)
                                                      textInputButtonTitle: HStringToNS (a.textInputButtonText)
                                                      textInputPlaceholder: HStringToNS (a.textInputPlaceholder)];
            }

            return [UNNotificationAction actionWithIdentifier: HStringToNS (a.identifier)
                                                        title: HStringToNS (a.title)
                                                      options: NSUInteger (a.destructive << 1 | (! a.triggerInBackground) << 2)];
           #else
            return nullptr;
    ...
    void* categoryToNSCategory (const Category& c, bool iOSEarlierThan10)
    {
        if (iOSEarlierThan10)
        {
            ...
            category.identifier = HStringToNS (c.identifier);
        ...
        else
        {
            ...
            return [UNNotificationCategory categoryWithIdentifier: HStringToNS (c.identifier)
                                                          actions: actions
                                                intentIdentifiers: @[]
    ...
    //==============================================================================
    UILocalNotification* HNotificationToUILocalNotification (const PushNotifications::Notification& n)
    {
        auto notification = [[UILocalNotification alloc] init];

        notification.alertTitle = HStringToNS (n.title);
        notification.alertBody  = HStringToNS (n.body);
        notification.category   = HStringToNS (n.category);
   ...
   #if defined (__IPHONE_10_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_10_0
    UNNotificationRequest* HNotificationToUNNotificationRequest (const PushNotifications::Notification& n)
    {
        // content
        auto content = [[UNMutableNotificationContent alloc] init];

        content.title              = HStringToNS (n.title);
        content.subtitle           = HStringToNS (n.subtitle);
        content.threadIdentifier   = HStringToNS (n.groupId);
        content.body               = HStringToNS (n.body);
        content.categoryIdentifier = HStringToNS (n.category);
        ...
        [propsDict setObject: HStringToNS (soundToPlayString) forKey: nsStringLiteral ("com.H.soundName")];
    ...
    PushNotifications::Notification unNotificationRequestToJuceNotification (UNNotificationRequest* r)
    {
        ...
        {
            static const Identifier soundName ("com.H.soundName");
...
struct PushNotifications::Pimpl : private PushNotificationsDelegate
{
    ...
    void sendLocalNotification (const Notification& n)
    {
        if (iOSEarlierThan10)
        {
            auto* notification = PushNotificationsDelegateDetails::HNotificationToUILocalNotification (n);
        ...
        else
        {
            ...
            UNNotificationRequest* request = PushNotificationsDelegateDetails::HNotificationToUNNotificationRequest (n);
    ...
    void removeDeliveredNotification (const String& identifier)
    {
        ...
        else
        {
            ...
            NSArray<NSString*>* identifiers = [NSArray arrayWithObject: HStringToNS (identifier)];
                                                              ...
                                                              void (^completionHandler)()) override
    {
        ...
        if (actionString == "com.apple.UNNotificationDefaultActionIdentifier")
            ...
            actionString = "com.H.NotificationDeleted";
