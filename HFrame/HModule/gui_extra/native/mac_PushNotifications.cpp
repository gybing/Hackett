...
namespace PushNotificationsDelegateDetailsOsx
{
    ...
    //==============================================================================
    NSUserNotification* HNotificationToNSUserNotification (const PushNotifications::Notification& n,
                                                              bool isEarlierThanMavericks,
                                                              bool isEarlierThanYosemite)
    {
        auto notification = [[NSUserNotification alloc] init];

        notification.title           = HStringToNS (n.title);
        notification.subtitle        = HStringToNS (n.subtitle);
        notification.informativeText = HStringToNS (n.body);
        ...
        if (n.actions.size() > 0)
            notification.actionButtonTitle = HStringToNS (n.actions.getReference (0).title);

        if (! isEarlierThanMavericks)
        {
            notification.identifier = HStringToNS (n.identifier);

            if (n.actions.size() > 0)
            {
                notification.hasReplyButton = n.actions.getReference (0).style == Action::text;
                notification.responsePlaceholder = HStringToNS (n.actions.getReference (0).textInputPlaceholder);
            }

            auto* imageDirectory = n.icon.contains ("/")
                                 ? HStringToNS (n.icon.upToLastOccurrenceOf ("/", false, true))
                                 : [NSString string];

            auto* imageName      = HStringToNS (n.icon.fromLastOccurrenceOf ("/", false, false)
                                                         .upToLastOccurrenceOf (".", false, false));
            auto* imageExtension = HStringToNS (n.icon.fromLastOccurrenceOf (".", false, false));
            ...
            if (! isEarlierThanYosemite)
            {
                if (n.actions.size() > 1)
                {
                    ...
                    for (int a = 1; a < n.actions.size(); ++a)
                        [additionalActions addObject: [NSUserNotificationAction actionWithIdentifier: HStringToNS (n.actions[a].identifier)
                                                                                               title: HStringToNS (n.actions[a].title)]];
...
struct PushNotifications::Pimpl : private PushNotificationsDelegate
{
    ...
    {
        auto* notification = PushNotificationsDelegateDetailsOsx::HNotificationToNSUserNotification (n, isEarlierThanMavericks, isEarlierThanYosemite);
    ...
    void removeDeliveredNotification (const String& identifier)
    {
        ...
        auto nsNotification = PushNotificationsDelegateDetailsOsx::HNotificationToNSUserNotification (n, isEarlierThanMavericks, isEarlierThanYosemite);
