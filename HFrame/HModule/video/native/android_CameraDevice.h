...
//
// native/java/com/roli/H/CameraCaptureSessionCaptureCallback.java
// native/java/com/roli/H/CameraCaptureSessionStateCallback.java
// native/java/com/roli/H/CameraDeviceStateCallback.java
// native/java/com/roli/H/JuceOrientationEventListener.java
//
// files with min sdk version 21
// See core/native/java/README.txt on how to generate this byte-code.
...
//==============================================================================
class AndroidRunnable  : public AndroidInterfaceImplementer
{
public:
    struct Owner
    {
    ...
    : private ActivityLifecycleCallbacks
{
    ...
    void open (InternalOpenCameraResultCallback cameraOpenCallbackToUse)
    {
        ...
        HAssert (cameraOpenCallback != nullptr);

        // The same camera can be opened only once!
        HAssert (scopedCameraDevice == nullptr);
...
private:
    ...
    static void printDebugCameraInfo (const LocalRef<jobject>& cameraManagerToUse, const String& cameraId)
    {
        ...
        HCAMERA_LOG ("Camera id: " + cameraId + ", characteristics keys num: " + String (size));
        ...
        for (int i = 0; i < size; ++i)
        {
            ...
            auto keyValueString = HString (jKeyValueString);
            ...
            else
            {
                ...
                {
                    HCAMERA_LOG ("Key: " + keyName + ", value: " + keyValueString);
                }
            }
                                             ...
                                             const String& keyValueString)
    {
        ...
        result << "]";
        HCAMERA_LOG ("Key: " + keyName + ", value: " + result);
    }
    ...
    private:
        ...
        static void printSizesLog (const Array<Rectangle<int>>& sizes, const String& className)
        {
            ...
            HCAMERA_LOG ("Sizes for class " + className);

          #if HCAMERA_LOG_ENABLED
            for (auto& s : sizes)
                HCAMERA_LOG (s.toString() + "\n");
          #endif
        }
    ...
    private:
        ...
        {
            HCAMERA_LOG ("onSurfaceTextureAvailable()");
        ...
        {
            HCAMERA_LOG ("onSurfaceTextureDestroyed()");
        ...
        {
            HCAMERA_LOG ("onSurfaceTextureSizeChanged()");
        ...
        {
            HCAMERA_LOG ("onSurfaceTextureUpdated()");
        }

        HDECLARE_NON_COPYABLE (PreviewDisplay)
    };
    ...
    private:
        ...
        HDECLARE_WEAK_REFERENCEABLE (ImageReader)

        void onImageAvailable (LocalRef<jobject>& /*imageReader*/) override
        {
            HCAMERA_LOG ("onImageAvailable()");
                                                             ...
                                                             int cameraSensorOrientation)
        {
            ...
            HAssert (env->GetArrayLength (planes) > 0);
    ...
    private:
        ...
        static jint HOrientationToNativeOrientation (int orientations) noexcept
        {
            enum
            {
        ...
        void onInfo (LocalRef<jobject>& recorder, int what, int extra) override
        {
            ...
            HCAMERA_LOG ("MediaRecorder::OnInfo: " + getInfoStringFromCode (what)
        ...
        void onError (LocalRef<jobject>& recorder, int what, int extra) override
        {
            ...
            HCAMERA_LOG ("MediaRecorder::onError: " + getErrorStringFromCode (what)
                                     + ", extra code = " + String (extra));
        }
    ...
    public:
        ...
        public:
            ...
            ~CaptureSession()
            {
                ...
                {
                    HCAMERA_LOG ("Exception or fatal error occurred while closing Capture Session, closing by force");
                }
                else if (calledClose)
                {
                    pendingClose.set (1);
        ...
        private:
            ...
            private:
                ...
                DECLARE_JNI_CLASS_WITH_BYTECODE (CameraCaptureSessionCaptureCallback, "com/roli/H/CameraCaptureSessionCaptureCallback", 21, CameraSupportByteCode, sizeof(CameraSupportByteCode))
                #undef JNI_CLASS_MEMBERS
                ...
                void lockFocus()
                {
                    ...
                    HCAMERA_LOG ("Performing auto-focus if possible...");
                ...
                void updateState (jobject captureResult)
                {
                    ...
                    switch (currentState)
                    {
                        ...
                        {
                            HCAMERA_LOG ("Still picture capture, updateState(), State::pendingFocusLock...");
                        ...
                        {
                            HCAMERA_LOG ("Still picture capture, updateState(), State::pendingExposurePrecapture...");
                        ...
                        {
                            HCAMERA_LOG ("Still picture capture, updateState(), State::pendingExposurePostPrecapture...");
                ...
                void captureStillPictureDelayed()
                {
                    ...
                    HCAMERA_LOG ("Still picture capture, device ready, capturing now...");
                ...
                {
                    HCAMERA_LOG ("cameraCaptureSessionCaptureCompleted()");
                ...
                {
                    HCAMERA_LOG ("cameraCaptureSessionCaptureFailed()");
                ...
                {
                    HCAMERA_LOG ("cameraCaptureSessionCaptureProgressed()");
                ...
                {
                    HCAMERA_LOG ("cameraCaptureSessionCaptureSequenceAborted()");
                ...
                {
                    HCAMERA_LOG ("cameraCaptureSessionCaptureSequenceCompleted()");
                ...
                {
                    HCAMERA_LOG ("cameraCaptureSessionCaptureStarted()");
            ...
            DECLARE_JNI_CLASS_WITH_MIN_SDK (CameraCaptureSessionStateCallback, "com/roli/H/CameraCaptureSessionStateCallback", 21)
            #undef JNI_CLASS_MEMBERS
            ...
            {
                HCAMERA_LOG ("cameraCaptureSessionActive()");
                ignoreUnused (session);
            }

            void cameraCaptureSessionClosed (jobject session)
            {
                HCAMERA_LOG ("cameraCaptureSessionClosed()");
                ignoreUnused (session);
            ...
            {
                HCAMERA_LOG ("cameraCaptureSessionConfigureFailed()");
                ignoreUnused (session);
            ...
            {
                HCAMERA_LOG ("cameraCaptureSessionConfigured()");
            ...
            {
                HCAMERA_LOG ("cameraCaptureSessionReady()");
                ignoreUnused (session);
            }
            ...
            HDECLARE_NON_COPYABLE (CaptureSession)
        };
    ...
    private:
        ...
        DECLARE_JNI_CLASS_WITH_MIN_SDK (CameraDeviceStateCallback, "com/roli/H/CameraDeviceStateCallback", 21)
        #undef JNI_CLASS_MEMBERS
        ...
        {
            HCAMERA_LOG ("cameraDeviceStateClosed()");
        ...
        {
            HCAMERA_LOG ("cameraDeviceStateDisconnected()");
        ...
        void cameraDeviceStateError (int errorCode)
        {
            ...
            HCAMERA_LOG ("cameraDeviceStateError(), error: " + error);
        ...
        {
            HCAMERA_LOG ("cameraDeviceStateOpened()");
    ...
    protected:
        ...
        {
            HAssert (previewDisplay.isReady());

            HCAMERA_LOG ("previewDisplayReady()");
            ...
            {
                HCAMERA_LOG ("Device error detected, not recreating a new camera session. The device needs to be reopened.");
                return;
            }
        ...
        {
            HCAMERA_LOG ("previewDisplayAboutToBeDestroyed()");
        ...
        void captureSessionConfigured (ScopedCameraDevice::CaptureSession* session) override
        {
            ...
            HAssert (session == captureSession.get());
        ...
        void startSession()
        {
            ...
            {
                HAssertfalse;
                HCAMERA_LOG ("Could not start capture session");
            }
        ...
        HDECLARE_WEAK_REFERENCEABLE (CaptureSessionMode<Mode>)
    };
    ...
    private:
        ...
        DECLARE_JNI_CLASS_WITH_MIN_SDK (OrientationEventListener, "com/roli/H/JuceOrientationEventListener", 21)
        #undef JNI_CLASS_MEMBERS
    ...
    {
        HCAMERA_LOG ("invokeCameraOpenCallback(), error = " + error);
    ...
    void startPreviewMode (ImageReader& ir)
    {
        ...
        HAssert (currentCaptureSessionMode.get() == nullptr);
    ...
    void startVideoRecordingMode (MediaRecorder& mr)
    {
        ...
        HAssert (scopedCameraDevice != nullptr && scopedCameraDevice->openedOk());
    ...
    {
        HCAMERA_LOG ("appPaused, closing camera...");
    ...
    void onActivityResumed (jobject) override
    {
        ...
        HCAMERA_LOG ("appResumed, opening camera...");
    ...
    void stopBackgroundThread()
    {
        ...
        HAssert(quitSafelyMethod != nullptr);
