...
struct CameraDevice::Pimpl
{
    ...
    static StringArray getAvailableDevices()
    {
        ...
        HCAMERA_LOG ("Available camera devices: ");

        for (AVCaptureDevice* device in getDevices())
        {
            HCAMERA_LOG ("Device start----------------------------------");
            printDebugCameraInfo (device);
            HCAMERA_LOG ("Device end----------------------------------");
...
private:
    ...
    static void printDebugCameraInfo (AVCaptureDevice* device)
    {
        ...
        HCAMERA_LOG ("Position: " + positionString);
        HCAMERA_LOG ("Model ID: " + nsStringToJuce (device.modelID));
        HCAMERA_LOG ("Localized name: " + nsStringToJuce (device.localizedName));
        HCAMERA_LOG ("Unique ID: " + nsStringToJuce (device.uniqueID));
        HCAMERA_LOG ("Lens aperture: " + String (device.lensAperture));

        HCAMERA_LOG ("Has flash: " + String ((int)device.hasFlash));
        HCAMERA_LOG ("Supports flash always on: " + String ((int)[device isFlashModeSupported: AVCaptureFlashModeOn]));
        HCAMERA_LOG ("Supports auto flash: " + String ((int)[device isFlashModeSupported: AVCaptureFlashModeAuto]));

        HCAMERA_LOG ("Has torch: " + String ((int)device.hasTorch));
        HCAMERA_LOG ("Supports torch always on: " + String ((int)[device isTorchModeSupported: AVCaptureTorchModeOn]));
        HCAMERA_LOG ("Supports auto torch: " + String ((int)[device isTorchModeSupported: AVCaptureTorchModeAuto]));

        HCAMERA_LOG ("Low light boost supported: " + String ((int)device.lowLightBoostEnabled));

        HCAMERA_LOG ("Supports auto white balance: " + String ((int)[device isWhiteBalanceModeSupported: AVCaptureWhiteBalanceModeAutoWhiteBalance]));
        HCAMERA_LOG ("Supports continuous auto white balance: " + String ((int)[device isWhiteBalanceModeSupported: AVCaptureWhiteBalanceModeContinuousAutoWhiteBalance]));

        HCAMERA_LOG ("Supports auto focus: " + String ((int)[device isFocusModeSupported: AVCaptureFocusModeAutoFocus]));
        HCAMERA_LOG ("Supports continuous auto focus: " + String ((int)[device isFocusModeSupported: AVCaptureFocusModeContinuousAutoFocus]));
        HCAMERA_LOG ("Supports point of interest focus: " + String ((int)device.focusPointOfInterestSupported));
        HCAMERA_LOG ("Smooth auto focus supported: " + String ((int)device.smoothAutoFocusSupported));
        HCAMERA_LOG ("Auto focus range restriction supported: " + String ((int)device.autoFocusRangeRestrictionSupported));

        HCAMERA_LOG ("Supports auto exposure: " + String ((int)[device isExposureModeSupported: AVCaptureExposureModeAutoExpose]));
        HCAMERA_LOG ("Supports continuous auto exposure: " + String ((int)[device isExposureModeSupported: AVCaptureExposureModeContinuousAutoExposure]));
        HCAMERA_LOG ("Supports custom exposure: " + String ((int)[device isExposureModeSupported: AVCaptureExposureModeCustom]));
        HCAMERA_LOG ("Supports point of interest exposure: " + String ((int)device.exposurePointOfInterestSupported));

       #if defined (__IPHONE_10_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_10_0
        if (iosVersion.major >= 10)
        {
            HCAMERA_LOG ("Device type: " + nsStringToJuce (device.deviceType));
            HCAMERA_LOG ("Locking focus with custom lens position supported: " + String ((int)device.lockingFocusWithCustomLensPositionSupported));
        }
       #endif
        ...
        {
            HCAMERA_LOG ("Min available video zoom factor: " + String (device.minAvailableVideoZoomFactor));
            HCAMERA_LOG ("Max available video zoom factor: " + String (device.maxAvailableVideoZoomFactor));
            HCAMERA_LOG ("Dual camera switch over video zoom factor: " + String (device.dualCameraSwitchOverVideoZoomFactor));
        }
       #endif

        HCAMERA_LOG ("Capture formats start-------------------");
        for (AVCaptureDeviceFormat* format in device.formats)
        {
            HCAMERA_LOG ("Capture format start------");
            printDebugCameraFormatInfo (format);
            HCAMERA_LOG ("Capture format end------");
        }
        HCAMERA_LOG ("Capture formats end-------------------");
    }

    static void printDebugCameraFormatInfo (AVCaptureDeviceFormat* format)
    {
        HCAMERA_LOG ("Media type: " + nsStringToJuce (format.mediaType));
        ...
        HCAMERA_LOG ("Supported colour spaces: " + colourSpaces);

        HCAMERA_LOG ("Video field of view: " + String (format.videoFieldOfView));
        HCAMERA_LOG ("Video max zoom factor: " + String (format.videoMaxZoomFactor));
        HCAMERA_LOG ("Video zoom factor upscale threshold: " + String (format.videoZoomFactorUpscaleThreshold));
        ...
        HCAMERA_LOG (videoFrameRateRangesString);

        HCAMERA_LOG ("Video binned: " + String (int(format.videoBinned)));

       #if defined (__IPHONE_8_0) && __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_8_0
        if (iosVersion.major >= 8)
        {
            HCAMERA_LOG ("Video HDR supported: " + String (int (format.videoHDRSupported)));
            HCAMERA_LOG ("High resolution still image dimensions: " + getHighResStillImgDimensionsString (format.highResolutionStillImageDimensions));
            HCAMERA_LOG ("Min ISO: " + String (format.minISO));
            HCAMERA_LOG ("Max ISO: " + String (format.maxISO));
            HCAMERA_LOG ("Min exposure duration: " + cmTimeToString (format.minExposureDuration));
            ...
            HCAMERA_LOG ("Auto focus system: " + autoFocusSystemString);

            HCAMERA_LOG ("Standard (iOS 5.0) video stabilization supported: " + String ((int) [format isVideoStabilizationModeSupported: AVCaptureVideoStabilizationModeStandard]));
            HCAMERA_LOG ("Cinematic video stabilization supported: " + String ((int) [format isVideoStabilizationModeSupported: AVCaptureVideoStabilizationModeCinematic]));
            HCAMERA_LOG ("Auto video stabilization supported: " + String ((int) [format isVideoStabilizationModeSupported: AVCaptureVideoStabilizationModeAuto]));
        }
       #endif
        ...
        if (iosVersion.major >= 11)
        {
            HCAMERA_LOG ("Min zoom factor for depth data delivery: " + String (format.videoMinZoomFactorForDepthDataDelivery));
            HCAMERA_LOG ("Max zoom factor for depth data delivery: " + String (format.videoMaxZoomFactorForDepthDataDelivery));
        }
       #endif
    }
    ...
    public:
        ...
        void startSessionForDeviceWithId (const String& cameraIdToUse)
        {
            dispatch_async (captureSessionQueue,^
                            {
                                cameraDevice = [AVCaptureDevice deviceWithUniqueID: HStringToNS (cameraIdToUse)];
        ...
        HDECLARE_WEAK_REFERENCEABLE (CaptureSession)
    ...
    private:
        ...
        private:
            ...
            static void printImageOutputDebugInfo (AVCaptureOutput* captureOutput)
            {
                ...
                if (Pimpl::getIOSVersion().major >= 10 && [captureOutput isKindOfClass: [AVCapturePhotoOutput class]])
                {
                    ...
                    HCAMERA_LOG ("Available image codec types: " + typesString);

                    HCAMERA_LOG ("Still image stabilization supported: " + String ((int) photoOutput.stillImageStabilizationSupported));
                    HCAMERA_LOG ("Dual camera fusion supported: " + String ((int) photoOutput.dualCameraFusionSupported));
                    HCAMERA_LOG ("Supports flash: "      + String ((int) [photoOutput.supportedFlashModes containsObject: @(AVCaptureFlashModeOn)]));
                    HCAMERA_LOG ("Supports auto flash: " + String ((int) [photoOutput.supportedFlashModes containsObject: @(AVCaptureFlashModeAuto)]));
                    HCAMERA_LOG ("Max bracketed photo count: " + String (photoOutput.maxBracketedCapturePhotoCount));
                    HCAMERA_LOG ("Lens stabilization during bracketed capture supported: " + String ((int) photoOutput.lensStabilizationDuringBracketedCaptureSupported));
                    HCAMERA_LOG ("Live photo capture supported: " + String ((int) photoOutput.livePhotoCaptureSupported));
                    ...
                    if (Pimpl::getIOSVersion().major >= 11)
                    {
                        ...
                        HCAMERA_LOG ("Available photo file types: " + typesString);
                        ...
                        HCAMERA_LOG ("Available RAW photo file types: " + typesString);
                        ...
                        HCAMERA_LOG ("Available live photo video codec types: " + typesString);

                        HCAMERA_LOG ("Dual camera dual photo delivery supported: " + String ((int) photoOutput.dualCameraDualPhotoDeliverySupported));
                        HCAMERA_LOG ("Camera calibration data delivery supported: " + String ((int) photoOutput.cameraCalibrationDataDeliverySupported));
                        HCAMERA_LOG ("Depth data delivery supported: " + String ((int) photoOutput.depthDataDeliverySupported));
                    }
                   #endif
                ...
                HCAMERA_LOG ("Available image codec types: " + typesString);
                HCAMERA_LOG ("Still image stabilization supported: " + String ((int) stillImageOutput.stillImageStabilizationSupported));
                HCAMERA_LOG ("Automatically enableds still image stabilization when available: " + String ((int) stillImageOutput.automaticallyEnablesStillImageStabilizationWhenAvailable));

                HCAMERA_LOG ("Output settings for image output: " + nsStringToJuce ([stillImageOutput.outputSettings description]));
            }
            ...
            private:
                ...
                static void didFinishCaptureForSettings (id, SEL, AVCapturePhotoOutput*, AVCaptureResolvedPhotoSettings*, NSError* error)
                {
                    ...
                    HCAMERA_LOG ("didFinishCaptureForSettings(), error = " + errorString);
                }
                ...
                static void didFinishProcessingPhoto (id self, SEL, AVCapturePhotoOutput*, AVCapturePhoto* capturePhoto, NSError* error)
                {
                    ...
                    HCAMERA_LOG ("didFinishProcessingPhoto(), error = " + errorString);
                                                                  ...
                                                                  NSError* error)
                {
                    ...
                    HCAMERA_LOG ("didFinishProcessingPhotoSampleBuffer(), error = " + errorString);
        ...
        public:
            ...
            void startRecording (const File& file, AVCaptureVideoOrientation orientationToUse)
            {
                ...
                auto url = [NSURL fileURLWithPath: HStringToNS (file.getFullPathName())
                                      isDirectory: NO];
        ...
        private:
            static void printVideoOutputDebugInfo (AVCaptureMovieFileOutput* output)
            {
                ...
                HCAMERA_LOG ("Available video codec types:");

               #if HCAMERA_LOG_ENABLED
                for (AVVideoCodecType type in output.availableVideoCodecTypes)
                    HCAMERA_LOG (nsStringToJuce (type));
               #endif

                HCAMERA_LOG ("Output settings per video connection:");

               #if HCAMERA_LOG_ENABLED
                for (AVCaptureConnection* connection in output.connections)
                    HCAMERA_LOG (nsStringToJuce ([[output outputSettingsForConnection: connection] description]));
               #endif
            }
            ...
            private:
                ...
                static void stopped (id self, SEL, AVCaptureFileOutput*, NSURL*, NSArray<AVCaptureConnection*>*, NSError* error)
                {
                    ...
                    HCAMERA_LOG ("Stopped recording, error = " + errorString);
    ...
    void cameraSessionStarted()
    {
        HCAMERA_LOG ("cameraSessionStarted()");
    ...
    void notifyPictureTaken (const Image& image)
    {
        HCAMERA_LOG ("notifyPictureTaken()");
