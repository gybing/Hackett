...
//
// native/java/com/roli/H/MediaControllerCallback.java
// native/java/com/roli/H/MediaSessionCallback.java
// native/java/com/roli/H/SystemVolumeObserver.java
//
    ...
    : public AndroidViewComponent, private ActivityLifecycleCallbacks, private SurfaceHolderCallback
{
    Pimpl (VideoComponent& ownerToUse, bool)
        : owner (ownerToUse),
          mediaSession (*this)
       #if HSYNC_VIDEO_VOLUME_WITH_OS_MEDIA_VOLUME
        , systemVolumeListener (*this)
       #endif
    {
        // Video requires SDK version 21 or higher
        HAssert (getAndroidSDKVersion() >= 21);
...
private:
    ...
    private:
        ...
        private:
            ...
            {
                HVIDEO_LOG ("MediaSessionController::playbackStateChanged()");
            ...
            DECLARE_JNI_CLASS_WITH_BYTECODE (AndroidMediaControllerCallback, "com/roli/H/MediaControllerCallback", 21, MediaSessionByteCode, sizeof(MediaSessionByteCode))
           #undef JNI_CLASS_MEMBERS
            ...
            static void audioInfoChanged (JNIEnv*, jobject, jlong host, jobject playbackInfo)
            {
                if (auto* myself = reinterpret_cast<VideoComponent::Pimpl::MediaSession::Controller*> (host))
                {
                    ...
                    HVIDEO_LOG ("MediaSessionController::audioInfoChanged()");
                }
            }
            ...
            static void metadataChanged (JNIEnv*, jobject, jlong host, jobject metadata)
            {
                if (auto* myself = reinterpret_cast<VideoComponent::Pimpl::MediaSession::Controller*> (host))
                {
                    ...
                    HVIDEO_LOG ("MediaSessionController::metadataChanged()");
                }
            }
            ...
            {
                if (auto* myself = reinterpret_cast<VideoComponent::Pimpl::MediaSession::Controller*> (host))
                    HVIDEO_LOG ("MediaSessionController::sessionDestroyed()");
            }
        };
        ...
        public:
            ...
            void load (const LocalRef<jstring>& mediaId, const LocalRef<jobject>& extras)
            {
                ...
                {
                    owner.errorOccurred ("Could not find video under path provided (" + HString (mediaId) + ")");
                    return;
                }
        ...
        private:
            ...
            {
                HVIDEO_LOG ("MediaPlayer::onPrepared()");
            ...
            {
                HVIDEO_LOG ("MediaPlayer::onSeekComplete()");
            ...
            {
                HVIDEO_LOG ("MediaPlayer::onCompletion()");
            ...
            {
                HVIDEO_LOG ("MediaPlayer::onInfo(), infoCode: " + String (what) + " (" + infoCodeToString (what) + ")"
        ...
        DECLARE_JNI_CLASS_WITH_MIN_SDK (AndroidMediaSessionCallback, "com/roli/H/MediaSessionCallback", 21)
        #undef JNI_CLASS_MEMBERS
        ...
        static void pauseCallback (JNIEnv*, jobject, jlong host)
        {
            ...
            {
                HVIDEO_LOG ("MediaSession::pauseCallback()");
                myself->player.pause();
        ...
        static void playCallback (JNIEnv*, jobject, jlong host)
        {
            if (auto* myself = reinterpret_cast<VideoComponent::Pimpl::MediaSession*> (host))
            {
                HVIDEO_LOG ("MediaSession::playCallback()");
        ...
        static void playFromMediaIdCallback (JNIEnv* env, jobject, jlong host, jstring mediaId, jobject extras)
        {
            if (auto* myself = reinterpret_cast<VideoComponent::Pimpl::MediaSession*> (host))
            {
                HVIDEO_LOG ("MediaSession::playFromMediaIdCallback()");
        ...
        static void seekToCallback (JNIEnv* /*env*/, jobject, jlong host, jlong pos)
        {
            if (auto* myself = reinterpret_cast<VideoComponent::Pimpl::MediaSession*> (host))
            {
                HVIDEO_LOG ("MediaSession::seekToCallback()");
        ...
        static void stopCallback(JNIEnv* env, jobject, jlong host)
        {
            if (auto* myself = reinterpret_cast<VideoComponent::Pimpl::MediaSession*> (host))
            {
                HVIDEO_LOG ("MediaSession::stopCallback()");
        ...
        void playerBufferingUpdated (int progress)
        {
            ...
            HVIDEO_LOG ("Buffering status update, seek pos: " + String (playPosPercent) + "%, buffered regions: " + ranges);
        }
        ...
        void onAudioFocusChange (int changeType) override
        {
            ...
            if (changeType == audioFocusGain)
                HVIDEO_LOG ("Audio focus gained");
            else
                HVIDEO_LOG ("Audio focus lost");

            if (changeType != audioFocusGain)
            {
                if (isPlaying())
                {
                    HVIDEO_LOG ("Received a request to abandon audio focus. Stopping playback...");
                    stop();
                }
    ...
    private:
        ...
        DECLARE_JNI_CLASS_WITH_MIN_SDK (SystemVolumeObserver, "com/roli/H/SystemVolumeObserver", 21)
        #undef JNI_CLASS_MEMBERS
    ...
    private:
        ...
        HDECLARE_WEAK_REFERENCEABLE (SystemVolumeListener)
    };
    ...
    void onActivityPaused (jobject) override
    {
        ...
        HVIDEO_LOG ("App paused, releasing media player...");
    ...
    void onActivityResumed (jobject) override
    {
        ...
        HVIDEO_LOG ("App resumed, restoring media player...");
