...
{
    HAssert (allChannels.contains (channelRange));
...
{
    HAssert (pitchbendRange >= 0 && pitchbendRange <= 96);
...
{
    if (notes.empty() || ! isUsingChannel (midiChannel))
        return;
...
const MPENote* MPEInstrument::getNotePtr (int midiChannel, TrackingMode mode) const noexcept
{
    ...
    HAssert (mode != allNotesOnChannel);
