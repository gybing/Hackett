...
{
    #define HINCREMENT_SRC_DEST         dest += (16 / sizeof (*dest)); src += (16 / sizeof (*dest));
    #define HINCREMENT_SRC1_SRC2_DEST   dest += (16 / sizeof (*dest)); src1 += (16 / sizeof (*dest)); src2 += (16 / sizeof (*dest));
    #define HINCREMENT_DEST             dest += (16 / sizeof (*dest));
    ...
    #define HBEGIN_VEC_OP \
        using Mode = FloatVectorHelpers::ModeType<sizeof(*dest)>::Mode; \
        { \
            const int numLongOps = num / Mode::numParallel;

    #define HFINISH_VEC_OP(normalOp) \
            num &= (Mode::numParallel - 1); \
            if (num == 0) return; \
        } \
        for (int i = 0; i < num; ++i) normalOp;

    #define HPERFORM_VEC_OP_DEST(normalOp, vecOp, locals, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        if (FloatVectorHelpers::isAligned (dest))   HVEC_LOOP (vecOp, dummy, Mode::loadA, Mode::storeA, locals, HINCREMENT_DEST) \
        else                                        HVEC_LOOP (vecOp, dummy, Mode::loadU, Mode::storeU, locals, HINCREMENT_DEST) \
        HFINISH_VEC_OP (normalOp)

    #define HPERFORM_VEC_OP_SRC_DEST(normalOp, vecOp, locals, increment, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        if (FloatVectorHelpers::isAligned (dest)) \
        { \
            if (FloatVectorHelpers::isAligned (src)) HVEC_LOOP (vecOp, Mode::loadA, Mode::loadA, Mode::storeA, locals, increment) \
            else                                     HVEC_LOOP (vecOp, Mode::loadU, Mode::loadA, Mode::storeA, locals, increment) \
        }\
        else \
        { \
            if (FloatVectorHelpers::isAligned (src)) HVEC_LOOP (vecOp, Mode::loadA, Mode::loadU, Mode::storeU, locals, increment) \
            else                                     HVEC_LOOP (vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
        } \
        HFINISH_VEC_OP (normalOp)

    #define HPERFORM_VEC_OP_SRC1_SRC2_DEST(normalOp, vecOp, locals, increment, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        if (FloatVectorHelpers::isAligned (dest)) \
        { \
            ...
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadA, Mode::loadA, Mode::storeA, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadA, Mode::loadU, Mode::storeA, locals, increment) \
            } \
            else \
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadU, Mode::loadA, Mode::storeA, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadU, Mode::loadU, Mode::storeA, locals, increment) \
            } \
        } \
        else \
        { \
            if (FloatVectorHelpers::isAligned (src1)) \
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadA, Mode::loadA, Mode::storeU, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadA, Mode::loadU, Mode::storeU, locals, increment) \
            } \
            else \
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadU, Mode::loadA, Mode::storeU, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
            } \
        } \
        HFINISH_VEC_OP (normalOp)

    #define HPERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals, increment, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        if (FloatVectorHelpers::isAligned (dest)) \
        { \
            if (FloatVectorHelpers::isAligned (src1)) \
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadA, Mode::loadA, Mode::loadA, Mode::storeA, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadA, Mode::loadU, Mode::loadA, Mode::storeA, locals, increment) \
            } \
            else \
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadU, Mode::loadA, Mode::loadA, Mode::storeA, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadU, Mode::loadU, Mode::loadA, Mode::storeA, locals, increment) \
            } \
        } \
        else \
        { \
            if (FloatVectorHelpers::isAligned (src1)) \
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadA, Mode::loadA, Mode::loadU, Mode::storeU, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadA, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
            } \
            else \
            { \
                if (FloatVectorHelpers::isAligned (src2))   HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadU, Mode::loadA, Mode::loadU, Mode::storeU, locals, increment) \
                else                                        HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
            } \
        } \
        HFINISH_VEC_OP (normalOp)
    ...
    #define HPERFORM_VEC_OP_DEST(normalOp, vecOp, locals, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        HVEC_LOOP (vecOp, dummy, Mode::loadU, Mode::storeU, locals, HINCREMENT_DEST) \
        HFINISH_VEC_OP (normalOp)

    #define HPERFORM_VEC_OP_SRC_DEST(normalOp, vecOp, locals, increment, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        HVEC_LOOP (vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
        HFINISH_VEC_OP (normalOp)

    #define HPERFORM_VEC_OP_SRC1_SRC2_DEST(normalOp, vecOp, locals, increment, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        HVEC_LOOP_TWO_SOURCES (vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
        HFINISH_VEC_OP (normalOp)

    #define HPERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals, increment, setupOp) \
        HBEGIN_VEC_OP \
        setupOp \
        HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD (vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
        HFINISH_VEC_OP (normalOp)
    ...
    //==============================================================================
    #define HVEC_LOOP(vecOp, srcLoad, dstLoad, dstStore, locals, increment) \
    ...
    #define HVEC_LOOP_TWO_SOURCES(vecOp, src1Load, src2Load, dstStore, locals, increment) \
        for (int i = 0; i < numLongOps; ++i) \
        { \
    ...
    #define HVEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(vecOp, src1Load, src2Load, dstLoad, dstStore, locals, increment) \
        for (int i = 0; i < numLongOps; ++i) \
        { \
    ...
    #define HLOAD_NONE(srcLoad, dstLoad)
    #define HLOAD_DEST(srcLoad, dstLoad)                        const Mode::ParallelType d = dstLoad (dest);
    #define HLOAD_SRC(srcLoad, dstLoad)                         const Mode::ParallelType s = srcLoad (src);
    #define HLOAD_SRC1_SRC2(src1Load, src2Load)                 const Mode::ParallelType s1 = src1Load (src1), s2 = src2Load (src2);
    #define HLOAD_SRC1_SRC2_DEST(src1Load, src2Load, dstLoad)   const Mode::ParallelType d = dstLoad (dest), s1 = src1Load (src1), s2 = src2Load (src2);
    #define HLOAD_SRC_DEST(srcLoad, dstLoad)                    const Mode::ParallelType d = dstLoad (dest), s = srcLoad (src);

    union signMask32 { float  f; uint32 i; };
    union signMask64 { double d; uint64 i; };

   #if HUSE_SSE_INTRINSICS || HUSE_ARM_NEON
    ...
    struct MinMax
    {
        ...
        static Type findMinOrMax (const Type* src, int num, const bool isMinimum) noexcept
        {
            ...
            return isMinimum ? findMinimum (src, num)
                             : findMaximum (src, num);
        }
...
//==============================================================================
void CALLTYPE FloatVectorOperations::clear (float* dest, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::clear (double* dest, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::fill (float* dest, float valueToFill, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vfill (&valueToFill, dest, 1, (size_t) num);
   #else
    HPERFORM_VEC_OP_DEST (dest[i] = valueToFill, val, HLOAD_NONE,
                              const Mode::ParallelType val = Mode::load1 (valueToFill);)
   #endif
}

void CALLTYPE FloatVectorOperations::fill (double* dest, double valueToFill, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::copy (float* dest, const float* src, int num) noexcept
{
    memcpy (dest, src, (size_t) num * sizeof (float));
}

void CALLTYPE FloatVectorOperations::copy (double* dest, const double* src, int num) noexcept
{
    memcpy (dest, src, (size_t) num * sizeof (double));
}

void CALLTYPE FloatVectorOperations::copyWithMultiply (float* dest, const float* src, float multiplier, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vsmul (src, 1, &multiplier, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = src[i] * multiplier, Mode::mul (mult, s),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType mult = Mode::load1 (multiplier);)
   #endif
}

void CALLTYPE FloatVectorOperations::copyWithMultiply (double* dest, const double* src, double multiplier, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::add (float* dest, float amount, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vsadd (dest, 1, &amount, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_DEST (dest[i] += amount, Mode::add (d, amountToAdd), HLOAD_DEST,
                              const Mode::ParallelType amountToAdd = Mode::load1 (amount);)
   #endif
}

void CALLTYPE FloatVectorOperations::add (double* dest, double amount, int num) noexcept
{
    HPERFORM_VEC_OP_DEST (dest[i] += amount, Mode::add (d, amountToAdd), HLOAD_DEST,
                              const Mode::ParallelType amountToAdd = Mode::load1 (amount);)
}

void CALLTYPE FloatVectorOperations::add (float* dest, const float* src, float amount, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vsadd (osx108sdkCompatibilityCast (src), 1, &amount, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = src[i] + amount, Mode::add (am, s),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType am = Mode::load1 (amount);)
   #endif
}

void CALLTYPE FloatVectorOperations::add (double* dest, const double* src, double amount, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::add (float* dest, const float* src, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vadd (src, 1, dest, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] += src[i], Mode::add (d, s), HLOAD_SRC_DEST, HINCREMENT_SRC_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::add (double* dest, const double* src, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::add (float* dest, const float* src1, const float* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vadd (src1, 1, src2, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC1_SRC2_DEST (dest[i] = src1[i] + src2[i], Mode::add (s1, s2), HLOAD_SRC1_SRC2, HINCREMENT_SRC1_SRC2_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::add (double* dest, const double* src1, const double* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::subtract (float* dest, const float* src, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vsub (src, 1, dest, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] -= src[i], Mode::sub (d, s), HLOAD_SRC_DEST, HINCREMENT_SRC_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::subtract (double* dest, const double* src, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::subtract (float* dest, const float* src1, const float* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vsub (src2, 1, src1, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC1_SRC2_DEST (dest[i] = src1[i] - src2[i], Mode::sub (s1, s2), HLOAD_SRC1_SRC2, HINCREMENT_SRC1_SRC2_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::subtract (double* dest, const double* src1, const double* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::addWithMultiply (float* dest, const float* src, float multiplier, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vsma (src, 1, &multiplier, dest, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] += src[i] * multiplier, Mode::add (d, Mode::mul (mult, s)),
                                  HLOAD_SRC_DEST, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType mult = Mode::load1 (multiplier);)
   #endif
}

void CALLTYPE FloatVectorOperations::addWithMultiply (double* dest, const double* src, double multiplier, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::addWithMultiply (float* dest, const float* src1, const float* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vma ((float*) src1, 1, (float*) src2, 1, dest, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC1_SRC2_DEST_DEST (dest[i] += src1[i] * src2[i], Mode::add (d, Mode::mul (s1, s2)),
                                             HLOAD_SRC1_SRC2_DEST,
                                             HINCREMENT_SRC1_SRC2_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::addWithMultiply (double* dest, const double* src1, const double* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::subtractWithMultiply (float* dest, const float* src, float multiplier, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] -= src[i] * multiplier, Mode::sub (d, Mode::mul (mult, s)),
                                  HLOAD_SRC_DEST, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType mult = Mode::load1 (multiplier);)
}

void CALLTYPE FloatVectorOperations::subtractWithMultiply (double* dest, const double* src, double multiplier, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] -= src[i] * multiplier, Mode::sub (d, Mode::mul (mult, s)),
                                  HLOAD_SRC_DEST, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType mult = Mode::load1 (multiplier);)
...
void CALLTYPE FloatVectorOperations::subtractWithMultiply (float* dest, const float* src1, const float* src2, int num) noexcept
{
    HPERFORM_VEC_OP_SRC1_SRC2_DEST_DEST (dest[i] -= src1[i] * src2[i], Mode::sub (d, Mode::mul (s1, s2)),
                                             HLOAD_SRC1_SRC2_DEST,
                                             HINCREMENT_SRC1_SRC2_DEST, )
}

void CALLTYPE FloatVectorOperations::subtractWithMultiply (double* dest, const double* src1, const double* src2, int num) noexcept
{
    HPERFORM_VEC_OP_SRC1_SRC2_DEST_DEST (dest[i] -= src1[i] * src2[i], Mode::sub (d, Mode::mul (s1, s2)),
                                             HLOAD_SRC1_SRC2_DEST,
                                             HINCREMENT_SRC1_SRC2_DEST, )
...
void CALLTYPE FloatVectorOperations::multiply (float* dest, const float* src, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vmul (src, 1, dest, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] *= src[i], Mode::mul (d, s), HLOAD_SRC_DEST, HINCREMENT_SRC_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::multiply (double* dest, const double* src, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::multiply (float* dest, const float* src1, const float* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vmul (src1, 1, src2, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC1_SRC2_DEST (dest[i] = src1[i] * src2[i], Mode::mul (s1, s2), HLOAD_SRC1_SRC2, HINCREMENT_SRC1_SRC2_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::multiply (double* dest, const double* src1, const double* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::multiply (float* dest, float multiplier, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vsmul (dest, 1, &multiplier, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_DEST (dest[i] *= multiplier, Mode::mul (d, mult), HLOAD_DEST,
                              const Mode::ParallelType mult = Mode::load1 (multiplier);)
   #endif
}

void CALLTYPE FloatVectorOperations::multiply (double* dest, double multiplier, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::multiply (float* dest, const float* src, float multiplier, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = src[i] * multiplier, Mode::mul (mult, s),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType mult = Mode::load1 (multiplier);)
...
void CALLTYPE FloatVectorOperations::multiply (double* dest, const double* src, double multiplier, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = src[i] * multiplier, Mode::mul (mult, s),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType mult = Mode::load1 (multiplier);)
...
void FloatVectorOperations::abs (float* dest, const float* src, int num) noexcept
{
    ...
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = std::abs (src[i]), Mode::bit_and (s, mask),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
...
void CALLTYPE FloatVectorOperations::convertFixedToFloat (float* dest, const int* src, float multiplier, int num) noexcept
{
   #if HUSE_ARM_NEON
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = src[i] * multiplier,
                                  vmulq_n_f32 (vcvtq_f32_s32 (vld1q_s32 (src)), multiplier),
                                  HLOAD_NONE, HINCREMENT_SRC_DEST, )
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = (float) src[i] * multiplier,
...
void CALLTYPE FloatVectorOperations::min (float* dest, const float* src, float comp, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = jmin (src[i], comp), Mode::min (s, cmp),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType cmp = Mode::load1 (comp);)
}

void CALLTYPE FloatVectorOperations::min (double* dest, const double* src, double comp, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = jmin (src[i], comp), Mode::min (s, cmp),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType cmp = Mode::load1 (comp);)
...
void CALLTYPE FloatVectorOperations::min (float* dest, const float* src1, const float* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vmin ((float*) src1, 1, (float*) src2, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC1_SRC2_DEST (dest[i] = jmin (src1[i], src2[i]), Mode::min (s1, s2), HLOAD_SRC1_SRC2, HINCREMENT_SRC1_SRC2_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::min (double* dest, const double* src1, const double* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::max (float* dest, const float* src, float comp, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = jmax (src[i], comp), Mode::max (s, cmp),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType cmp = Mode::load1 (comp);)
}

void CALLTYPE FloatVectorOperations::max (double* dest, const double* src, double comp, int num) noexcept
{
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = jmax (src[i], comp), Mode::max (s, cmp),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType cmp = Mode::load1 (comp);)
...
void CALLTYPE FloatVectorOperations::max (float* dest, const float* src1, const float* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
    vDSP_vmax ((float*) src1, 1, (float*) src2, 1, dest, 1, (vDSP_Length) num);
   #else
    HPERFORM_VEC_OP_SRC1_SRC2_DEST (dest[i] = jmax (src1[i], src2[i]), Mode::max (s1, s2), HLOAD_SRC1_SRC2, HINCREMENT_SRC1_SRC2_DEST, )
   #endif
}

void CALLTYPE FloatVectorOperations::max (double* dest, const double* src1, const double* src2, int num) noexcept
{
   #if HUSE_VDSP_FRAMEWORK
...
void CALLTYPE FloatVectorOperations::clip (float* dest, const float* src, float low, float high, int num) noexcept
{
    HAssert(high >= low);
   ...
   #else
    HPERFORM_VEC_OP_SRC_DEST (dest[i] = jmax (jmin (src[i], high), low), Mode::max (Mode::min (s, hi), lo),
                                  HLOAD_SRC, HINCREMENT_SRC_DEST,
                                  const Mode::ParallelType lo = Mode::load1 (low); const Mode::ParallelType hi = Mode::load1 (high);)
   #endif
}

void CALLTYPE FloatVectorOperations::clip (double* dest, const double* src, double low, double high, int num) noexcept
{
    HAssert(high >= low);
...
Range<float> CALLTYPE FloatVectorOperations::findMinAndMax (const float* src, int num) noexcept
{
   #if HUSE_SSE_INTRINSICS || HUSE_ARM_NEON
...
Range<double> CALLTYPE FloatVectorOperations::findMinAndMax (const double* src, int num) noexcept
{
   #if HUSE_SSE_INTRINSICS || HUSE_ARM_NEON
...
float CALLTYPE FloatVectorOperations::findMinimum (const float* src, int num) noexcept
{
   #if HUSE_SSE_INTRINSICS || HUSE_ARM_NEON
    return FloatVectorHelpers::MinMax<FloatVectorHelpers::BasicOps32>::findMinOrMax (src, num, true);
   #else
    return findMinimum (src, num);
   #endif
}

double CALLTYPE FloatVectorOperations::findMinimum (const double* src, int num) noexcept
{
   #if HUSE_SSE_INTRINSICS || HUSE_ARM_NEON
...
float CALLTYPE FloatVectorOperations::findMaximum (const float* src, int num) noexcept
{
   #if HUSE_SSE_INTRINSICS || HUSE_ARM_NEON
    return FloatVectorHelpers::MinMax<FloatVectorHelpers::BasicOps32>::findMinOrMax (src, num, false);
   #else
    return findMaximum (src, num);
   #endif
}

double CALLTYPE FloatVectorOperations::findMaximum (const double* src, int num) noexcept
{
   #if HUSE_SSE_INTRINSICS || HUSE_ARM_NEON
...
intptr_t CALLTYPE FloatVectorOperations::getFpStatusRegister() noexcept
{
    intptr_t fpsr = 0;
  #if HINTEL && HUSE_SSE_INTRINSICS
    fpsr = static_cast<intptr_t> (_mm_getcsr());
  #elif defined (__arm64__) || defined (__aarch64__) || HUSE_ARM_NEON
   ...
   #endif
  #else
   #if ! (defined (HINTEL) || defined (HARM))
    HAssertfalse; // No support for getting the floating point status register for your platform
   #endif
  #endif
...
void CALLTYPE FloatVectorOperations::setFpStatusRegister (intptr_t fpsr) noexcept
{
  #if HINTEL && HUSE_SSE_INTRINSICS
...
void CALLTYPE FloatVectorOperations::enableFlushToZeroMode (bool shouldEnable) noexcept
{
  ...
  #else
   #if ! (defined (HINTEL) || defined (HARM))
    HAssertfalse; // No support for flush to zero mode on your platform
   #endif
    ignoreUnused (shouldEnable);
  #endif
}

void CALLTYPE FloatVectorOperations::disableDenormalisedNumberSupport (bool shouldDisable) noexcept
{
  #if HUSE_SSE_INTRINSICS || (HUSE_ARM_NEON || defined (__arm64__) || defined (__aarch64__))
   #if HUSE_SSE_INTRINSICS
   ...
   #if ! (defined (HINTEL) || defined (HARM))
    HAssertfalse; // No support for disable denormals mode on your platform
   #endif
  #endif
}

bool CALLTYPE FloatVectorOperations::areDenormalsDisabled() noexcept
{
  #if HUSE_SSE_INTRINSICS || (HUSE_ARM_NEON || defined (__arm64__) || defined (__aarch64__))
   #if HUSE_SSE_INTRINSICS
    intptr_t mask = 0x8040;
...
public:
    ...
    struct TestRunner
    {
        static void runTest (UnitTest& u, Random random)
        {
            ...
            u.expect (valuesMatch (FloatVectorOperations::findMinimum (data1, num), findMinimum (data1, num)));
            u.expect (valuesMatch (FloatVectorOperations::findMaximum (data1, num), findMaximum (data1, num)));

            u.expect (valuesMatch (FloatVectorOperations::findMinimum (data2, num), findMinimum (data2, num)));
            u.expect (valuesMatch (FloatVectorOperations::findMaximum (data2, num), findMaximum (data2, num)));
