...
{
    HAssert (voice != nullptr);

    voice->stopNote (velocity, allowTailOff);

    // the subclass MUST call clearCurrentNote() if it's not tailing off! RTFM for stopNote()!
    HAssert (allowTailOff || (voice->getCurrentlyPlayingNote() < 0 && voice->getCurrentlyPlayingSound() == nullptr));
}
    ...
    for (auto* voice : voices)
    {
              ...
              && voice->isPlayingChannel (midiChannel))
        {
            if (auto sound = voice->getCurrentlyPlayingSound())
            {
                ...
                {
                    HAssert (! voice->keyIsDown || voice->isSustainPedalDown() == sustainPedalsDown [midiChannel]);
                                                 ...
                                                 int /*midiChannel*/, int midiNoteNumber) const
{
    ...
    for (auto* voice : voices)
    {
        ...
        {
            HAssert (voice->isVoiceActive()); // We wouldn't be here otherwise
