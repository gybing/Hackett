                                  ...
                                  private ServiceConnection
{
                          ...
                          const StringArray& subscriptionIdentifiers, bool creditForUnusedSubscription)
    {
        ...
        {
            // Item already bought.
            notifyAboutPurchaseResult ({ {}, productIdentifier, HString (getPackageName()), {}, {} }, true, statusCodeToUserString (responseCode));
        }
    }

    void restoreProductsBoughtList (bool, const String&)
    {
    ...
    void onServiceConnected (jobject, jobject iBinder) override
    {
        ...
        HAssert (isInAppPurchasesSupported());
    }
                    ...
                    if (responseList != 0)
                    {
                        ...
                        if (iterator.get() != 0)
                        {
                            for (;;)
                            {
                                ...
                                auto response = LocalRef<jstring> ((jstring)env->CallObjectMethod (iterator, JavaIterator.next));

                                if (response.get() != 0)
                                {
                                    var responseData = JSON::parse (HString (response.get()));
        ...
        Array<InAppPurchases::Purchase> getProductsBought (const String& productType, jstring continuationToken)
        {
            ...
            if (ownedItems.get() != 0)
            {
                ...
                if (responseCode == 0)
                {
                    ...
                    {
                        auto sku          = HString ((jstring) (env->CallObjectMethod (ownedSkus.get(),        JavaArrayList.get, i)));
                        auto purchaseData = HString ((jstring) (env->CallObjectMethod (purchaseDataList.get(), JavaArrayList.get, i)));
                        auto signature    = HString ((jstring) (env->CallObjectMethod (signatureList.get(),    JavaArrayList.get, i)));
    ...
    void inAppPurchaseCompleted (jobject intentData)
    {
        ...
        auto purchaseDataString  = pd.get()  != 0 ? HString (pd.get())  : String();
        auto dataSignatureString = sig.get() != 0 ? HString (sig.get()) : String();
    ...
    static String statusCodeToUserString (int statusCode)
    {
        switch (statusCode)
        {
            ...
            default: HAssertfalse; return NEEDS_TRANS ("Unknown status");
        }
    }
...
//==============================================================================
void inAppPurchaseCompleted (void* intentData)
{
