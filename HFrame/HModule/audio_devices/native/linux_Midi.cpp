...
public:
    ...
    {
        #ifdef HALSA_MIDI_NAME
         return HALSA_MIDI_NAME;
        #else
    ...
    struct Port
    {
        ...
        {
            HAssert (cb != nullptr && input != nullptr);
            callback = cb;
            midiInput = input;
        }
...
private:
    ...
    public:
        MidiInputThread (AlsaClient& c)
            : Thread ("H MIDI Input"), client (c)
        {
            HAssert (client.get() != nullptr);
        }
                                             ...
                                             const String& deviceIdentifierToOpen)
{
    ...
    if (auto seqHandle = client->get())
    {
        ...
        HAssert (systemInfo != nullptr);

        if (snd_seq_system_info (seqHandle, systemInfo) == 0)
        {
            snd_seq_client_info_alloca (&clientInfo);
            HAssert (clientInfo != nullptr);
