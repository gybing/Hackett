...
*/
#define HASIOCALLBACK __cdecl

//==============================================================================
namespace ASIODebugging
{
   #if HASIO_DEBUGGING
    #define HASIO_LOG(msg)               ASIODebugging::logMessage (msg)
    #define HASIO_LOG_ERROR(msg, errNum) ASIODebugging::logError ((msg), (errNum))
    ...
    #define HASIO_LOG(msg)               ASIODebugging::dummyLog()
    #define HASIO_LOG_ERROR(msg, errNum) ignoreUnused (errNum); ASIODebugging::dummyLog()
   #endif
}
...
struct ASIOSampleFormat
{
    ...
    ASIOSampleFormat (long type) noexcept
    {
        switch (type)
        {
            ...
            default:
                HAssertfalse;  // (not a valid format code..)
                break;
        }
    }
...
public:
         ...
         classId (clsID)
    {
        ...
        HAssert (currentASIODev[slotNumber] == nullptr);
    ...
    void updateSampleRates()
    {
        ...
        if (sampleRates != newRates)
        {
            ...
            HASIO_LOG ("Rates: " + s.joinIntoString (" "));
           #endif
        }
    }
                 ...
                 double sr, int bufferSizeSamples) override
    {
        ...
        HAssert (currentCallback == nullptr);
        ...
        HASIO_LOG ("disposing buffers");
        err = asioObject->disposeBuffers();

        HASIO_LOG ("creating buffers: " + String (totalBuffers) + ", " + String (currentBlockSizeSamples));
        ...
        if (err != ASE_OK)
        {
            ...
            HASIO_LOG_ERROR ("create buffers 2", err);
        ...
        if (err == ASE_OK)
        {
            ...
            HAssert (numActiveOutputChans == n);

            for (int i = types.size(); --i >= 0;)
                HASIO_LOG ("channel format: " + String (types[i]));

            HAssert (n <= totalBuffers);
            ...
            else
            {
                ...
                if (! calledback)
                {
                    ...
                    HASIO_LOG ("no callbacks - stopping..");
                    asioObject->stop();
                }
            }
        }
    ...
    {
        HASIO_LOG ("showing control panel");
...
private:
    ...
    int readBufferSizes (int bufferSizeSamples)
    {
        ...
        {
            HASIO_LOG ("Using preferred size for buffer..");
            ...
            else
            {
                ...
                HASIO_LOG_ERROR ("getBufferSize1", err);
            }
    ...
    void addBufferSizes (long minSize, long maxSize, long preferredSize, long granularity)
    {
        ...
        HASIO_LOG (String ((int) minSize) + "->" + String ((int) maxSize) + ", "
    ...
    double getSampleRate() const
    {
        ...
        HASIO_LOG_ERROR ("getSampleRate", err);
        return cr;
    }
    ...
    void setSampleRate (double newRate)
    {
        ...
        {
            HASIO_LOG ("rate change: " + String (currentSampleRate) + " to " + String (newRate));
            auto err = asioObject->setSampleRate (newRate);

            if (err == ASE_NoClock && numClockSources > 0)
            {
                HASIO_LOG ("trying to set a clock source..");
                Thread::sleep (10);
                err = asioObject->setClockSource (clocks[0].index);
                HASIO_LOG_ERROR ("setClockSource2", err);
                Thread::sleep (10);
    ...
    void updateClockSources()
    {
        ...
        {
            HASIO_LOG ("setting clock source");
            auto err = asioObject->setClockSource (clocks[0].index);
            HASIO_LOG_ERROR ("setClockSource1", err);
            Thread::sleep (20);
        }
        else
        {
            if (numClockSources == 0)
                HASIO_LOG ("no clock sources!");
        }
    }
    ...
    void readLatencies()
    {
        ...
        if (asioObject->getLatencies (&inputLatency, &outputLatency) != 0)
            HASIO_LOG ("getLatencies() failed");
        else
            HASIO_LOG ("Latencies: in = " + String ((int) inputLatency) + ", out = " + String ((int) outputLatency));
    }
    ...
    void createDummyBuffers (long preferredSize)
    {
        ...
        HASIO_LOG ("creating buffers (dummy): " + String (numChans) + ", " + String ((int) preferredSize));

        if (preferredSize > 0)
        {
            auto err = asioObject->createBuffers (bufferInfos, numChans, preferredSize, &callbacks);
            HASIO_LOG_ERROR ("dummy buffers", err);
        }
        ...
        if (totalNumInputChans != newInps || totalNumOutputChans != newOuts)
        {
            ...
            HASIO_LOG (String ((int) totalNumInputChans) + " in; " + String ((int) totalNumOutputChans) + " out");
        }
    ...
    bool loadDriver()
    {
        ...
        if (crashed)
            HASIO_LOG ("** Driver crashed while being opened");
    ...
    String initDriver()
    {
        ...
        const bool initOk = !! asioObject->init (messageWindowHandle);
        String driverError;
    ...
    String openDevice()
    {
        ...
        HASIO_LOG ("opening device: " + getName());
            ...
            if ((error = initDriver()).empty())
            {
                     ...
                     && (err = asioObject->getChannels (&totalNumInputChans, &totalNumOutputChans)) == 0)
                {
                    ...
                    if ((err = refreshBufferSizes()) == 0)
                    {
                        ...
                        {
                            HASIO_LOG ("setting default sample rate");
                            err = asioObject->setSampleRate (44100.0);
                            HASIO_LOG_ERROR ("setting sample rate", err);
    ...
    //==============================================================================
    void HASIOCALLBACK callback (long index)
    {
        if (isStarted)
        {
            processBuffer (index);
    ...
    long asioMessagesCallback (long selector, long value)
    {
        switch (selector)
        {
            ...
            case kAsioBufferSizeChange:  HASIO_LOG ("kAsioBufferSizeChange"); resetRequest(); return 1;
            case kAsioResetRequest:      HASIO_LOG ("kAsioResetRequest");     resetRequest(); return 1;
            case kAsioResyncRequest:     HASIO_LOG ("kAsioResyncRequest");    resetRequest(); return 1;
            case kAsioLatenciesChanged:  HASIO_LOG ("kAsioLatenciesChanged"); return 1;
    ...
    {
        static ASIOTime* HASIOCALLBACK bufferSwitchTimeInfoCallback (ASIOTime*, long index, long)
        {
        ...
        static void HASIOCALLBACK bufferSwitchCallback (long index, long)
        {
            if (auto* d = currentASIODev[deviceIndex])
                d->callback (index);
        }

        static long HASIOCALLBACK asioMessagesCallback (long selector, long value, void*, double*)
        {
            if (auto* d = currentASIODev[deviceIndex])
        ...
        static void HASIOCALLBACK sampleRateChangedCallback (ASIOSampleRate)
        {
            if (auto* d = currentASIODev[deviceIndex])
                d->resetRequest();
        }
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ASIOAudioIODevice)
};
...
public:
    ...
    static int findFreeSlot()
    {
        ...
        HAssertfalse;  // unfortunately you can only have a finite number
    ...
    HDECLARE_WEAK_REFERENCEABLE (ASIOAudioIODeviceType)
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ASIOAudioIODeviceType)
};
