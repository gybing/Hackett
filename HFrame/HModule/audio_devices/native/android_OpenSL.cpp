...
struct BufferHelpers<float>
{
    ...
    static void convertFromOpenSL (const float* srcInterleaved, AudioBuffer<float>& audioBuffer)
    {
        if (audioBuffer.getNumChannels() == 1)
        {
            HAssert (srcInterleaved == audioBuffer.getWritePointer (0));
            return;
        }
    ...
    static void convertToOpenSL (const AudioBuffer<float>& audioBuffer, float* dstInterleaved)
    {
        if (audioBuffer.getNumChannels() == 1)
        {
            HAssert (dstInterleaved == audioBuffer.getReadPointer (0));
            return;
        }
...
public:
    ...
    public:
        ...
        {
            HAssert (numInputChannels > 0 || numOutputChannels > 0);
        ...
        virtual void start()             { stop(); HAssert (callback.get() != nullptr); running = true; }
                 ...
                 int bufferSize) override
    {
        ...
        HAssert ((actualBufferSize * audioBuffersToEnqueue) == totalPreferredBufferSize);
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OpenSLAudioIODevice)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OpenSLAudioDeviceType)
};
