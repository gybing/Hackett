...
struct MidiServiceType
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiServiceType)
};
...
private:
    ...
    private:
        ...
        struct MidiHeader
        {
            ...
            HDECLARE_NON_COPYABLE (MidiHeader)
        };
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiInCollector)
    };
                                ...
                                public Win32MidiDeviceQuery<Win32InputWrapper>
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Win32InputWrapper)
    };
    ...
    struct MidiOutHandle    : public ReferenceCountedObject
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiOutHandle)
    };
                                 ...
                                 public Win32MidiDeviceQuery<Win32OutputWrapper>
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Win32OutputWrapper)
    };
...
#ifndef HFORCE_WINRT_MIDI
 #define HFORCE_WINRT_MIDI 0
#endif

#ifndef HWINRT_MIDI_LOGGING
 #define HWINRT_MIDI_LOGGING 0
#endif

#if HWINRT_MIDI_LOGGING
 #define HWINRT_MIDI_LOG(x)  DBG(x)
#else
 #define HWINRT_MIDI_LOG(x)
#endif
...
private:
    ...
    struct BLEDeviceWatcher final   : private DeviceCallbackHandler
    {
        ...
        HRESULT addDevice (IDeviceInformation* addedDeviceInfo) override
        {
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to query added BLE device ID!");
                return S_OK;
            }

            auto deviceID = WinRTWrapper::getInstance()->hStringToString (deviceIDHst);
            HWINRT_MIDI_LOG ("Detected paired BLE device: " << deviceID);
            ...
            if (auto* containerIDValue = getValueFromDeviceInfo ("System.Devices.Aep.ContainerId", addedDeviceInfo))
            {
                ...
                if (containerID.!empty())
                {
                    ...
                    HWINRT_MIDI_LOG ("Adding BLE device: " << deviceID << " " << info.containerID
            ...
            HWINRT_MIDI_LOG ("Failed to get a container ID for BLE device: " << deviceID);
            return S_OK;
        }
        ...
        HRESULT removeDevice (IDeviceInformationUpdate* removedDeviceInfo) override
        {
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to query removed BLE device ID!");
                return S_OK;
            }

            auto removedDeviceId = WinRTWrapper::getInstance()->hStringToString (removedDeviceIdHstr);

            HWINRT_MIDI_LOG ("Removing BLE device: " << removedDeviceId);

            {
                ...
                if (devices.contains (removedDeviceId))
                {
                    ...
                    HWINRT_MIDI_LOG ("Removed BLE device: " << removedDeviceId);
                }
            }
        ...
        HRESULT updateDevice (IDeviceInformationUpdate* updatedDeviceInfo) override
        {
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to query updated BLE device ID!");
                return S_OK;
            }

            auto updatedDeviceId = WinRTWrapper::getInstance()->hStringToString (updatedDeviceIdHstr);

            HWINRT_MIDI_LOG ("Updating BLE device: " << updatedDeviceId);
            ...
            if (auto* connectedValue = getValueFromDeviceInfo ("System.Devices.Aep.IsConnected", updatedDeviceInfo))
            {
...

                {
                    ...
                    {
                        HWINRT_MIDI_LOG ("BLE device connection status change: " << updatedDeviceId << " " << info.containerID << " " << (isConnected ? "connected" : "disconnected"));
                        listeners.call ([&info](Listener& l) { l.bleDeviceDisconnected (info.containerID); });
                    }
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BLEDeviceWatcher);
    };
    ...
    struct MidiIODeviceWatcher final   : private DeviceCallbackHandler
    {
        ...
        HRESULT addDevice (IDeviceInformation* addedDeviceInfo) override
        {
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to query added MIDI device ID!");
                return S_OK;
            }

            info.deviceID = WinRTWrapper::getInstance()->hStringToString (deviceID);

            HWINRT_MIDI_LOG ("Detected MIDI device: " << info.deviceID);
            ...
            {
                HWINRT_MIDI_LOG ("MIDI device not enabled: " << info.deviceID);
                return S_OK;
            }
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to query detected MIDI device name for " << info.deviceID);
                return S_OK;
            }
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to query detected MIDI device defaultness for " << info.deviceID << " " << info.name);
                return S_OK;
            }

            info.isDefault = isDefault;

            HWINRT_MIDI_LOG ("Adding MIDI device: " << info.deviceID << " " << info.containerID << " " << info.name);
        ...
        HRESULT removeDevice (IDeviceInformationUpdate* removedDeviceInfo) override
        {
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to query removed MIDI device ID!");
                return S_OK;
            }

            auto removedDeviceId = WinRTWrapper::getInstance()->hStringToString (removedDeviceIdHstr);

            HWINRT_MIDI_LOG ("Removing MIDI device: " << removedDeviceId);

            {
                ...
                for (int i = 0; i < connectedDevices.size(); ++i)
                {
                    if (connectedDevices[i].deviceID == removedDeviceId)
                    {
                        ...
                        HWINRT_MIDI_LOG ("Removed MIDI device: " << removedDeviceId);
                        break;
                    }
                }
            }
        ...
        bool start()
        {
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to get MIDI device selector!");
                return false;
            }
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MidiIODeviceWatcher);
    };
    ...
    private:
        ...
        void bleDeviceDisconnected (const String& containerID) override
        {
            if (containerID == deviceInfo.containerID)
            {
                HWINRT_MIDI_LOG ("Disconnecting MIDI port from BLE disconnection: " << deviceInfo.deviceID
...

    {
              ...
              callback (cb)
        {
            ...
            if (midiPort == nullptr)
            {
                HWINRT_MIDI_LOG ("Timed out waiting for midi input port creation");
                return;
            }
            ...
            if (FAILED (hr))
            {
                HWINRT_MIDI_LOG ("Failed to set MIDI input callback");
                HAssertfalse;
            }
        }
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WinRTInputWrapper);
    };
                                       ...
                                       private WinRTIOWrapper <IMidiOutPortStatics, IMidiOutPort>
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WinRTOutputWrapper);
    };
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (WinRTMidiService)
};

#endif   // HUSE_WINRT_MIDI
...
struct MidiService :  public DeletedAtShutdown
{
    ...
    {
        HAssert (getInstance()->internal != nullptr);
        return *getInstance()->internal.get();
    }

    HDECLARE_SINGLETON (MidiService, false)

private:
    std::unique_ptr<MidiServiceType> internal;
};

HIMPLEMENT_SINGLETON (MidiService)
