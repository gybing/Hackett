...
#if HCOREAUDIO_LOGGING_ENABLED
 #define HCOREAUDIOLOG(a) { String camsg ("CoreAudio: "); camsg << a; Logger::writeToLog (camsg); }
#else
 #define HCOREAUDIOLOG(a)
#endif
...
float CALLTYPE SystemAudioVolume::getGain()              { return SystemVol (kAudioHardwareServiceDeviceProperty_VirtualMasterVolume).getGain(); }
bool  CALLTYPE SystemAudioVolume::setGain (float gain)   { return SystemVol (kAudioHardwareServiceDeviceProperty_VirtualMasterVolume).setGain (gain); }
bool  CALLTYPE SystemAudioVolume::isMuted()              { return SystemVol (kAudioDevicePropertyMute).isMuted(); }
bool  CALLTYPE SystemAudioVolume::setMuted (bool mute)   { return SystemVol (kAudioDevicePropertyMute).setMuted (mute); }
...
public:
         ...
         isOutputDevice (output)
    {
        ...
        HCOREAUDIOLOG ("Creating CoreAudioInternal\n"
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CoreAudioInternal)
};
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CoreAudioIODevice)
};
...
private:
    ...
    void run() override
    {
        ...
        HAssert (numInputChans + numOutputChans == buffer.getNumChannels());
    ...
    struct DeviceWrapper  : private AudioIODeviceCallback
    {
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (DeviceWrapper)
    };

    OwnedArray<DeviceWrapper> devices;

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (AudioIODeviceCombiner)
};
...
private:
    ...
    HDECLARE_WEAK_REFERENCEABLE (CoreAudioIODeviceType)
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CoreAudioIODeviceType)
};
