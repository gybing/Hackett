...
- (id) init: (AudioSessionHolder*) holder;
- (void) dealloc;
...
- (void) audioSessionChangedInterruptionType: (NSNotification*) notification
{
    ...
    if (getNotificationValueForKey (notification, AVAudioSessionInterruptionTypeKey, value))
    {
...
- (void) handleRouteChange: (NSNotification*) notification
{
    ...
    if (getNotificationValueForKey (notification, AVAudioSessionRouteChangeReasonKey, value))
...
#ifndef HIOS_AUDIO_LOGGING
 #define HIOS_AUDIO_LOGGING 0
#endif

#if HIOS_AUDIO_LOGGING
 #define HIOS_AUDIO_LOG(x)  DBG(x)
#else
 #define HIOS_AUDIO_LOG(x)
#endif
...
static void logNSError (NSError* e)
{
    if (e != nil)
    {
        HIOS_AUDIO_LOG ("iOS Audio error: " << [e.localizedDescription UTF8String]);
        HAssertfalse;
    }
}

#define HNSERROR_CHECK(X)     { NSError* error = nil; X; logNSError (error); }
...
private:
    ...
    HDECLARE_WEAK_REFERENCEABLE (iOSAudioIODeviceType)
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (iOSAudioIODeviceType)
};
                                      ...
                                      public AsyncUpdater
{
    Pimpl (iOSAudioIODeviceType* ioDeviceType, iOSAudioIODevice& ioDevice)
        : deviceType (ioDeviceType),
          owner (ioDevice)
    {
        HIOS_AUDIO_LOG ("Creating iOS audio device");
    ...
    static void setAudioSessionCategory (NSString* category)
    {
       ...
       #if ! HDISABLE_AUDIO_MIXING_WITH_OTHER_APPS
        ...
        HNSERROR_CHECK ([[AVAudioSession sharedInstance] setCategory: category
                                                             withOptions: options
                                                                   error: &error]);
    }
    ...
    {
        HNSERROR_CHECK ([[AVAudioSession sharedInstance] setActive: enabled
                                                                 error: &error]);
    }
    ...
    int tryBufferSize (const double currentSampleRate, const int newBufferSize)
    {
        ...
        auto session = [AVAudioSession sharedInstance];
        HNSERROR_CHECK ([session setPreferredIOBufferDuration: bufferDuration
                                                            error: &error]);
    ...
    void updateAvailableBufferSizes()
    {
        ...
        HIOS_AUDIO_LOG ("Buffer size after detecting available buffer sizes: " << bufferSize);
    }

    double trySampleRate (double rate)
    {
        auto session = [AVAudioSession sharedInstance];
        HNSERROR_CHECK ([session setPreferredSampleRate: rate
                                                      error: &error]);
    ...
    void updateAvailableSampleRates()
    {
        ...
        HIOS_AUDIO_LOG ("Lowest supported sample rate: "  << lowestRate);
        HIOS_AUDIO_LOG ("Highest supported sample rate: " << highestRate);

        for (double rate = lowestRate + 1000; rate < highestRate; rate += 1000)
        {
            const double supportedRate = trySampleRate (rate);
            HIOS_AUDIO_LOG ("Trying a sample rate of " << rate << ", got " << supportedRate);
        ...
        HIOS_AUDIO_LOG ("Sample rate after detecting available sample rates: " << sampleRate);
    }
    ...
    void updateHardwareInfo (bool forceUpdate = false)
    {
        ...
        HIOS_AUDIO_LOG ("Updating hardware info");
    ...
    {
        HIOS_AUDIO_LOG ("Setting target sample rate: " << targetSampleRate);
        sampleRate = trySampleRate (targetSampleRate);
        HIOS_AUDIO_LOG ("Actual sample rate: " << sampleRate);

        HIOS_AUDIO_LOG ("Setting target buffer size: " << targetBufferSize);
        bufferSize = tryBufferSize (sampleRate, targetBufferSize);
        HIOS_AUDIO_LOG ("Actual buffer size: " << bufferSize);
    }
                 ...
                 double sampleRateWanted, int bufferSizeWanted)
    {
        ...
        HIOS_AUDIO_LOG ("Opening audio device:"
    ...
    bool setAudioPreprocessingEnabled (bool enable)
    {
        ...
        HNSERROR_CHECK ([session setMode: mode
                                       error: &error]);
    ...
    Image getIcon (int size)
    {
        if (interAppAudioConnected)
        {
            ...
            if (hostUIImage != nullptr)
                return createImageFromUIImage (hostUIImage);
        }
        return Image();
    }
    ...
    void handleStatusChange (bool enabled, const char* reason)
    {
        ...
        HIOS_AUDIO_LOG ("handleStatusChange: enabled: " << (int) enabled << ", reason: " << reason);
    ...
    void handleRouteChange (AVAudioSessionRouteChangeReason reason)
    {
        ...
        HIOS_AUDIO_LOG ("handleRouteChange: " << reasonString);
                                        ...
                                        AudioUnitElement element)
    {
        ...
        HIOS_AUDIO_LOG ("handleAudioUnitPropertyChange: propertyID: " << String (propertyID)
                                                            << " scope: " << String (scope)
    ...
    void handleInterAppAudioConnectionChange()
    {
        ...
        HIOS_AUDIO_LOG ("handleInterAppAudioConnectionChange: " << (connected ? "connected"
                                                                                  : "disconnected"));
    ...
    {
        HIOS_AUDIO_LOG ("Creating the audio unit");
...

        {
            ...
            {
                HIOS_AUDIO_LOG ("Internal buffer size: " << String (framesPerSlice));
    ...
    static void fixAudioRouteIfSetToReceiver()
    {
        ...
        for (AVAudioSessionPortDescription* port in route.outputs)
        {
            ...
            {
                HNSERROR_CHECK ([session overrideOutputAudioPort: AVAudioSessionPortOverrideSpeaker
                                                               error: &error]);
                setAudioSessionActive (true);
            }
        }
    ...
    void handleStreamFormatChange()
    {
        ...
        {
            HIOS_AUDIO_LOG ("Stream format has changed: Sample rate " << desc.mSampleRate);
            triggerAsyncUpdate();
        }
    }
        ...
        public:
                  ...
                  inactiveChannelIndices (getInactiveChannelIndices (activeChannelIndices, numHardwareChannels))
            {
               #if HIOS_AUDIO_LOGGING
                {
                    ...
                    HIOS_AUDIO_LOG ((isInput ? "Input" : "Output") << " channel configuration: {" << info << "}");
                }
               #endif
            }
