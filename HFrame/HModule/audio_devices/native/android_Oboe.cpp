...
#ifndef HOBOE_LOG_ENABLED
 #define HOBOE_LOG_ENABLED 1
#endif

#if HOBOE_LOG_ENABLED
 #define HOBOE_LOG(x) DBG(x)
#else
 #define HOBOE_LOG(x) {}
#endif
...
struct OboeAudioIODeviceBufferHelpers<float>
{
    ...
    static void convertFromOboe (const float* srcInterleaved, AudioBuffer<float>& audioBuffer, int numSamples)
    {
        ...
        if (audioBuffer.getNumChannels() == 1)
        {
            HAssert (audioBuffer.getWritePointer (0) == srcInterleaved);
            return;
        }
    ...
    static void convertToOboe (const AudioBuffer<float>& audioBuffer, float* dstInterleaved, int numSamples)
    {
        ...
        if (audioBuffer.getNumChannels() == 1)
        {
            HAssert (audioBuffer.getReadPointer (0) == dstInterleaved);
            return;
        }
...
public:
          ...
          maxNumOutputChannels (maxNumOutputChannelsToUse)
    {
        ...
        HAssert (inputDeviceId != -1 || outputDeviceId != -1);
    }
...
private:
    ...
    public:
        ...
        void start()
        {
            ...
            if (openedOk() && stream != nullptr)
            {
                ...
                HOBOE_LOG ("Requested Oboe stream start with result: " + getOboeString (startResult));

                startResult = stream->waitForStateChange (expectedState, &nextState, timeoutNanos);

                HOBOE_LOG ("Starting Oboe stream with result: " + getOboeString (startResult);
        ...
        int getXRunCount() const
        {
            if (stream != nullptr)
            {
                ...
                HOBOE_LOG ("Failed to get Xrun count: " + getOboeString (count.error()));
            }
    ...
    private:
                   ...
                   oboe::AudioStreamCallback* callback = nullptr)
        {
            ...
            HOBOE_LOG (String ("Preparing Oboe stream with params:")
            ...
            HOBOE_LOG ("Building Oboe stream with result: " + getOboeString (openResult)
                 + "\nStream state = " + (stream != nullptr ? getOboeString (stream->getState()) : String ("?")));

            if (stream != nullptr && bufferSize != 0)
            {
                HOBOE_LOG ("Setting the bufferSizeInFrames to " + String (bufferSize));
                stream->setBufferSizeInFrames (bufferSize);
            }

            HOBOE_LOG (String ("Stream details:")
        ...
        void close()
        {
            if (stream != nullptr)
            {
                ...
                HOBOE_LOG ("Requested Oboe stream close with result: " + getOboeString (result));
            }
        }
    ...
    protected:
                                            ...
                                            this))
        {
            if (numInputChannels > 0)
            {
                ...
                if (inputStream->openedOk() && outputStream->openedOk())
                {
                    ...
                    HAssert (inputStream->getNativeStream()->getSampleRate()
                               ...
                               int bufferSize, oboe::AudioFormat format)
        {
            if (auto* nativeStream = stream != nullptr ? stream->getNativeStream() : nullptr)
            {
                ...
                HAssert (numChannels == nativeStream->getChannelCount());
                HAssert (sampleRate == 0 || sampleRate == nativeStream->getSampleRate());
                HAssert (format == nativeStream->getFormat());
            }
        }
    ...
    private:
        ...
        oboe::DataCallbackResult onAudioReady (oboe::AudioStream* stream, void* audioData, int32_t numFrames) override
        {
            if (audioCallbackGuard.compareAndSetBool (1, 0))
            {
                ...
                HAssert (stream->getDirection() == oboe::Direction::Output && stream == outputStream->getNativeStream());
                ...
                if (inputStream != nullptr)
                {
                    ...
                    {
                        HOBOE_LOG ("Unsupported input stream audio format: " + getOboeString (nativeInputStream->getFormat()));
                        HAssertfalse;
                    ...
                    {
                        HOBOE_LOG ("Failed to read from input stream: " + getOboeString (result.error()));
                    }
        ...
        {
            // only output stream should be the master stream receiving callbacks
            HAssert (stream->getDirection() == oboe::Direction::Output);

            HOBOE_LOG ("Oboe stream onErrorBeforeClose(): " + getOboeString (error));
        ...
        void onErrorAfterClose (oboe::AudioStream* stream, oboe::Result error) override
        {
            ...
            HOBOE_LOG ("Oboe stream onErrorAfterClose(): " + getOboeString (error));
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OboeAudioIODevice)
};
...
public:
 ...
 private:
    void checkAvailableDevices()
    {
        ...
        HOBOE_LOG ("-----InputDevices:");

        for (auto& device : inputDevices)
        {
            HOBOE_LOG ("name = " << device.name);
            HOBOE_LOG ("id = " << String (device.id));
            HOBOE_LOG ("sample rates size = " << String (device.sampleRates.size()));
            HOBOE_LOG ("num channels = " + String (device.numChannels));
        }

        HOBOE_LOG ("-----OutputDevices:");
    ...
    void addDevice (const LocalRef<jobject>& device, JNIEnv* env)
    {
        ...
        auto name = HString ((jstring) env->CallObjectMethod (device, getProductNameMethod));
    ...
    static const char* deviceTypeToString (int type)
    {
        switch (type)
        {
            ...
            default:  HAssertfalse; return ""; // type not supported yet, needs to be added!
        }
    }
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (OboeAudioIODeviceType)
};
...
public:
    ...
    {
        HOBOE_LOG ("OboeRealtimeThread: Oboe stream onErrorBeforeClose(): " + getOboeString (error));
        ignoreUnused (error);
        HAssertfalse;  // Should never get here!
    }

    void onErrorAfterClose (oboe::AudioStream*, oboe::Result error) override
    {
        HOBOE_LOG ("OboeRealtimeThread: Oboe stream onErrorAfterClose(): " + getOboeString (error));
        ignoreUnused (error);
        HAssertfalse;  // Should never get here!
    }
...
pthread_t createRealtimeAudioThread (void* (*entry) (void*), void* userPtr)
{
