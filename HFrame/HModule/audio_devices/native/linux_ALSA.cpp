...
#ifndef HALSA_LOGGING
 #define HALSA_LOGGING 0
#endif

#if HALSA_LOGGING
 #define HALSA_LOG(dbgtext)   { String tempDbgBuf ("ALSA: "); tempDbgBuf << dbgtext; Logger::writeToLog (tempDbgBuf); DBG (tempDbgBuf); }
 #define HCHECKED_RESULT(x)   (logErrorMessage (x, __LINE__))

 static int logErrorMessage (int err, int lineNum)
 {
    if (err < 0)
        HALSA_LOG ("Error: line " << lineNum << ": code " << err << " (" << snd_strerror (err) << ")");

    return err;
 }
#else
 #define HALSA_LOG(x)         {}
 #define HCHECKED_RESULT(x)   (x)
#endif

#define HALSA_FAILED(x)  failed (x)
...
static void getDeviceNumChannels (snd_pcm_t* handle, unsigned int* minChans, unsigned int* maxChans)
{
    ...
    if (snd_pcm_hw_params_any (handle, params) >= 0)
    {
        ...
        HALSA_LOG ("getDeviceNumChannels: " << (int) *minChans << " " << (int) *maxChans);
    ...
    {
        HALSA_LOG ("getDeviceNumChannels failed");
    }
}
                                 ...
                                 bool testInput)
{
    ...
    if (testOutput)
    {
        ...
        if (HCHECKED_RESULT (snd_pcm_open (&pcmHandle, deviceID.toUTF8().getAddress(), SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK)) >= 0)
        {
    ...
    if (testInput)
    {
        ...
        if (HCHECKED_RESULT (snd_pcm_open (&pcmHandle, deviceID.toUTF8(), SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK) >= 0))
        {
...
public:
    ALSADevice (const String& devID, bool forInput)
        : handle (nullptr),
          ...
          isInterleaved (true)
    {
        HALSA_LOG ("snd_pcm_open (" << deviceID.toUTF8().getAddress() << ", forInput=" << (int) forInput << ")");
        ...
        if (err < 0)
        {
            ...
            HALSA_LOG ("snd_pcm_open failed; " << error);
        }
    }
    ...
    bool setParameters (unsigned int sampleRate, int numChannels, int bufferSize)
    {
        ...
        HALSA_LOG ("ALSADevice::setParameters(" << deviceID << ", "
        ...
        if (HALSA_FAILED (snd_pcm_hw_params_set_rate_near (handle, hwParams, &sampleRate, 0))
            || HALSA_FAILED (snd_pcm_hw_params_set_channels (handle, hwParams, (unsigned int ) numChannels))
            || HALSA_FAILED (snd_pcm_hw_params_set_periods_near (handle, hwParams, &periods, &dir))
            || HALSA_FAILED (snd_pcm_hw_params_set_period_size_near (handle, hwParams, &samplesPerPeriod, &dir))
            || HALSA_FAILED (snd_pcm_hw_params (handle, hwParams)))
        {
            return false;
        }
        ...
        if (HALSA_FAILED (snd_pcm_hw_params_get_period_size (hwParams, &frames, &dir))
             || HALSA_FAILED (snd_pcm_hw_params_get_periods (hwParams, &periods, &dir)))
            latency = 0;
        else
            latency = (int) frames * ((int) periods - 1); // (this is the method JACK uses to guess the latency..)

        HALSA_LOG ("frames: " << (int) frames << ", periods: " << (int) periods
        ...
        if (HALSA_FAILED (snd_pcm_sw_params_current (handle, swParams))
            || HALSA_FAILED (snd_pcm_sw_params_get_boundary (swParams, &boundary))
            || HALSA_FAILED (snd_pcm_sw_params_set_silence_threshold (handle, swParams, 0))
            || HALSA_FAILED (snd_pcm_sw_params_set_silence_size (handle, swParams, boundary))
            || HALSA_FAILED (snd_pcm_sw_params_set_start_threshold (handle, swParams, samplesPerPeriod))
            || HALSA_FAILED (snd_pcm_sw_params_set_stop_threshold (handle, swParams, boundary))
            || HALSA_FAILED (snd_pcm_sw_params (handle, swParams)))
        {
            return false;
        }
    ...
    {
        HAssert (numChannelsRunning <= outputChannelBuffer.getNumChannels());
        ...
        if (numDone < 0)
        {
            ...
            if (HALSA_FAILED (snd_pcm_recover (handle, (int) numDone, 1 /* silent */)))
                return false;
        }

        if (numDone < numSamples)
            HALSA_LOG ("Did not write all samples: numDone: " << numDone << ", numSamples: " << numSamples);
    ...
    {
        HAssert (numChannelsRunning <= inputChannelBuffer.getNumChannels());
        ...
        if (isInterleaved)
        {
            ...
            if (num < 0)
            {
                ...
                if (HALSA_FAILED (snd_pcm_recover (handle, (int) num, 1 /* silent */)))
                    return false;
            }


            if (num < numSamples)
                HALSA_LOG ("Did not read all samples: num: " << num << ", numSamples: " << numSamples);
...
private:
    ...
    static AudioData::Converter* createConverter (bool forInput, int bitDepth,
                                                  ...
                                                  bool interleaved)
    {
        HALSA_LOG ("format: bitDepth=" << bitDepth << ", isFloat=" << (int) isFloat
    ...
    bool failed (const int errorNum)
    {
        ...
        HALSA_LOG ("ALSA error: " << error);
        return true;
    }

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ALSADevice)
};
...
public:
               ...
               int newBufferSize)
    {
        ...
        if (inputDevice != nullptr && HALSA_FAILED (snd_pcm_prepare (inputDevice->handle)))
            return;

        if (outputDevice != nullptr && HALSA_FAILED (snd_pcm_prepare (outputDevice->handle)))
            return;
    ...
    void close()
    {
        if (isThreadRunning())
        {
            ...
            {
                HALSA_LOG ("Thread is stuck in i/o.. Is pulseaudio suspended?");
    ...
    void run() override
    {
        while (! threadShouldExit())
        {
            if (inputDevice != nullptr && inputDevice->handle != nullptr)
            {
                ...
                {
                    HALSA_FAILED (snd_pcm_wait (inputDevice->handle, 2000));
                    ...
                    if (avail < 0)
                        HALSA_FAILED (snd_pcm_recover (inputDevice->handle, (int) avail, 0));
                }
            ...
            if (outputDevice != nullptr && outputDevice->handle != nullptr)
            {
                HALSA_FAILED (snd_pcm_wait (outputDevice->handle, 2000));
                ...
                if (avail < 0)
                    HALSA_FAILED (snd_pcm_recover (outputDevice->handle, (int) avail, 0));

                audioIoInProgress = true;

                if (! outputDevice->writeToOutputDevice (outputChannelBuffer, bufferSize))
                {
                    HALSA_LOG ("write failure");
                    break;
                }
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ALSAThread)
};
...
private:
    ...
    bool testDevice (const String& id, const String& outputName, const String& inputName)
    {
        ...
        {
            HALSA_LOG ("testDevice: '" << id.toUTF8().getAddress() << "' -> isInput: "
    ...
    void enumerateAlsaSoundcards()
    {
        ...
        while (outputIds.size() + inputIds.size() <= 64)
        {
            ...
            if (HCHECKED_RESULT (snd_ctl_open (&handle, ("hw:" + String (cardNum)).toRawUTF8(), SND_CTL_NONBLOCK)) >= 0)
            {
                if (HCHECKED_RESULT (snd_ctl_card_info (handle, info)) >= 0)
                {
                    ...
                    for (;;)
                    {
                        ...
                        for (unsigned int subDevice = 0, nbSubDevice = 1; subDevice < nbSubDevice; ++subDevice)
                        {
                            ...
                            HALSA_LOG ("Soundcard ID: " << id << ", name: '" << name
                                            << ", isInput:"  << (int) isInput
                ...
                HCHECKED_RESULT (snd_ctl_close (handle));
            }
        }
    }
    ...
    */
    void enumerateAlsaPCMDevices()
    {
        ...
        if (HCHECKED_RESULT (snd_device_name_hint (-1, "pcm", &hints)) == 0)
        {
            for (char** h = (char**) hints; *h; ++h)
            {
                ...
                HALSA_LOG ("ID: " << id << "; desc: " << description << "; ioid: " << ioid);
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ALSAAudioIODeviceType)
};
