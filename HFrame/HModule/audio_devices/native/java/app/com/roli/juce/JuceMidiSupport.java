...
public class JuceMidiSupport
{
    ...
    public static class BluetoothManager extends ScanCallback
    {
        ...
        public void startStopScan (boolean shouldStart)
        {
            ...
            {
                Log.d ("H", "BluetoothManager error: could not get default Bluetooth adapter");
                return;
            }
            ...
            {
                Log.d ("H", "BluetoothManager error: could not get Bluetooth LE scanner");
                return;
            }
        ...
        public boolean pairBluetoothMidiDevice (String address)
        {
            ...
            if (btDevice == null)
            {
                Log.d ("H", "failed to create buletooth device from address");
                return false;
            }
        ...
        public void onScanResult (int callbackType, ScanResult result)
        {
            ...
            {
                Log.d ("H", "ScanSettings.CALLBACK_TYPE_MATCH_LOST");
                BluetoothDevice device = result.getDevice ();
    ...
    public static class JuceMidiInputPort extends MidiReceiver implements JuceMidiPort
    {
        ...
        public void close ()
        {
            if (androidPort != null)
            {
                ...
                {
                    Log.d ("H", "IO Exception while closing port");
                }
            }
        ...
        {
            if (count > 0)
                handleReceive (HHost, msg, offset, count, timestamp);
        }
    ...
    public static class JuceMidiOutputPort implements JuceMidiPort
    {
        ...
        public void sendMidi (byte[] msg, int offset, int count)
        {
            if (androidPort != null)
            {
                ...
                {
                    Log.d ("H", "send midi had IO exception");
                }
            }
        }
    ...
    public static class MidiDeviceManager extends MidiManager.DeviceCallback implements MidiManager.OnDeviceOpenedListener
    {
        ...
        public MidiDeviceManager (Context contextToUse)
        {
            ...
            if (manager == null)
            {
                Log.d ("H", "MidiDeviceManager error: could not get MidiManager system service");
                return;
            }
        ...
        {
            // only update the list when H asks for a new list
            synchronized (MidiDeviceManager.class)
            {
                    ...
                    if (devicePair != null)
                    {
                        ...
                        if (device != null)
                        {
                            ...
                            if (isInput)
                            {
                                ...
                                if (outputPort != null)
                                    HMidiPort = new JuceMidiInputPort (this, outputPort, portInfo, host);
                            } else
                            {
                                MidiInputPort inputPort = device.openInputPort (portInfo.portIndex);

                                if (inputPort != null)
                                    HMidiPort = new JuceMidiOutputPort (this, inputPort, portInfo);
                            }

                            if (HMidiPort != null)
                            {
                                openPorts.put (portInfo, new WeakReference<JuceMidiPort> (HMidiPort));
        ...
        public void unpairBluetoothDevice (String address)
        {
            ...
            synchronized (MidiDeviceManager.class)
            {
                ...
                if (midiDevicePair != null)
                {
                    ...
                    {
                        Log.d ("H", "IOException while closing midi device");
                    }
                }
            }
        }
