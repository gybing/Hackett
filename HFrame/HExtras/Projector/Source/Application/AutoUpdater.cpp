...
namespace VersionHelpers
{
    ...
    bool isNewVersion (const String& current, const String& other)
    {
        ...
        HAssert (currentTokens.size() == 3 && otherTokens.size() == 3);
...
void LatestVersionCheckerAndUpdater::queryUpdateServer()
{
    ...
    URL latestVersionURL ("https://my.roli.com/software_versions/update_to/Projector/"
    ...
    {
        if (statusCode == 204)
            AlertWindow::showMessageBoxAsync (AlertWindow::InfoIcon, "No New Version Available", "Your H version is up to date.");
        else
...
public:
    ...
    {
        titleLabel.setText ("H version " + newVersion, dontSendNotification);
        titleLabel.setFont ({ 15.0f, Font::bold });
        titleLabel.setJustificationType (Justification::centred);
        addAndMakeVisible (titleLabel);

        contentLabel.setText ("A new version of H is available - would you like to download it?", dontSendNotification);
    ...
    void resized() override
    {
        ...
        auto topSlice = b.removeFromTop (HIconBounds.getHeight())
                         .withTrimmedLeft (HIconBounds.getWidth());
    ...
    void paint (Graphics& g) override
    {
        ...
        if (HIcon != nullptr)
            HIcon->drawWithin (g, HIconBounds.toFloat(),
    ...
    static std::unique_ptr<DialogWindow> launchDialog (const String& newVersion, const String& releaseNotes)
    {
        ...
        options.dialogTitle = "Download H version " + newVersion + "?";
...
private:
    ...
    std::unique_ptr<Drawable> HIcon;
    Rectangle<int> HIconBounds { 10, 10, 64, 64 };
...
void LatestVersionCheckerAndUpdater::askUserForLocationToDownload()
{
    FileChooser chooser ("Please select the location into which you'd like to install the new version",
                         { getAppSettings().getStoredPath (Ids::HPath, TargetOS::getThisOS()).get() });

    if (chooser.browseForDirectory())
    {
        auto targetFolder = chooser.getResult();

        if (isHFolder (targetFolder))
        {
            if (targetFolder.getChildFile (".git").isDirectory())
            {
                AlertWindow::showMessageBoxAsync (AlertWindow::WarningIcon, "Downloading New H Version",
                                                  "This folder is a GIT repository!\n\nYou should use a \"git pull\" to update it to the latest version.");

                return;
            }

            if (! AlertWindow::showOkCancelBox (AlertWindow::WarningIcon, "Overwrite Existing H Folder?",
                                                String ("Do you want to overwrite the folder:\n\n" + targetFolder.getFullPathName() + "\n\n..with the latest version from H.com?\n\n"
        ...
        {
            targetFolder = targetFolder.getChildFile ("H").getNonexistentSibling();
        }
...
private:
    ...
    Result unzipDownload (const MemoryBlock& data)
    {
        ...
        if (zip.getNumEntries() == 0)
            return Result::fail ("The downloaded file was not a valid H file!");
...
void restartProcess (const File& targetFolder)
{
   #if HMAC || HLINUX
    #if HMAC
     auto newProcess = targetFolder.getChildFile ("Projector.app").getChildFile ("Contents").getChildFile ("MacOS").getChildFile ("Projector");
    #elif HLINUX
     auto newProcess = targetFolder.getChildFile ("Projector");
    #endif

    StringArray command ("/bin/sh", "-c", "while killall -0 Projector; do sleep 5; done; " + newProcess.getFullPathName().quoted());
   #elif HWINDOWS
    auto newProcess = targetFolder.getChildFile ("Projector.exe");

    auto command = "cmd.exe /c\"@echo off & for /l %a in (0) do ( tasklist | find \"Projector\" >nul & ( if errorlevel 1 ( "
                    + targetFolder.getChildFile ("Projector.exe").getFullPathName().quoted() + " & exit /b ) else ( timeout /t 10 >nul ) ) )\"";
   #endif
...
//==============================================================================
HIMPLEMENT_SINGLETON (LatestVersionCheckerAndUpdater)
