...
#include "../Utility/Helpers/TranslationHelpers.h"
...
namespace
{
    ...
    struct LoadedProject
    {
        LoadedProject (const ArgumentList::Argument& fileToLoad)
        {
            ...
            if (! projectFile.hasFileExtension (Project::projectFileExtension))
                ConsoleApplication::fail (projectFile.getFullPathName() + " isn't a valid Hr project file!");
        ...
        {
            bool scanHPath = false, scanUserPaths = false;
    ...
    //==============================================================================
    /* Running a command-line of the form "proHr --resave foobar.Hr" will try to load
    ...
    */
    ...
    static void cleanWhitespace (const File& file, CleanupOptions options)
    {
        ...
        if (content.contains ("%""%") && content.contains ("//["))
            return; // ignore proHr GUI template files
    ...
    static void generateObfuscatedStringCode (const ArgumentList& args)
    {
        ...
        out << preferredLineFeed
            ...
            << preferredLineFeed
            << "    HAssert (result == " << originalText.quoted() << ");" << preferredLineFeed
    ...
    static void encodeBinary (const ArgumentList& args)
    {
        ...
        header << "// Auto-generated binary data by the Projector" << preferredLineFeed
    ...
    static void setGlobalPath (const ArgumentList& args)
    {
        ...
        auto settingsFile = userAppData.getChildFile ("Projector").getChildFile ("Projector.settings");
    ...
    static void createProjectFromPIP (const ArgumentList& args)
    {
        ...
        File HModulesPath, userModulesPath;

        if (args.size() > 3)
        {
            HModulesPath = args[3].resolveAsFile();

            if (! HModulesPath.exists())
                ConsoleApplication::fail ("Specified H modules directory doesn't exist.");
        ...
        PIPGenerator generator (pipFile, outputDir, HModulesPath, userModulesPath);
    ...
    static void showHelp()
    {
        ...
        auto appName = HApplication::getInstance()->getApplicationName();
        ...
        std::cout << appName << std::endl
                  ...
                  << "    Scans the given folder for C/C++ source files (recursively), and normalises any H-style comment division lines (i.e. any lines that look like //===== or //------- or /////////// will be replaced)." << std::endl
                  << std::endl
                  ...
                  << "    Scans each of the given folders (recursively) for any NEEDS_TRANS macros, and generates a translation file that can be used with Projector's translation file builder" << std::endl
                  << std::endl
                  ...
                  << std::endl
                  << " " << appName << " --create-project-from-pip path/to/PIP path/to/output path/to/H/modules (optional) path/to/user/modules (optional)" << std::endl
                  << "    Generates a folder containing a H project in the specified output path using the specified PIP file. Use the optional H and user module paths to override "
