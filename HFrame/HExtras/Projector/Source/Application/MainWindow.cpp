...
#include "../Wizards/NewProjectWizardClasses.h"
#include "../Utility/UI/JucerTreeViewBase.h"
#include "../ProjectSaving/ProjectSaver.h"

//==============================================================================
MainWindow::MainWindow()
    : DocumentWindow (ProjectorApplication::getApp().getApplicationName(),
                      ProjectorApplication::getApp().lookAndFeel.getCurrentColourScheme()
                      ...
                      false)
{
   ...
   #if ! HMAC
    setMenuBar (ProjectorApplication::getApp().getMenuModel());
   #endif
    ...
    ApplicationCommandManager& commandManager = ProjectorApplication::getCommandManager();
...
MainWindow::~MainWindow()
{
    ...
    removeKeyListener (ProjectorApplication::getCommandManager().getKeyMappings());
...
void MainWindow::createProjectContentCompIfNeeded()
{
    if (getProjectContentComponent() == nullptr)
    {
        ...
        HAssert (getProjectContentComponent() != nullptr);
    }
}
...
void MainWindow::setTitleBarIcon()
{
    if (auto* peer = getPeer())
    {
        if (currentProject != nullptr)
        {
            ...
            peer->setIcon (ImageCache::getFromMemory (BinaryData::icon_png, BinaryData::icon_pngSize));
        }
        else
        {
...
{
    ProjectorApplication::getApp().mainWindowList.closeWindow (this);
}

bool MainWindow::closeProject (Project* project, bool askUserToSave)
{
    HAssert (project == currentProject.get() && project != nullptr);
    ...
    if (! ProjectorApplication::getApp().openDocumentManager.closeAllDocumentsUsingProject (*project, askUserToSave))
        return false;
...
void MainWindow::moveProject (File newProjectFileToOpen)
{
    ...
    if (currentProject != nullptr)
    {
        ProjectorApplication::getApp().getCommandManager().invokeDirectly (openInIDE ? CommandIDs::saveAndOpenInIDE
...
bool MainWindow::canOpenFile (const File& file) const
{
    return (! file.isDirectory())
             && (file.hasFileExtension (Project::projectFileExtension)
                  || ProjectorApplication::getApp().openDocumentManager.canOpenFile (file));
}
...
bool MainWindow::tryToOpenPIP (const File& pipFile)
{
    ...
    if (! ProjectorApplication::getApp().mainWindowList.openFile (generator.getJucerFile()))
        return false;
...
{
    if (auto* window = ProjectorApplication::getApp().mainWindowList.getMainWindowForFile (generator.getJucerFile()))
    {
...
void MainWindow::activeWindowStatusChanged()
{
    ...
    ProjectorApplication::getApp().openDocumentManager.reloadModifiedFiles();
        ...
        if (p->hasProjectBeenModified())
        {
            ...
            MessageManager::callAsync ([=] ()
            {
                ...
                auto result = AlertWindow::showOkCancelBox (AlertWindow::QuestionIcon,
                                                            TRANS ("The .Hr file has been modified since the last save."),
                ...
                {
                    ProjectorApplication::getApp().getCommandManager().invokeDirectly (CommandIDs::saveProject, true);
                }
            });
        }
    }
...
{
    HAssert (currentProject == nullptr);
...
void MainWindow::getCommandInfo (const CommandID commandID, ApplicationCommandInfo& result)
{
    switch (commandID)
    {
        ...
        case CommandIDs::goToPreviousWindow:
            ...
            result.setActive (ProjectorApplication::getApp().mainWindowList.windows.size() > 1);
...
bool MainWindow::perform (const InvocationInfo& info)
{
    switch (info.commandID)
    {
        ...
        case CommandIDs::goToPreviousWindow:
            ProjectorApplication::getApp().mainWindowList.goToSiblingWindow (this, -1);
            break;
...
{
    if (currentProject != nullptr)
        setName (currentProject->getProjectNameString() + " - Projector");
    else
        setName ("Projector");
}
...
{
    HAssert (windows.contains (w));

   #if ! HMAC
    if (windows.size() == 1 && ! isInReopenLastProjects)
    {
        HApplicationBase::getInstance()->systemRequestedQuit();
    }
    else
   #endif
    {
