...
#include "../Wizards/NewFileWizard.h"
#include "JucerDocument.h"
#include "ObjectTypes.h"
#include "UI/JucerDocumentEditor.h"
#include "UI/TestComponent.h"
#include "UtilityFunctions.h"
#include "Documents/ComponentDocument.h"
#include "Documents/ButtonDocument.h"
...
//==============================================================================
const char* const JucerDocument::HrCompXmlTag = "HR_COMPONENT";

std::unique_ptr<XmlElement> JucerDocument::createXml() const
{
    auto doc = std::make_unique<XmlElement> (HrCompXmlTag);
...
{
    if (xml.hasTagName (HrCompXmlTag)
...
void JucerDocument::fillInGeneratedCode (GeneratedCode& code) const
{
    ...
    HAssert (e != nullptr);
    code.HrMetadata = e->toString (XmlElement::TextFormat().withoutHeader());
...
bool JucerDocument::findTemplateFiles (String& headerContent, String& cppContent) const
{
    ...
    headerContent = BinaryData::ComponentTemplate_h;
    cppContent    = BinaryData::ComponentTemplate_cpp;
    return true;
}
...
bool JucerDocument::flushChangesToDocuments (Project* project, bool isInitial)
{
    ...
    OpenDocumentManager& odm = ProjectorApplication::getApp().openDocumentManager;
...
bool JucerDocument::reloadFromDocument()
{
    ...
    if (newXML == nullptr || ! newXML->hasTagName (HrCompXmlTag))
        return false;
...
std::unique_ptr<XmlElement> JucerDocument::pullMetaDataFromCppFile (const String& cpp)
{
    ...
    auto startLine = indexOfLineStartingWith (lines, "BEGIN_HR_METADATA", 0);

    if (startLine > 0)
    {
        auto endLine = indexOfLineStartingWith (lines, "END_HR_METADATA", startLine);
...
bool JucerDocument::isValidJucerCppFile (const File& f)
{
    if (f.hasFileExtension (cppFileExtensions))
    {
        ...
        if (xml != nullptr)
            return xml->hasTagName (HrCompXmlTag);
    }
...
static JucerDocument* createDocument (SourceCodeDocument* cpp)
{
    ...
    if (xml == nullptr || ! xml->hasTagName (JucerDocument::HrCompXmlTag))
        return nullptr;
...
public:
    ...
    {
        std::unique_ptr<JucerDocument> HrDoc (JucerDocument::createForCppFile (getProject(), getFile()));

        if (HrDoc != nullptr)
            return new JucerDocumentEditor (HrDoc.release());
    ...
    void createNewFile (Project& project, Project::Item parent) override
    {
        ...
        if (newFile != File())
        {
            ...
            if (auto* cpp = dynamic_cast<SourceCodeDocument*> (odm.openFile (&project, cppFile)))
            {
                ...
                {
                    std::unique_ptr<JucerDocument> HrDoc (new ComponentDocument (cpp));

                    if (HrDoc != nullptr)
                    {
                        HrDoc->setClassName (newFile.getFileNameWithoutExtension());

                        HrDoc->flushChangesToDocuments (&project, true);
                        HrDoc.reset();
