...
public:
    ...
    void fillInCreationCode (GeneratedCode& code, Component* component, const String& memberVariableName) override
    {
        ...
        for (int i = 0; i < t->getNumTabs(); ++i)
        {
            ...
            if (isTabUsingJucerComp (t, i))
            {
                File HrCpp = code.document->getCppFile().getSiblingFile (getTabJucerFile (t, i));

                std::unique_ptr<JucerDocument> doc (JucerDocument::createForCppFile (nullptr, HrCpp));

                if (doc != nullptr)
                {
                    code.includeFilesCPP.add (HrCpp.withFileExtension (".h"));
    ...
    static std::unique_ptr<XmlElement> getTabState (TabbedComponent* tc, int tabIndex)
    {
        ...
        if (TabDemoContentComp* const tdc = dynamic_cast<TabDemoContentComp*> (tc->getTabContentComponent (tabIndex)))
        {
            ...
            xml->setAttribute ("HrComponentFile", tdc->HrComponentFile);
        }
    ...
    static void restoreTabState (TabbedComponent* tc, int tabIndex, const XmlElement& xml)
    {
        ...
        if (TabDemoContentComp* const tdc = dynamic_cast<TabDemoContentComp*> (tc->getTabContentComponent (tabIndex)))
        {
            ...
            tdc->HrComponentFile = xml.getStringAttribute ("HrComponentFile");
    ...
    static String getTabJucerFile (TabbedComponent* tc, int tabIndex)
    {
        ...
        return tdc != nullptr ? tdc->HrComponentFile : String();
    }
...
private:
    ...
    public:
        ...
        void resized() override
        {
            if (HrComp != nullptr)
            {
                HrComp->setBounds (getLocalBounds());
                setOpaque (HrComp->isOpaque());
            }
        }
        ...
        void updateContent()
        {
            ...
            {
                if (HrComp == nullptr
                    || HrComp->getOwnerDocument() == nullptr
                    || HrComp->getFilename() != HrComponentFile)
                {
                    HrComp.reset();

                    HrComp.reset (new TestComponent (ComponentTypeHandler::findParentDocument (this), nullptr, false));
                    HrComp->setFilename (HrComponentFile);
                    HrComp->setToInitialSize();

                    addAndMakeVisible (HrComp.get());
                }
            }
            else
            {
        ...
        String HrComponentFile;
        std::unique_ptr<TestComponent> HrComp;
    };
    ...
    public:
        ...
        {
            HAssertfalse; // shouldn't get called
        }
    ...
    public:
        TabJucerFileProperty (TabbedComponent* const comp, JucerDocument& doc, const int tabIndex_)
            : FilePropertyComponent ("Hr file", false, true),
              component (comp),
              document (doc),
              tabIndex (tabIndex_)
        {
