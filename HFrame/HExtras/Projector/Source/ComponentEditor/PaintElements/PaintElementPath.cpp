...
#include "PaintElementPath.h"
#include "../Properties/PositionPropertyBase.h"
#include "PaintElementUndoableAction.h"
...
bool PaintElementPath::getPoint (int index, int pointNumber, double& x, double& y, const Rectangle<int>& parentArea) const
{
    ...
    HAssert (pointNumber < 3 || p->type == Path::Iterator::cubicTo);
    HAssert (pointNumber < 2 || p->type == Path::Iterator::cubicTo || p->type == Path::Iterator::quadraticTo);
...
{
    const Rectangle<int> area (((PaintRoutineEditor*) getParentComponent())->getComponentArea());
    HAssert (getParentComponent() != nullptr);
