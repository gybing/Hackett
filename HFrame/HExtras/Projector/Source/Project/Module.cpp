...
#include "../ProjectSaving/ProjectExport_Xcode.h"
#include "../Application/Application.h"

//==============================================================================
ModuleDescription::ModuleDescription (const File& folder)
   : moduleFolder (folder),
     moduleInfo (parseHHeaderMetadata (getHeader()))
{
}
...
void LibraryModule::writeIncludes (ProjectSaver& projectSaver, OutputStream& out)
{
    ...
    {
        auto HModuleFolder = moduleInfo.getFolder();

        auto localModuleFolder = project.getLocalModuleFolder (id);
        localModuleFolder.createDirectory();
        projectSaver.copyFolder (HModuleFolder, localModuleFolder);
    }
...
Array<LibraryModule::CompileUnit> LibraryModule::getAllCompileUnits (ProjectType::Target::Type forTarget) const
{
    ...
    for (auto& cu : units)
    {
        ...
        HAssert (cu.isCompiledForObjC || cu.isCompiledForNonObjC);
    }
...
bool EnabledModuleList::isAudioPluginModuleMissing() const
{
    return project.getProjectType().isAudioPlugin()
            && ! isModuleEnabled ("audio_plugin_client");
}
    ...
    if (info.isValid())
    {
        ...
        if (! isModuleEnabled (moduleID))
        {
            ...
            if (sendAnalyticsEvent)
            {
                ...
                Analytics::getInstance()->logEvent ("Module Added", data, ProjectorAnalyticsEvent::projectEvent);
            }
        }
    }
}
...
bool isHFolder (const File& f)
{
    return isHModulesFolder (f.getChildFile ("modules"));
}

bool isHModulesFolder (const File& f)
{
    return f.isDirectory() && f.getChildFile ("core").isDirectory();
}
