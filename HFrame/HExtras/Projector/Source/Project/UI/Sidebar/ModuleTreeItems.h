...
public:
    ...
    Icon getIcon() const override
    {
        ...
        if (! isSelected())
        {
            if (moduleInfo.isValid() && moduleInfo.getVendor() == "H")
            {
...
private:
        ...
        void refresh()
        {
            ...
            for (Project::ExporterIterator exporter (project); exporter.next();)
            {
                ...
                props.add (pathComponent,
                           ...
                           "This can be an absolute path, or relative to the Hr project folder, but it "
            ...
            auto menuItemString = (TargetOS::getThisOS() == TargetOS::osx ? "\"Projector->Global Paths...\""
                                                                          : "\"File->Global Paths...\"");
            ...
            props.add (new BooleanPropertyComponent (globalPathValue,
                       ...
                       "This means that if this Projector project is opened on another machine it will use that machine's global path as the path to this module.");
    ...
    private:
        ...
        private:
            ...
            HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ModuleInfoComponent)
        };
        ...
        private:
            ...
            HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MissingDependenciesComponent)
        };
        ...
        struct CppStandardWarningComponent    : public PropertyComponent
        {
            ...
            HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (CppStandardWarningComponent)
        };
    };

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ModuleItem)
};
...
public:
          ...
          moduleListTree (p.getEnabledModules().state)
    {
        ...
        ProjectorApplication::getApp().getHPathModuleList().addListener (this);
        ProjectorApplication::getApp().getUserPathsModuleList().addListener (this);
        project.getExporterPathsModuleList().addListener (this);
    }

    ~EnabledModulesItem() override
    {
        ProjectorApplication::getApp().getHPathModuleList().removeListener (this);
        ProjectorApplication::getApp().getUserPathsModuleList().removeListener (this);
    ...
    void showPopupMenu() override
    {
        ...
        // H path
        PopupMenu HPathModules;

        for (auto& mod : ProjectorApplication::getApp().getHPathModuleList().getAllModules())
            HPathModules.addItem (index++, mod.first, ! enabledModules.isModuleEnabled (mod.first));

        HPathModules.addSeparator();
        HPathModules.addItem (-1, "Re-scan path");

        allModules.addSubMenu ("Global H modules path", HPathModules);

        // User path
        index = 200;
        PopupMenu userPathModules;

        for (auto& mod : ProjectorApplication::getApp().getUserPathsModuleList().getAllModules())
    ...
    void handlePopupMenuResult (int resultCode) override
    {
        ...
        {
            if      (resultCode == -1)  ProjectorApplication::getApp().rescanHPathModules();
            else if (resultCode == -2)  ProjectorApplication::getApp().rescanUserPathModules();
        ...
        else if (resultCode > 0)
        {
            ...
            if (resultCode < 200)
            {
                list = ProjectorApplication::getApp().getHPathModuleList().getAllModules();
                offset = 100;
            }
            else if (resultCode < 300)
            {
                list = ProjectorApplication::getApp().getUserPathsModuleList().getAllModules();
                offset = 200;
            }
...
private:
    ...
    {
        auto HPathModuleList = ProjectorApplication::getApp().getHPathModuleList().getAllModules();

        auto& userPathModules = ProjectorApplication::getApp().getUserPathsModuleList();
        userPathModules.removeDuplicates (HPathModuleList);

        auto& exporterPathModules = project.getExporterPathsModuleList();
        exporterPathModules.removeDuplicates (HPathModuleList);
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (EnabledModulesItem)
};
