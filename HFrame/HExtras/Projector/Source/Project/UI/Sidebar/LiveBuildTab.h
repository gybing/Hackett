...
struct LiveBuildSettingsComponent  : public Component
{
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LiveBuildSettingsComponent)
};
...
public:
    ...
    String errorMessage;
    Component::SafePointer<ProjectorAppClasses::ErrorListComp> errorListComp;
...
private:
    ...
    String getErrorMessage()
    {
        ...
        if (! isMac && ! isWin && ! isLinux)
            return "Live-build features are not supported on your system.\n\n"
                   "Please check supported platforms at www.H.com!";

        if (isLinux)
            return "Live-build features for Linux are under development.\n\n"
                   "Please check for updates at www.H.com!";
    ...
    void buildConcertina (CompileEngineChildProcess& child)
    {
        ...
        errorListComp = new ProjectorAppClasses::ErrorListComp (child.errorList);
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LiveBuildTab)
};
