...
public:
    ...
    {
        HAssert (columnHeaders.size() == columnWidths.size());
    ...
    {
        HAssert (isPositiveAndBelow (index, widths.size()));
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (InfoButton)
};
...
private:
    ...
    {
        auto availableTextWidth = ProjectorLookAndFeel::getTextWidthForPropertyComponent (pp);

        auto font = ProjectorLookAndFeel::getPropertyComponentFont();
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (PropertyGroupComponent)
};
