...
#include "Project.h"
#include "../ProjectSaving/ProjectSaver.h"
...
Project::~Project()
{
    ...
    ProjectorApplication::getApp().openDocumentManager.closeAllDocumentsUsingProject (*this, false);
}

const char* Project::projectFileExtension = ".Hr";
...
{
    projectRoot.setProperty (Ids::HrVersion, ProjectInfo::versionString, nullptr);
...
{
    HAssert (! ProjectorApplication::getApp().isRunningCommandLine);
...
{
    projectNameValue.referTo         (projectRoot, Ids::name,             getUndoManager(), "H Project");
    ...
    displaySplashScreenValue.referTo (projectRoot, Ids::displaySplashScreen, getUndoManager(), ! ProjectorApplication::getApp().isPaidOrGPL());
    ...
    if (ProjectorApplication::getApp().isPaidOrGPL())
    {
        reportAppUsageValue.setDefault (ProjectorApplication::getApp().licenseController->getState().applicationUsageDataState
...
{
    return HMAJOR_VERSION * 100000
         + HMINOR_VERSION * 1000
         + HBUILDNUMBER;
}

static bool isModuleNewerThanProjector (const ModuleDescription& module)
{
    if (module.getID().startsWith ("")
...
void Project::warnAboutOldProjectorVersion()
{
    for (auto& HModule : ProjectorApplication::getApp().getHPathModuleList().getAllModules())
    {
        if (isModuleNewerThanProjector ({ HModule.second }))
        {
            // Projector is out of date!
            if (ProjectorApplication::getApp().isRunningCommandLine)
                std::cout <<  "WARNING! This version of the Projector is out-of-date!" << std::endl;
            else
                AlertWindow::showMessageBoxAsync (AlertWindow::WarningIcon,
                                                  "Projector",
                                                  "This version of the Projector is out-of-date!"
                                                  "\n\n"
                                                  "Always make sure that you're running the very latest version, "
                                                  "preferably compiled directly from the H repository that you're working with!");
...
{
    auto xml = parseXMLIfTagMatches (file, Ids::HRPROJECT.toString());
    ...
    if (! newTree.hasType (Ids::HRPROJECT))
    ...
    if (! ProjectorApplication::getApp().isRunningCommandLine)
        warnAboutOldProjectorVersion();

    compileEngineSettings.reset (new CompileEngineSettings (projectRoot));

    exporterPathsModuleList.reset (new AvailableModuleList());
    rescanExporterPathModules (! ProjectorApplication::getApp().isRunningCommandLine);
...
Result Project::saveProject (const File& file, bool isCommandLineApp)
{
    ...
    if (! isCommandLineApp)
    {
        ProjectorApplication::getApp().openDocumentManager.saveAll();
...
void Project::moveTemporaryDirectory (const File& newParentDirectory)
{
    ...
    if (auto* window = ProjectorApplication::getApp().mainWindowList.getMainWindowForFile (getFile()))
    {
...
void Project::createPropertyEditors (PropertyListBuilder& props)
{
    ...
    {
        String licenseRequiredTagline ("Required for closed source applications without an Indie or Pro H license");
        String licenseRequiredInfo ("In accordance with the terms of the H 5 End-Use License Agreement (www.H.com/H-5-licence), "
                                    "this option can only be disabled for closed source applications if you have a H Indie or Pro "
                                    "license, or are using H under the GPL v3 license.");

        StringPairArray description;
        description.set ("Report H app usage", "This option controls the collection of usage data from users of this H application.");
        description.set ("Display the H splash screen", "This option controls the display of the standard H splash screen.");

        if (ProjectorApplication::getApp().isPaidOrGPL())
        {
            props.add (new ChoicePropertyComponent (reportAppUsageValue, String ("Report H App Usage") + " (" + licenseRequiredTagline + ")"),
                       description["Report H app usage"] + " " + licenseRequiredInfo);

            props.add (new ChoicePropertyComponent (displaySplashScreenValue, String ("Display the H Splash Screen") + " (" + licenseRequiredTagline + ")"),
                       description["Display the H splash screen"] + " " + licenseRequiredInfo);
        }
        else
        {
            StringArray options;
            ...
            props.add (new ChoicePropertyComponent (Value(), "Report H App Usage", options, vars),
                       description["Report H app usage"] + " " + licenseRequiredInfo);

            props.add (new ChoicePropertyComponent (Value(), "Display the H Splash Screen", options, vars),
                       description["Display the H splash screen"] + " " + licenseRequiredInfo);
        }
    }
    ...
    props.add (new ChoicePropertyComponent (splashScreenColourValue, "Splash Screen Colour",
                                            { "Dark", "Light" },
                                            { "Dark", "Light" }),
               "Choose the colour of the H splash screen.");
...

    {
        ...
        props.add (new ChoicePropertyComponent (maxBinaryFileSizeValue, "BinaryData.cpp Size Limit", maxSizeNames, maxSizeCodes),
                   "When splitting binary data into multiple cpp files, the Projector attempts to keep the file sizes below this threshold. "
...
void Project::createAudioPluginPropertyEditors (PropertyListBuilder& props)
{
    props.add (new MultiChoicePropertyComponent (pluginFormatsValue, "Plugin Formats",
               ...
               "or H version 5.3.2. You also need a VST2 license from Steinberg to distribute VST2 plug-ins.");
...
bool Project::Item::renameFile (const File& newFile)
{
    ...
    {
        setFile (newFile);
        ProjectorApplication::getApp().openDocumentManager.fileHasBeenRenamed (oldFile, newFile);
        return true;
    }
...
Icon Project::Item::getIcon (bool isOpen) const
{
    ...
    if (isMainGroup())
        return { icons.HLogo, Colours::orange };
...
{
    return projectRoot.getOrCreateChildWithName (Ids::HOPTIONS, nullptr);
}
...
{
    auto configValue = projectRoot.getChildWithName (Ids::HOPTIONS).getProperty (name, "default");
...
{
    return getEnabledModules().isModuleEnabled ("audio_processors") && isConfigFlagEnabled ("HPLUGINHOST_AU");
}

bool Project::isVSTPluginHost()
{
    return getEnabledModules().isModuleEnabled ("audio_processors") && isConfigFlagEnabled ("HPLUGINHOST_VST");
}

bool Project::isVST3PluginHost()
{
    return getEnabledModules().isModuleEnabled ("audio_processors") && isConfigFlagEnabled ("HPLUGINHOST_VST3");
}
...
ModuleIDAndFolder Project::getModuleWithID (const String& id)
{
    ...
    const auto& list = (isHModule (id) ? ProjectorApplication::getApp().getHPathModuleList().getAllModules()
                                          : ProjectorApplication::getApp().getUserPathsModuleList().getAllModules());
...
{
    HAssert (index >= 0 && index < getNumExporters());
...
bool Project::ExporterIterator::next()
{
    ...
    if (exporter == nullptr)
    {
        HAssertfalse; // corrupted project file?
        return next();
    }
