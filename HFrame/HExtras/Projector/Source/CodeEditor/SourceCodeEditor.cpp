   ...
   : CodeEditorComponent (codeDocument, tokeniser), file (f)
{
    ...
    setCommandManager (&ProjectorApplication::getCommandManager());
}
...
public:
    FindPanel()
    {
        ...
        editor.onReturnKey  = [] { ProjectorApplication::getCommandManager().invokeDirectly (CommandIDs::findNext, true); };
...
void GenericCodeEditorComponent::showFindPanel()
{
    if (findPanel == nullptr)
    {
        ...
        findPanel->setCommandManager (&ProjectorApplication::getCommandManager());
...
void CppCodeEditorComponent::insertComponentClass()
{
    ...
    while (aw.runModalLoop() != 0)
    {
        ...
        {
            String code (BinaryData::InlineComponentTemplate_h);
