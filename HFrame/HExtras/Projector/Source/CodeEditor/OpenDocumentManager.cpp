...
#include "OpenDocumentManager.h"
#include "../CodeEditor/ItemPreviewComponent.h"
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (UnknownDocument)
};
...
OpenDocumentManager::Document* OpenDocumentManager::openFile (Project* project, const File& file)
{
    ...
    HAssert (d != nullptr);  // should always at least have been picked up by UnknownDocument
...
bool RecentDocumentList::documentAboutToClose (OpenDocumentManager::Document* document)
{
    ...
    HAssert (! previousDocs.contains (document));
    HAssert (! nextDocs.contains (document));
