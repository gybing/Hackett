...
public:
    LicenseWebview (ModalComponentManager::Callback* callbackToUse, const String& request)
        : DialogWindow ("Log-in to Projector", Colour (0xfff1f1f1), true, true)
    {
...
private:
    class LicenseWebviewContent  : public Component
    {
        ...
        struct RedirectWebBrowserComponent : public WebBrowserComponent
        {
            ...
            HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (RedirectWebBrowserComponent)
        };
                         ...
                         private LicenseController::StateChangedCallback
        {
            Header()  : avatarButton ("User Settings", &getIcons().user)
            {
                ...
                if (auto* licenseController = ProjectorApplication::getApp().licenseController.get())
                {
            ...
            void paint (Graphics& g) override
            {
                ...
                if (HLogo != nullptr)
                    HLogo->drawWithin (g, r.toFloat(), RectanglePlacement::xLeft + RectanglePlacement::yMid, 1.0);
            }
            ...
            std::unique_ptr<Drawable> HLogo { Drawable::createFromImageData (BinaryData::Hlogowithtext_svg,
                                                                                BinaryData::Hlogowithtext_svgSize) };
            IconButton avatarButton;

            HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Header)
        };
    ...
    public:
            ...
            : parentWindow (parentWindowToUse), modalCallback (callbackToUse), webview (*this)
        {
           ...
           #if HWINDOWS // windows needs the webcomponent be visible
        ...
        bool pageAboutToLoad (const String& page)
        {
            ...
            bool isValid = (url.getDomain().endsWith ("roli.com") || url.getDomain().endsWith ("H.com"));
        ...
        static bool isValidURL (const URL& url)     { return (url.getDomain().endsWith ("roli.com") || url.getDomain().endsWith ("H.com")); }
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LicenseWebviewContent)
    };

    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (LicenseWebview)
};
