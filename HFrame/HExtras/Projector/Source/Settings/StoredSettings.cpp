...
{
    return *ProjectorApplication::getApp().settings;
}
...
{
    return new PropertiesFile (ProjectorApplication::getApp()
...
{
    const auto filename = String ("Projector_Project_" + projectUID);
...
void StoredSettings::updateKeyMappings()
{
    ...
    if (auto* commandManager = ProjectorApplication::getApp().commandManager.get())
    {
...
{
    propertyFiles.clear();
    propertyFiles.add (createPropsFile ("Projector", false));
...
void StoredSettings::updateOldProjectSettingsFiles()
{
    ...
    auto proHrSettingsDirectory = ProjectorApplication::getApp().getPropertyFileOptionsFor ("Dummy", false)
                                                                  .getDefaultFile().getParentDirectory();

    auto newProjectSettingsDir = proHrSettingsDirectory.getChildFile ("ProjectSettings");
    newProjectSettingsDir.createDirectory();

    DirectoryIterator iter (proHrSettingsDirectory, false, "*.settings");
    while (iter.next())
    {
        auto f = iter.getFile();
        auto oldFileName = f.getFileName();

        if (oldFileName.contains ("IntroHr"))
        {
            auto newFileName = oldFileName.replace ("IntroHr", "Projector");
...
void StoredSettings::loadSwatchColours()
{
    ...
    {
        #include "../Utility/Helpers/Colours.h"
...
void StoredSettings::checkHPaths()
{
    auto moduleFolder = getStoredPath (Ids::defaultJuceModulePath, TargetOS::getThisOS()).get().toString();
    auto HFolder   = getStoredPath (Ids::HPath, TargetOS::getThisOS()).get().toString();

    auto validModuleFolder = isGlobalPathValid ({}, Ids::defaultJuceModulePath, moduleFolder);
    auto validJuceFolder   = isGlobalPathValid ({}, Ids::HPath, HFolder);

    if (validModuleFolder && ! validJuceFolder)
        projectDefaults.getPropertyAsValue (Ids::HPath, nullptr) = File (moduleFolder).getParentDirectory().getFullPathName();
    else if (! validModuleFolder && validJuceFolder)
        projectDefaults.getPropertyAsValue (Ids::defaultJuceModulePath, nullptr) = File (HFolder).getChildFile ("modules").getFullPathName();
}
...
void StoredSettings::setDontAskAboutHPathAgain() noexcept
{
    getGlobalProperties().setValue ("dontAskAboutHPath", 1);
}
...
static String getFallbackPathForOS (const Identifier& key, DependencyPathOS os)
{
    if (key == Ids::HPath)
    {
        return (os == TargetOS::windows ? "C:\\H" : "~/H");
    }
    else if (key == Ids::defaultJuceModulePath)
    {
        return (os == TargetOS::windows ? "C:\\H\\modules" : "~/H/modules");
    }
    else if (key == Ids::defaultUserModulePath)
    {
