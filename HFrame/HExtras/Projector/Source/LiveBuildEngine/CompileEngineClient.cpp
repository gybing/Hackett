...
#include "CompileEngineClient.h"
#include "CompileEngineServer.h"
#include "CompileEngineSettings.h"
...
//==============================================================================
static File getProjectorTempFolder() noexcept
{
   #if HMAC
    return { "~/Library/Caches/com.H.proHr" };
   #else
    return File::getSpecialLocation (File::tempDirectory).getChildFile ("com.H.proHr");
   #endif
}
...
static File getCacheLocationForProject (Project& project) noexcept
{
    ...
    return getProjectorTempFolder().getChildFile ("Intermediate Files").getChildFile (cacheFolderName);
}
...
public:
    ...
    void sendRebuild()
    {
        ...
        if (areAnyModulesMissing (project))
        {
            ...
            owner.errorList.resetToError ("Some of your H modules can't be found! "
...
private:
    ...
    String getGlobalDefs()
    {
        ...
        // Use the H implementation of std::function until the live build
    ...
    void scanForProjectFiles (Project& proj, ProjectBuildInfo& build)
    {
        ...
        {
            auto isVSTHost = project.getEnabledModules().isModuleEnabled ("audio_processors")
                   && (project.isConfigFlagEnabled ("HPLUGINHOST_VST3") || project.isConfigFlagEnabled ("HPLUGINHOST_VST"));
    ...
    static bool doesProjectMatchSavedHeaderState (Project& project)
    {
        ...
        if (auto xml = parseXMLIfTagMatches (project.getFile(), Ids::HRPROJECT.toString()))
        {
    ...
    StringArray getSystemIncludePaths()
    {
        ...
        auto isVSTHost = project.getEnabledModules().isModuleEnabled ("audio_processors")
                       && (project.isConfigFlagEnabled ("HPLUGINHOST_VST3")
                             || project.isConfigFlagEnabled ("HPLUGINHOST_VST"));
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ChildProcess)
};
...
void CompileEngineChildProcess::openPreview (const ClassDatabase::Class& comp)
{
    if (process != nullptr)
    {
        ...
        OwnedArray<MainWindow>& windows = ProjectorApplication::getApp().mainWindowList.windows;
...
{
    if (HApplication* app = HApplication::getInstance())
...
{
    for (MainWindow* mw : ProjectorApplication::getApp().mainWindowList.windows)
