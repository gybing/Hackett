...
#include "../Utility/UI/PropertyComponents/PropertyComponentsWithEnablement.h"
...
public:
    ...
    Array<Project::Item>& getAllGroups() noexcept               { HAssert (itemGroups.size() > 0); return itemGroups; }
    const Array<Project::Item>& getAllGroups() const noexcept   { HAssert (itemGroups.size() > 0); return itemGroups; }
    ...
    private:
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (BuildConfiguration)
    };
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ConfigIterator)
    };
    ...
    private:
        const ProjectExporter& exporter;
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ConstConfigIterator)
    };
...
private:
    ...
    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ProjectExporter)
};
