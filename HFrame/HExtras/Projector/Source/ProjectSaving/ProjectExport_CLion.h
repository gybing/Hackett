...
public:
    ...
    void create (const OwnedArray<LibraryModule>&) const override
    {
        ...
        out << "# Automatically generated CMakeLists, created by the Projector" << newLine
            << "# Do not edit this file! Your changes will be overwritten when you re-save the Projector project!" << newLine
            << newLine;
...
private:
    ...
    {
        HAssert (isExporterSupported (exporter));
    ...
    void writeCMakeListsMakefileSection (OutputStream& out, MakefileProjectExporter& exporter) const
    {
        ...
        for (ProjectExporter::ConstConfigIterator c (exporter); c.next();)
        {
            ...
            out << "execute_process (COMMAND uname -m OUTPUT_VARIABLE HARCH_LABEL OUTPUT_STRIP_TRAILING_WHITESPACE)" << newLine
                << newLine;
                ...
                if (target->type == ProjectType::Target::Type::AudioUnitPlugIn)
                {
                    ...
                    for (auto& item : exporter.getAllGroups())
                    {
                        if (item.getName() == ProjectSaver::getJuceCodeGroupName())
                        {
                            ...
                            RelativePath rFile ("JuceLibraryCode/include_audio_plugin_client_AU.r", RelativePath::projectFolder);
    ...
    //==============================================================================
    HDECLARE_NON_COPYABLE (CLionProjectExporter)
};
