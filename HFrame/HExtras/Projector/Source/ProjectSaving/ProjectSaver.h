...
public:
    ...
    public:
        ...
        HDECLARE_NON_COPYABLE (SaveThread)
    };
    ...
    static const char* getGeneratedGroupID() noexcept               { return "__Hlibfiles"; }
    Project::Item& getGeneratedCodeGroup()                          { return generatedFilesGroup; }

    static String getJuceCodeGroupName()                            { return "H Library Code"; }
...
private:
    ...
    String loadUserContentFromAppConfig() const
    {
        ...
        {
            userContent.add ({});
            userContent.add ("// (You can add your own code in this section, and the Projector will not overwrite it)");
            userContent.add ({});
        }
    ...
    void checkModuleValidity (OwnedArray<LibraryModule>& modules)
    {
        ...
        for (auto moduleIter = modules.begin(); moduleIter != modules.end(); ++moduleIter)
        {
            if (auto* module = *moduleIter)
            {
                if (! module->isValid())
                {
                    addError ("At least one of your H module paths is invalid!\n"
                              "Please go to the Modules settings page and ensure each path points to the correct H modules folder.");
                    return;
                }
    ...
    void writeAppConfig (MemoryOutputStream& out, const OwnedArray<LibraryModule>& modules, const String& userContent)
    {
        ...
        out << "    There's a section below where you can add your own custom code safely, and the" << newLine
            << "    Projector will preserve the contents of that block, but the best way to change" << newLine
            << "    any of these definitions is by using the Projector's project settings." << newLine
            << newLine
        ...
        out << newLine
            ...
            << "  ==============================================================================" << newLine
            << newLine
            << "   In accordance with the terms of the H 5 End-Use License Agreement, the" << newLine
            << "   H Code in SECTION A cannot be removed, changed or otherwise rendered" << newLine
            << "   ineffective unless you have a H Indie or Pro license, or are using H" << newLine
            << "   under the GPL v3 license." << newLine
            << newLine
            << "   End User License Agreement: www.H.com/H-5-licence" << newLine
            << newLine
            << "  ==============================================================================" << newLine
            << "*/" << newLine
            << newLine
            ...
            << newLine
            << "#ifndef HDISPLAY_SPLASH_SCREEN" << newLine
            << " #define HDISPLAY_SPLASH_SCREEN "   << (project.shouldDisplaySplashScreen() ? "1" : "0") << newLine
            << "#endif" << newLine << newLine

            << "#ifndef HREPORT_APP_USAGE" << newLine
            << " #define HREPORT_APP_USAGE "        << (project.shouldReportAppUsage()      ? "1" : "0") << newLine
            << "#endif" << newLine
            << newLine
            << "// END SECTION A" << newLine
            << newLine
            << "#define HUSE_DARK_SPLASH_SCREEN "  << (project.getSplashScreenColourString() == "Dark" ? "1" : "0") << newLine;
...

        {
            ...
            out << newLine
                << "//==============================================================================" << newLine
                << "#ifndef    HSTANDALONE_APPLICATION" << newLine
                << " #if defined(JucePlugin_Name) && defined(JucePlugin_Build_Standalone)" << newLine
                << "  #define  HSTANDALONE_APPLICATION JucePlugin_Build_Standalone" << newLine
                << " #else" << newLine
                << "  #define  HSTANDALONE_APPLICATION " << (isStandaloneApplication ? "1" : "0") << newLine
                << " #endif" << newLine
                << "#endif" << newLine;
        }
    }
    ...
    void writeAppHeader (MemoryOutputStream& out, const OwnedArray<LibraryModule>& modules)
    {
        ...
        out << "    This is the header file that your files should include in order to get all the" << newLine
            << "    H library headers. You should avoid including the H headers directly in" << newLine
        ...
        out << newLine
            << "#if ! DONT_SET_USING_HNAMESPACE" << newLine
            << " // If your code uses a lot of H classes, then this will obviously save you" << newLine
            << " // a lot of typing, but can be disabled by setting DONT_SET_USING_HNAMESPACE." << newLine
            << " using namespace H;" << newLine
            << "#endif" << newLine
            << newLine
            << "#if ! HDONT_DECLARE_PROJECTINFO" << newLine
    ...
    void writeReadmeFile()
    {
        ...
        out << newLine
            ...
            << " ================" << newLine
            << newLine
            << "The purpose of this folder is to contain files that are auto-generated by the Projector," << newLine
            << "and ALL files in this folder will be mercilessly DELETED and completely re-written whenever" << newLine
            << "the Projector saves your project." << newLine
            << newLine
            ...
            << "modifications after the Projector has saved its changes)." << newLine;
    ...
    {
        auto unityScriptContents = replaceLineFeeds (BinaryData::UnityPluginGUIScript_cs,
                                                     projectLineFeed);
    ...
    private:
        ...
        HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ExporterJob)
    };


    HDECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ProjectSaver)
};
