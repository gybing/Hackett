...
public:
    ...
    static const char* getDefaultActivityClass()         { return "com.roli.H.JuceActivity"; }
    static const char* getDefaultApplicationClass()      { return "com.roli.H.JuceApp"; }
    ...
    void createToolchainExporterProperties (PropertyListBuilder& props)
    {
        props.add (new TextPropertyComponent (gradleVersion, "Gradle Version", 32, false),
                   "The version of gradle that is used to build this app (4.10 is fine for H)");
...
private:
    void writeCmakeFile (const File& file) const
    {
        ...
        mo << "# Automatically generated makefile, created by the Projector" << newLine
        ...
        if (! isLibrary())
            mo << "SET(BINARY_NAME \"jni\")" << newLine << newLine;

        if (project.getConfigFlag ("HUSE_ANDROID_OBOE").get())
        {
        ...
        if (getNumConfigurations() > 0)
        {
            ...
            for (ConstConfigIterator config (*this); config.next();)
            {
                ...
                mo << (first ? "IF" : "ELSEIF") << "(HBUILD_CONFIGURATION MATCHES \"" << cfg.getProductFlavourCMakeIdentifier() <<"\")" << newLine;
            ...
            if (! first)
            {
                ...
                if (config)
                {
                    ...
                    {
                        mo << "ELSE(HBUILD_CONFIGURATION MATCHES \"" << cfg->getProductFlavourCMakeIdentifier() <<"\")" << newLine;
                        mo << "    MESSAGE( FATAL_ERROR \"No matching build-configuration found.\" )" << newLine;
                        mo << "ENDIF(HBUILD_CONFIGURATION MATCHES \"" << cfg->getProductFlavourCMakeIdentifier() <<"\")" << newLine << newLine;
                    }
                }
            }
        }
    ...
    String getAndroidProductFlavours() const
    {
        ...
        for (ConstConfigIterator config (*this); config.next();)
        {
            ...
            mo << "                    arguments \"-DHBUILD_CONFIGURATION=" << cfg.getProductFlavourCMakeIdentifier() << "\""
    ...
    String getAndroidBuildTypes() const
    {
        ...
        for (int i = 0; i < numConfigs; ++i)
        {
            ...
            mo << "             signingConfig signingConfigs.HSigning" << newLine;
    ...
    String getAndroidJavaSourceSets (const OwnedArray<LibraryModule>& modules) const
    {
        ...
        if (androidCustomActivityClass.get() == getDefaultActivityClass())
            addOptJavaFolderToSourceSetsForModule (javaSourceSets, modules, "gui_basics");

        if (androidEnableRemoteNotifications.get())
            addOptJavaFolderToSourceSetsForModule (javaSourceSets, modules, "gui_extra");
    ...
    void createBaseExporterProperties (PropertyListBuilder& props)
    {
        ...
        props.add (new TextPropertyComponent (androidCustomActivityClass, "Custom Android Activity", 256, false),
                   ...
                   "should be used instead of default com.roli.H.JuceActivity. If you specify a custom Activity "
                   "then you should implement onNewIntent() function like the one in com.roli.H.JuceActivity, if "
        ...
        props.add (new TextPropertyComponent (androidCustomApplicationClass, "Custom Android Application", 256, false),
                   ...
                   "should be used instead of H's default JuceApp class. If you specify a custom App then you must "
                   "call com.roli.H.Java.initialiseH somewhere in your code before calling any H functions.");
    ...
    void createManifestExporterProperties (PropertyListBuilder& props)
    {
        ...
        props.add (new TextPropertyComponent (androidManifestCustomXmlElements, "Custom Manifest XML Content", 8192, true),
                   "You can specify custom AndroidManifest.xml content overriding the default one generated by Projector. "
                   "Projector will automatically create any missing and required XML elements and attributes "
    ...
    void copyExtraResourceFiles() const
    {
        for (ConstConfigIterator config (*this); config.next();)
        {
            ...
            if (androidEnableRemoteNotifications.get())
            {
                ...
                HAssert (file.existsAsFile() && file.getFileName() == "google-services.json");
    ...
    static LibraryModule* getCoreModule (const OwnedArray<LibraryModule>& modules)
    {
        for (int i = modules.size(); --i >= 0;)
            if (modules.getUnchecked (i)->getID() == "core")
    ...
    {
        return (isLibrary() ? File::createLegalFileName (config.getTargetBinaryNameString().trim()) : "jni");
    }
    ...
    StringPairArray getAndroidPreprocessorDefs() const
    {
        ...
        defines.set ("HANDROID", "1");
        defines.set ("HANDROID_API_VERSION", androidMinimumSDK.get());
        defines.set ("HPUSH_NOTIFICATIONS", "1");
        defines.set ("HPUSH_NOTIFICATIONS_ACTIVITY", String::formatted("\"%s\"", getJNIActivityClassName().toUTF8()));

        if (androidInAppBillingPermission.get())
            defines.set ("HIN_APP_PURCHASES", "1");

        if (supportsGLv3())
            defines.set ("HANDROID_GL_ES_VERSION_3_0", "1");

        if (androidEnableRemoteNotifications.get())
        {
            defines.set ("HFIREBASE_INSTANCE_ID_SERVICE_CLASSNAME", "com_roli_JuceFirebaseInstanceIdService");
            defines.set ("HFIREBASE_MESSAGING_SERVICE_CLASSNAME", "com_roli_JuceFirebaseMessagingService");
        }
    ...
    {
        if (project.getEnabledModules().isModuleEnabled ("opengl"))
        {
    ...
    void createServiceElements (XmlElement& application) const
    {
        if (androidEnableRemoteNotifications.get())
        {
            ...
            service->setAttribute ("android:name", "com.roli.H.JuceFirebaseMessagingService");
            ...
            service->setAttribute ("android:name", "com.roli.H.JuceFirebaseInstanceIdService");
    ...
    HDECLARE_NON_COPYABLE (AndroidProjectExporter)
};
