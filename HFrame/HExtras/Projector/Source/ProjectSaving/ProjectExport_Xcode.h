...
public:
    ...
    String getDefaultLaunchStoryboardName() const      { HAssert (iOS); return "LaunchScreen"; }
    ...
    void createExporterProperties (PropertyListBuilder& props) override
    {
        ...
        props.add (new TextPropertyComponent (customXcodeResourceFoldersValue, "Custom Xcode Resource Folders", 8192, true),
                   ...
                   "without re-saving the Projector project.");
        ...
        if (getProject().getProjectType().isAudioPlugin())
            props.add (new ChoicePropertyComponent (duplicateAppExResourcesFolderValue, "Add Duplicate Resources Folder to App Extension"),
                       "Disable this to prevent the Projector from creating a duplicate resources folder for AUv3 app extensions.");
        ...
        props.add (new TextPropertyComponent (customPListValue, "Custom PList", 8192, true),
                   ...
                   "settings that the Projector creates. BEWARE! When doing this, be careful to remove from the XML any "
                   "values that you DO want the Projector to change!");
    ...
    void updateDeprecatedProjectSettingsInteractively() override
    {
        if (hasInvalidPostBuildScript())
        {
            String alertWindowText = iOS ? "Your Xcode (iOS) Exporter settings use an invalid post-build script. Click 'Update' to remove it."
                                         : "Your Xcode (OSX) Exporter settings use a pre-H 4.2 post-build script to move the plug-in binaries to their plug-in install folders.\n\n"
                                           "Since H 4.2, this is instead done using \"AU/VST/VST2/AAX/RTAS Binary Location\" in the Xcode (OS X) configuration settings.\n\n"
    ...
    {
        // check whether the script is identical to the old one that the IntroHr used to auto-generate
    ...
    struct XcodeTarget : ProjectType::Target
    {
        ...
        Array<SourceFileInfo> getSourceFilesInfo (const Project::Item& projectItem) const
        {
                     ...
                     && owner.getProject().getTargetTypeFromFilePath (projectItem.getFile(), true) == targetType)
            {
                ...
                HAssert (info.path.getRoot() == RelativePath::buildTargetFolder);
        ...
        StringPairArray getTargetSettings (const XcodeBuildConfiguration& config) const
        {
            ...
            if (config.getTargetBinaryRelativePathString().!empty())
            {
                ...
                HAssert (! xcodeCopyToProductInstallPathAfterBuild);
        ...
        StringArray getHeaderSearchPaths (const BuildConfiguration& config) const
        {
            ...
            if (owner.project.getEnabledModules().isModuleEnabled ("audio_plugin_client"))
            {
                // Needed to compile .r files
                paths.add (owner.getModuleFolderRelativeToProject ("audio_plugin_client")
...
private:
    ...
    void writeIcnsFile (const OwnedArray<Drawable>& images, OutputStream& out) const
    {
        ...
        for (int i = 0; i < images.size(); ++i)
        {
            ...
            HAssert (image.getWidth() == image.getHeight());
        ...
        HAssert (data.getDataSize() > 0); // no suitable sized images?
    ...
    void addFrameworks() const
    {
        if (! projectType.isStaticLibrary())
        {
            ...
            if (isiOS() && project.getConfigFlag ("HUSE_CAMERA").get())
            ...
            if (! project.getConfigFlag ("HQUICKTIME").get())
    ...
    void checkAndAddFileReference (std::unique_ptr<ValueTree> v) const
    {
        ...
        if (existing >= 0)
        {
            ...
            HAssert (pbxFileReferences.getUnchecked (existing)->isEquivalentTo (*v));
        }
        else
        {
...
private:
    ...
    HDECLARE_NON_COPYABLE (XcodeProjectExporter)
};
