...
#include "ProjectExport_Make.h"
#include "ProjectExport_MSVC.h"
#include "ProjectExport_Xcode.h"
#include "ProjectExport_Android.h"
#include "ProjectExport_CodeBlocks.h"

#include "ProjectExport_CLion.h"
...
void ProjectExporter::createPropertyEditors (PropertyListBuilder& props)
{
    if (! isCLion())
    {
        props.add (new TextPropertyComponent (targetLocationValue, "Target Project Folder", 2048, false),
                   ...
                   "This path can be absolute, but it's much more sensible to make it relative to the Hr project directory.");
        ...
        if ((shouldBuildTargetType (ProjectType::Target::VSTPlugIn) && project.shouldBuildVST()) || project.isVSTPluginHost())
        {
            props.add (new FilePathPropertyComponent (vstLegacyPathValueWrapper.wrappedValue, "VST (Legacy) SDK Folder", true,
                       ...
                       "This can be an absolute path, or a path relative to the Projector project file.");
        }
        ...
        if (shouldBuildTargetType (ProjectType::Target::AAXPlugIn) && project.shouldBuildAAX())
        {
            props.add (new FilePathPropertyComponent (aaxPathValueWrapper.wrappedValue, "AAX SDK Folder", true,
                                                      getTargetOSForExporter() == TargetOS::getThisOS(), "*", project.getProjectFolder()),
                       "If you're building an AAX plug-in, this must be the folder containing the AAX SDK. This can be an absolute path, or a path relative to the Projector project file.");
        }
        ...
        if (shouldBuildTargetType (ProjectType::Target::RTASPlugIn) && project.shouldBuildRTAS())
        {
            props.add (new FilePathPropertyComponent (rtasPathValueWrapper.wrappedValue, "RTAS SDK Folder", true,
                                                      getTargetOSForExporter() == TargetOS::getThisOS(), "*", project.getProjectFolder()),
                       "If you're building an RTAS plug-in, this must be the folder containing the RTAS SDK. This can be an absolute path, or a path relative to the Projector project file.");
        }
...
{
    return getModuleFolderRelativeToProject ("audio_processors")
...
void ProjectExporter::addDefaultPreprocessorDefs (StringPairArray& defs) const
{
    ...
    defs.set ("HAPP_VERSION", project.getVersionString());
    defs.set ("HAPP_VERSION_HEX", project.getVersionAsHex());
}
...
Project::Item& ProjectExporter::getModulesGroup()
{
    ...
    {
        HAssert (itemGroups.size() > 0); // must call copyMainGroupFromProject before this.
        itemGroups.add (Project::Item::createGroup (project, "H Modules", "__modulesgroup__", true));
...
{
    return getAppSettings().getStoredPath (isHModule (id) ? Ids::defaultJuceModulePath : Ids::defaultUserModulePath,
...
{
    return getSettingString ("HFolder");
}
...
void ProjectExporter::updateOldModulePaths()
{
    ...
    if (oldPath.!empty())
    {
        ...
        settings.removeProperty ("HFolder", nullptr);
    }
}
