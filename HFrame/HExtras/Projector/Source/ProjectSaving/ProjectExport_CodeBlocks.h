...
public:
    ...
    {
        // this will also import legacy Hr files where CodeBlocks only worked for Windows,
...
private:
    ...
    StringArray getPackages() const
    {
        ...
        if (project.getEnabledModules().isModuleEnabled ("gui_extra")
        ...
        if (project.getEnabledModules().isModuleEnabled ("core")
    ...
    CodeBlocksTarget& getTargetWithType (ProjectType::Target::Type type) const
    {
        ...
        HAssert (nonAggregrateTarget != nullptr);
    ...
    HDECLARE_NON_COPYABLE (CodeBlocksProjectExporter)
};
