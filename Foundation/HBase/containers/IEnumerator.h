...
public:
	...
	IEnumObject<Enumerator<T, std::pair<TE, int> > > where_i(std::function<bool(T, int)> predicate) const
	{
	...
	IEnumObject<Enumerator<T, std::pair<TE, int> > > where(std::function<bool(T)> predicate) const
	{
		return where_i([=](T a, int) {return predicate(a); });
	}

	IEnumObject<Enumerator<T, std::pair<TE, int> > > take(int count) const
	{
	...
	IEnumObject<Enumerator<T, std::pair<TE, int> > > takeWhile_i(std::function<bool(T, int)> predicate) const
	{
		return Enumerator<T, std::pair<TE, int> >([=](std::pair<TE, int> & pair)->T {
	...
	IEnumObject<Enumerator<T, std::pair<TE, int> > > takeWhile(std::function<bool(T)> predicate) const
	{
		return takeWhile_i([=](T t, int) {return predicate(t); });
	}

	IEnumObject<Enumerator<T, std::pair<TE, int> > > skip(int count) const
	{
		return where_i([=](T, int i) {return i >= count; });
	}

	IEnumObject<Enumerator<T, std::pair<TE, int> > > skipWhile_i(std::function<bool(T, int)> predicate) const
	{
		return Enumerator<T, std::pair<TE, int> >([=](std::pair<TE, int> & pair)->T {
			if (pair.second != 0)
	...
	IEnumObject<Enumerator<T, std::pair<TE, int> > > skipWhile(std::function<bool(T)> predicate) const
	{
		return skipWhile_i([=](T t, int /*i*/) { return predicate(t); });
	}

	template<typename TRet>
	IEnumObject<Enumerator<TRet, std::pair<TE, int> > > select_i(std::function<TRet(T, int)> transform) const
	{
	...
	IEnumObject<Enumerator<decltype(get_return_type<TFunc, T, int>()), std::pair<TE, int> > > select_i(TFunc transform) const
	{
		return select_i<decltype(get_return_type<TFunc, T, int>())>(transform);
	}

	template<typename TRet>
	IEnumObject<Enumerator<TRet, std::pair<TE, int> > > select(std::function<TRet(T)> transform) const
	{
		return select_i<TRet>([=](T a, int) {return transform(a); });
	}

	template<typename TFunc>
	IEnumObject<Enumerator<decltype(get_return_type<TFunc, T>()), std::pair<TE, int> > > select(TFunc transform) const
	{
		return select<decltype(get_return_type<TFunc, T>())>(transform);
	}

	template<typename TRet>
	IEnumObject<Enumerator<TRet, std::pair<TE, int> > > cast() const
	{
		return select_i<TRet>([=](T a, int) {return a; });
	}

	template<typename TRet>
	IEnumObject<Enumerator<T, std::pair<TE, std::set<TRet> > > > distinct(std::function<TRet(T)> transform) const
	{
	...
	template<typename TFunc>
	IEnumObject<Enumerator<T, std::pair<TE, std::set<decltype(get_return_type<TFunc, T>())> > > > distinct(TFunc transform) const
	{
		return distinct<decltype(get_return_type<TFunc, T>())>(transform);
	}


	IEnumObject<Enumerator<T, std::pair<TE, std::set<T> > > > distinct() const
	{
...
public:
	...
	template<typename TRet>
	IEnumObject<Enumerator<T, IteratorContainerPair<typename std::multiset<T, transform_comparer<T, TRet> >::iterator,
	...
	template<typename TFunc>
	IEnumObject<Enumerator<T, IteratorContainerPair<typename std::multiset<T, transform_comparer<T, decltype(get_return_type<TFunc, T>())> >::iterator,
	...
	IEnumObject<Enumerator<T, IteratorContainerPair<typename std::multiset<T, transform_comparer<T, T> >::iterator,
		std::multiset<T, transform_comparer<T, T> > > > > orderBy() const
	{
		return orderBy<T>([](T a) {return a; });
	}

	IEnumObject<Enumerator<T, IteratorContainerPair<typename std::vector<T>::const_reverse_iterator, std::vector<T> > > > reverse() const
	{
	...
	IEnumObject<Enumerator<T, std::pair<bool, std::pair<TE, TE2> > > > concat(IEnumObject<TE2> rhs) const
	{
...
public:
	...
	IEnumObject<Enumerator<int, std::pair<int, std::pair<TE, T> > > > bytes(BytesDirection direction = FirstToLast) const
	{
	...
	template<typename TRet>
	IEnumObject<Enumerator<TRet, TE> > unbytes(BytesDirection direction = FirstToLast) const
	{
	...
	IEnumObject<Enumerator<int, std::pair<int, std::pair<IEnumObject<Enumerator<int, std::pair<int, std::pair<TE, T> > > >, unsigned char> > > >
		bits(BitsDirection direction = HighToLow, BytesDirection bytesDirection = FirstToLast) const
	{
		typedef std::pair<int, std::pair<IEnumObject<Enumerator<int, std::pair<int, std::pair<TE, T> > > >, unsigned char> > DataType;
	...
	IEnumObject<Enumerator<unsigned char, TE> > unbits(BitsDirection direction = HighToLow) const
	{
	...
	template<typename TRet>
	IEnumObject<Enumerator<TRet, Enumerator<unsigned char, TE> > > unbits(BitsDirection direction = HighToLow, BytesDirection bytesDirection = FirstToLast) const
	{
		return unbits(direction).template unbytes<TRet>(bytesDirection);
	}

	template<typename TE_>
	friend std::ostream & operator << (std::ostream & stream, IEnumObject<TE_> linq);
};

template<typename TE>
std::ostream & operator << (std::ostream & stream, IEnumObject<TE> linq)
{
...
IEnumObject<Enumerator<T, TI>> IEnum(TI begin, TI end)
{
...
template<typename T, typename TI>
IEnumObject<Enumerator<T, std::pair<TI, int> > > IEnum(TI begin, int length)
{
...
auto IEnum(T(&array)[N])
-> decltype(IEnum<T>(array, array + N))
{
	return IEnum<T>(array, array + N);
}

template<template<class> class TV, typename TT>
auto IEnum(const TV<TT> & container)
-> decltype(IEnum<TT>(std::begin(container), std::end(container)))
{
	return IEnum<TT>(std::begin(container), std::end(container));
}

// std::list, std::vector, std::dequeue
template<template<class, class> class TV, typename TT, typename TU>
auto IEnum(const TV<TT, TU> & container)
-> decltype(IEnum<TT>(std::begin(container), std::end(container)))
{
	return IEnum<TT>(std::begin(container), std::end(container));
}
...
auto IEnum(const TV<TT, TS, TU> & container)
-> decltype(IEnum<TT>(std::begin(container), std::end(container)))
{
	return IEnum<TT>(std::begin(container), std::end(container));
}
...
auto IEnum(const TV<TK, TT, TS, TU> & container)
-> decltype(IEnum<std::pair<TK, TT> >(std::begin(container), std::end(container)))
{
	return IEnum<std::pair<TK, TT> >(std::begin(container), std::end(container));
}

// std::array
template<template<class, size_t> class TV, typename TT, size_t TL>
auto IEnum(const TV<TT, TL> & container)
-> decltype(IEnum<TT>(std::begin(container), std::end(container)))
{
	return IEnum<TT>(std::begin(container), std::end(container));
}
...
template<typename T>
IEnumObject<Enumerator<T, int> > repeat(T value, int count)
{
...
template<typename T>
IEnumObject<Enumerator<T, std::pair<bool, T> > > range(T begin, T end, T step)
{
